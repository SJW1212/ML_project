...
class PerformanceAnalyzer:
    ...
    def portfolio_daily_returns(self, assets: Dict[str, AssetData], capital_weights: Dict[str, float]) -> pd.DataFrame:
        """Build a portfolio return series without silently truncating history.

        Earlier versions used an inner join across tickers. That created a silent
        common-date cutoff when one asset had a shorter prediction file. The local
        production package now uses an outer join and treats missing per-asset
        returns as 0.0 for that asset only. Validation separately emits a WARN when
        asset date ranges differ, so the UI can surface the data coverage issue.
        """
        merged = None
        for ticker, asset in assets.items():
        ...
        ret = pd.Series(0.0, index=merged.index, dtype=float)
    ...
    def performance_metrics(self, returns: pd.Series) -> Dict[str, float]:
        ...
        annual_excess_mean = annual_mean - float(self.config.risk_free_rate)
        annual_vol = float(r.std(ddof=0) * math.sqrt(252.0))
        sharpe = annual_excess_mean / annual_vol if annual_vol > 1e-12 else np.nan
        downside_vol = float(r[r < 0].std(ddof=0) * math.sqrt(252.0)) if (r < 0).any() else np.nan
        sortino = annual_excess_mean / downside_vol if np.isfinite(downside_vol) and downside_vol > 1e-12 else np.nan
