...
class SignalClassifier:
    ...
    def _risk_class(row: pd.Series) -> str:
        ...
        if pred in {"고변동", "위험", "HIGH_VOL", "RISK_OFF"} or (np.isfinite(phv) and phv >= 0.65):
            return "HIGH_RISK"
        if (np.isfinite(phv) and phv >= 0.40) or (np.isfinite(pdown) and pdown >= 0.50):
            return "WATCH"
        return "NORMAL"
    ...
    def _direction_class(row: pd.Series) -> str:
        ...
        pdown = MathUtils.safe_float(row.get("prob_down_strengthening_score"), np.nan)
        if pred in {"상승", "UP"} or (np.isfinite(pup) and pup >= 0.45 and (not np.isfinite(pdown) or pup > pdown)):
            return "UP_STRENGTH"
        if pred in {"하락", "DOWN"} or (np.isfinite(pdown) and pdown >= 0.45 and (not np.isfinite(pup) or pdown > pup)):
    ...
    def _monitoring_note(row: pd.Series) -> str:
        ...
        pup = MathUtils.safe_float(row.get("prob_up_strengthening_score"), np.nan)
        pdown = MathUtils.safe_float(row.get("prob_down_strengthening_score"), np.nan)
        if np.isfinite(pdown) and pdown >= 0.50:
            return "DOWN_STRENGTH_WATCH"
        if np.isfinite(phv) and phv >= 0.40:
            return "HIGH_VOL_WATCH"
        if np.isfinite(pup) and pup >= 0.45:
