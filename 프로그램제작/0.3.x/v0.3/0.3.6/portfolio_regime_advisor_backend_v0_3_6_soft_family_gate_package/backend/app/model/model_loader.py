...
class ModelLoader:
    ...
    def feature_columns_path(self, model_version: str, ticker: str) -> Path:
        return self.model_dir / model_version / ticker.upper() / "feature_columns.json"

    def read_feature_columns(self, model_version: str, ticker: str) -> Optional[List[str]]:
        path = self.feature_columns_path(model_version, ticker)
        if not path.exists():
            return None
        try:
    ...
    def manifest_path(self, model_version: str, ticker: str) -> Path:
        return self.model_dir / model_version / ticker.upper() / "artifact_manifest.json"

    def read_manifest(self, model_version: str, ticker: str) -> Dict[str, Any]:
        path = self.manifest_path(model_version, ticker)
        if not path.exists():
            return {}
        try:
