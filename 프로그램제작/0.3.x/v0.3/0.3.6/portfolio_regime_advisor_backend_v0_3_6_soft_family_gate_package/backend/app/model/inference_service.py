...
from .soft_family_gate import SoftFamilyGate
...
class InferenceService:
    ...
    def __init__(self, model_loader: ModelLoader, feature_pipeline: FeaturePipeline, policy_engine: AllocationPolicyEngine | None = None, selective_service: SelectiveInferenceService | None = None, horizon_ensemble: HorizonEnsemble | None = None, soft_family_gate: SoftFamilyGate | None = None):
        ...
        self.soft_family_gate = soft_family_gate or SoftFamilyGate(ensemble=self.horizon_ensemble)
    ...
    def infer_ticker(
        ...
        # v0.3.6: soft family gate. Family failures are no longer hard-dropped.
        # Each family ensemble probability is shrunk toward neutral 0.5 according
        # to family-level confidence derived from head gates and validation metrics.
        family_soft_gates = self.soft_family_gate.evaluate_all(
            ensemble_results=ensemble_results,
            head_gates=selective.get("head_gates", {}),
            metrics=metrics,
        )
        highvol_fg = family_soft_gates["highvol"]
        up_fg = family_soft_gates["up_strength"]
        down_fg = family_soft_gates["down_strength"]

        result["raw_prob_high_vol_ensemble"] = self._clip01(highvol_ens.probability)
        result["raw_prob_up_strengthening_ensemble"] = self._clip01(up_ens.probability)
        result["raw_prob_down_strengthening_ensemble"] = self._clip01(down_ens.probability)
        result["prob_high_vol_ensemble"] = self._clip01(highvol_fg.effective_probability)
        result["prob_up_strengthening_ensemble"] = self._clip01(up_fg.effective_probability)
        result["prob_down_strengthening_ensemble"] = self._clip01(down_fg.effective_probability)
        result["effective_prob_high_vol"] = result["prob_high_vol_ensemble"]
        result["effective_prob_up_strengthening"] = result["prob_up_strengthening_ensemble"]
        result["effective_prob_down_strengthening"] = result["prob_down_strengthening_ensemble"]
        result["prob_high_vol"] = result["effective_prob_high_vol"]
        result["prob_normal"] = self._clip01(1.0 - result["prob_high_vol"])
        result["prob_overall_risk"] = result["prob_high_vol"]
        result["prob_up_strengthening_score"] = result["effective_prob_up_strengthening"]
        result["prob_down_strengthening_score"] = result["effective_prob_down_strengthening"]
        result["family_gates"] = {k: v.to_dict() for k, v in family_soft_gates.items()}
        result["family_confidence"] = {k: v.confidence for k, v in family_soft_gates.items()}
        result["family_gate_status"] = {k: v.status for k, v in family_soft_gates.items()}
        ...
        result["highvol_family_status"] = highvol_fg.status
        result["up_strength_family_status"] = up_fg.status
        result["down_strength_family_status"] = down_fg.status
