...
class AllocationService:
    ...
    @staticmethod
    def _weight_col(df: pd.DataFrame, preferred: str) -> str:
        candidates = [preferred, "stock_weight" if "stock" in preferred else preferred]
        for c in candidates:
            if c in df.columns:
                return c
        raise ValueError(f"No weight column found for {preferred}")
