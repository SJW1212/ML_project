"""
XGBoost v8.6.23 - Multi-Horizon Train-Window Ratio No-Sideway Strength Allocation
====================================================================
...
실행 예시
    py xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23.py --speed-profile balanced --h10-down-only --condition-search
    py xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23.py --speed-profile fast --h10-down-only
    py xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23.py --speed-profile full --adaptive-label --h10-down-only
...
class Config:
    ...
    result_dir: str = "results_xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23"
    ...
    # v8.6.23 No-Sideway Multi-Horizon Train-Window Ratio Upside Strength Trigger
...
def apply_direction_strength_specialist_policy(
    ...
    """
    v8.6.23 Multi-Horizon Train-Window Ratio Upside Strength Trigger.

    핵심 해석:
    - 5D / 10D / 20D UP_STRENGTHENING 라벨을 각각 학습한다.
    - 각 horizon의 학습창은 train_rows = horizon * multiplier 방식으로 다르게 둔다.
    - allocation은 5D/10D/20D 확률의 합의 구조로 결정한다.
    - 5D 단독은 약한 단기 반등, 10D+20D 및 3-horizon 합의는 강한 추세 강화로 본다.
    - RISK_OFF / EXTREME_RISK에서는 공격 전환을 차단한다.
    """
    ph = _row_float(row, "prob_high_vol")
    ...
    t5 = float(getattr(cfg, "up_strength_pred_threshold_5d", 0.30))
    ...
    if regime not in {"RISK_OFF", "EXTREME_RISK"} and pbd < block_bear:
        if pred_count == 1 and ph < low_vol_1:
            if pred5:
                target_stock = max(target_stock, float(getattr(cfg, "up_strength_single_5d_stock_weight", 0.82)))
            elif pred10:
                target_stock = max(target_stock, float(getattr(cfg, "up_strength_single_10d_stock_weight", 0.82)))
            elif pred20:
                target_stock = max(target_stock, float(getattr(cfg, "up_strength_single_20d_stock_weight", 0.82)))
            offensive_tier = 1

        elif pred_count == 2 and ph < low_vol_2:
            if pred5 and pred10:
                target_stock = max(target_stock, float(getattr(cfg, "up_strength_pair_5d_10d_stock_weight", 0.92)))
            elif pred5 and pred20:
                target_stock = max(target_stock, float(getattr(cfg, "up_strength_pair_5d_20d_stock_weight", 0.92)))
            elif pred10 and pred20:
                target_stock = max(target_stock, float(getattr(cfg, "up_strength_pair_10d_20d_stock_weight", 0.92)))
            offensive_tier = 2

        elif pred_count == 3 and ph < low_vol_3:
            target_stock = max(target_stock, float(getattr(cfg, "up_strength_all3_base_stock_weight", 1.00)))
            offensive_tier = 3
            strong_all3 = bool(
                pus_score >= float(getattr(cfg, "up_strength_all3_strong_score_threshold", 0.45))
                and p5 >= float(getattr(cfg, "up_strength_all3_strong_5d_threshold", 0.30))
                and p10 >= float(getattr(cfg, "up_strength_all3_strong_10d_threshold", 0.38))
                and p20 >= float(getattr(cfg, "up_strength_all3_strong_20d_threshold", 0.38))
                and ph < float(getattr(cfg, "up_strength_all3_strong_high_vol_threshold", 0.55))
            )
            if strong_all3:
                target_stock = max(target_stock, float(getattr(cfg, "up_strength_all3_strong_stock_weight", 1.00)))

        # 5D 단독은 단기 반등 노이즈일 가능성이 높으므로 BEAR 추세에서 과도한 spike를 제한한다.
        if trend_state == "BEAR" and five_day_only:
            target_stock = min(target_stock, max(base_stock, 0.78))
    ...
    meta = {
        ...
        "up_strength_consensus_pattern": f"{pred_pattern}:p5={p5:.4f}|p10={p10:.4f}|p20={p20:.4f}|score={pus_score:.4f}|tier={offensive_tier}",
        ...
        "p20_tier": int(3 if pred20 and p20 >= float(getattr(cfg, "up_strength_bonus_threshold_3", 0.45)) else (2 if pred20 and p20 >= float(getattr(cfg, "up_strength_bonus_threshold_2", 0.35)) else (1 if pred20 else 0))),
        "policy_note": "multi_horizon_train_window_ratio_consensus_stage1_risk_gate",
    }
    return w, meta
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23_diagnostics",
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.23 No-Sideway Multi-Horizon Train-Window Ratio Upside Strength Trigger + Objective Condition Search")
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23_summary.json"
    ...
    cfg.result_dir = f"results_xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.23: multi-horizon consensus spike. v8.6.19의 base 60%는 너무 낮아 CAGR이 눌렸으므로 상향한다.
        ...
        cfg.up_strength_weight_5d = 0.10
        cfg.up_strength_weight_10d = 0.30
        cfg.up_strength_weight_20d = 0.60
    ...
    file_prefix = f"{safe_ticker}_xgb_multi_horizon_train_window_ratio_no_sideway_v8_6_23"
