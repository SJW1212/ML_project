...
class AllocationPolicy:
    ...
    def __init__(self, config: ProductionConfig):
        self.config = config
        self.three_way_fallback_tickers: List[str] = []
        self.inverse_vol_metadata: Dict[str, Dict[str, object]] = {}
    ...
    def _inverse_vol_weights(self, assets: Dict[str, AssetData]) -> Dict[str, float]:
        ...
        for ticker, asset in assets.items():
            ...
            if self.config.inverse_vol_column in df.columns:
                ...
                source = self.config.inverse_vol_column
            if not np.isfinite(vol) or vol <= 0:
                ...
                source = "stock_next_return_tail60_std" if np.isfinite(vol) and vol > 0 else "fallback_unit_vol"
            clean_vol = max(float(vol) if np.isfinite(vol) else 1.0, self.config.inverse_vol_floor)
            raw[ticker] = 1.0 / clean_vol
            metadata[ticker] = {"vol_source": source, "annualized_vol": clean_vol}
        self.inverse_vol_metadata = metadata
    ...
    def build_allocation_rows(
        ...
        for ticker in self.config.assets:
            ...
            stock, bond, cash = self._normalize_three_way(stock, bond, cash, ticker=ticker)
    ...
    def _normalize_three_way(self, stock: float, bond: float, cash: float, *, ticker: str) -> tuple[float, float, float]:
        values = [stock, bond, cash]
        clean = [0.0 if not np.isfinite(v) else max(0.0, float(v)) for v in values]
        total = sum(clean)
        if total <= 0:
            self.three_way_fallback_tickers.append(ticker)
            return 0.0, 0.0, 1.0
