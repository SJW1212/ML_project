    ...
    def build(self, ohlcv: pd.DataFrame, include_labels: bool = False) -> pd.DataFrame:
        ...
        if include_labels:
            ...
            for h in self.config.horizons:
                ...
                # High-vol label uses the same volatility-scaled, point-in-time threshold family.
                # This avoids the unstable rolling future-absolute-return quantile that can distort
                # positive rates for several years after an extreme volatility regime.
                future_abs = future_ret.abs()
                high_vol_threshold = (self.config.k_high_vol * current_vol * np.sqrt(h)).clip(
                    lower=self.config.min_abs_threshold,
                    upper=self.config.max_abs_threshold,
                )
                df[f"high_vol_threshold_{h}d"] = high_vol_threshold
