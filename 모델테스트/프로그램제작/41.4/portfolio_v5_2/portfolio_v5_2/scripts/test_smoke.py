...
def main():
    ...
    assert payload["engine_status"]["production_ready_for_investment"] is False
    assert payload["ui"]["primary_warning"]
    assert "benchmarks" in payload and "items" in payload["benchmarks"]
    assert len(payload["latest_signals"]) == 4
    assert payload["allocation"]["allocation_rows"][0]["action_ko"]
    assert payload["performance"]["equity_curve_tail"][-1]["return_status"] == "pending"
    assert payload["performance"]["equity_curve_tail"][-1]["portfolio_return"] is None
