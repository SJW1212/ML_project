"""
XGBoost Universal Minimal v8.8.0
=================================
...
@dataclass
class Config:
    model_version: str = "v8.8.0_universal_minimal_no_downrisk"
    ...
    result_dir: str = "results_v8_8_0_minimal"
...
def save_outputs(df: pd.DataFrame, summary: Dict[str, object], diagnostics: Dict[str, pd.DataFrame], cfg: Config) -> None:
    ...
    prefix = f"{cfg.target_ticker.lower()}_xgb_universal_minimal_v8_8_0"
...
def run_single(cfg: Config) -> Dict[str, object]:
    ...
    diagnostics = {
        ...
        "monthly_returns": periodic_returns(result, "M"),
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="XGBoost Universal Minimal v8.8.0 - no Down-risk ablation")
    ...
    p.add_argument("--result-dir", default="results_v8_8_0_minimal")
...
def main() -> None:
    ...
    if len(summaries) > 1:
        ...
        comp.to_csv(root / "v8_8_0_multi_asset_summary.csv", index=False, encoding="utf-8-sig")
        try:
            (root / "v8_8_0_multi_asset_summary.md").write_text(comp.to_markdown(index=False), encoding="utf-8")
        except Exception:
            pass
        print(f"\n[MULTI] saved: {root / 'v8_8_0_multi_asset_summary.csv'}")
