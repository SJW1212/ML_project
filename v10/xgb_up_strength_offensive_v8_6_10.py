...
class Config:
    ...
    result_dir: str = "results_xgb_up_strength_offensive_v8_6_10"
    ...
    # v8.6.10 Up-Strength Offensive Overlay
    ...
    # v8.6.10 offensive thresholds
    up_strength_bonus_threshold_1: float = 0.50
    up_strength_bonus_threshold_2: float = 0.60
    bear_down_strength_cut_threshold_1: float = 0.55
    bear_down_strength_cut_threshold_2: float = 0.65
    direction_strength_max_stock_bonus: float = 0.08
    ...
    up_strength_low_vol_threshold_1: float = 0.45
    up_strength_low_vol_threshold_2: float = 0.35
    up_strength_bear_block_threshold: float = 0.60
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    """
    v8.6.10: allocation gate에서 사용할 하락 위험 점수.
...
def apply_direction_strength_specialist_policy(
    ...
    if regime == "NORMAL" and ph < low_vol_1 and pbd < block_bear:
        if pus >= t1:
        ...
        if ph < low_vol_2 and pus >= t2 and puw < 0.65:
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_up_strength_offensive_v8_6_10_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_up_strength_offensive_v8_6_10_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_up_strength_offensive_v8_6_10_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_up_strength_offensive_v8_6_10_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.10 Up-Strength Offensive Overlay + Objective Condition Search")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.10 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.10 up-strength offensive overlay 모델 학습/사용 비활성화")
...
def main() -> None:
    ...
    cfg.result_dir = f"results_xgb_up_strength_offensive_v8_6_10_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.10: Stage1 high-vol 중심 risk gate + UP_STRENGTHENING offensive overlay
        cfg.gate_normal_high_vol_threshold = 0.45
        cfg.gate_high_vol_threshold = 0.62
        cfg.gate_riskoff_downrisk_threshold = 0.62
        cfg.no_trade_band = max(cfg.no_trade_band, 0.10)
        cfg.normal_stock_weight = 0.92
        cfg.watch_stock_weight = 0.86
        cfg.risk_off_stock_weight = 0.58
        cfg.risk_off_bond_weight, cfg.risk_off_cash_weight = _risk_off_bond_cash_from_stock(cfg.risk_off_stock_weight)
        ...
        cfg.up_strength_bonus_threshold_1 = float(getattr(cfg, "up_strength_bonus_threshold_1", 0.50))
        cfg.up_strength_bonus_threshold_2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.60))
        cfg.direction_strength_max_stock_bonus = max(float(getattr(cfg, "direction_strength_max_stock_bonus", 0.08)), 0.08)
    ...
    pred_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_10_predictions.csv"
    summary_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_10_summary.json"
    latest_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_10_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_10_stage1_feature_importance.csv"
    importance_up_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_10_up_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_10_downrisk_feature_importance.csv"
    importance_down_price_trend_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_10_downrisk_price_trend_feature_importance.csv"
    importance_down_price_volume_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_10_downrisk_price_volume_feature_importance.csv"
    importance_down_volatility_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_10_downrisk_volatility_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_10_condition_search.csv"
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_up_strength_offensive_v8_6_10_{name}.csv"
