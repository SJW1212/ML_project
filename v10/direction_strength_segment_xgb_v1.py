r"""
Direction Strength Segment XGB Lab v1
====================================
...
실행 예시
    py direction_strength_label_lab_v1_1.py --profile smoke --dry-run
    py direction_strength_label_lab_v1_1.py --profile focused
    py direction_strength_label_lab_v1_1.py --base-script ..\xgb_multi_branch_pruned_features_v8_6_5.py --profile focused
    py direction_strength_label_lab_v1_1.py --ohlcv-csv qqq_ohlcv.csv --profile quick

Segment XGB v1 변경점
    - 기존 조건 유지: horizon=20, XGBoost, retrain_every_n_days=20
    - 전체 구간 학습 대신 주요 시장 구간별 train filter 추가
    - segment_xgb profile 기본 제공: 과도한 전체 탐색 없이 20개 내외 trial
    - bull_trend / bear_stress / recovery / high_vol / major_only 구간 학습 지원
    - --dry-run, --save-every 체크포인트 저장 유지
...
def mask_by_filter(df: pd.DataFrame, mode: str) -> pd.Series:
    """Training/evaluation segment masks.

    모든 mask는 time-t에서 관측 가능한 피처만 사용한다.
    따라서 segment 학습 자체는 실험 설계상 선택 편향 위험은 있지만,
    미래 라벨을 직접 참조하지 않는다.
    """
    idx = df.index
    if mode == "all":
    ...
    trend_score = current_trend_score(df)
    ...
    ret120 = df.get("return_120d", pd.Series(np.nan, index=idx))
    ...
    sideway_lowvol = (rank <= 0.60) & (trend_score.between(2, 4)) & (ret60.abs() <= 0.08)
    ...
    masks = {
        ...
        "recovery": recovery,
        "sideway_lowvol": sideway_lowvol,
        "major_only": major_only,
    }
    if mode not in masks:
        raise ValueError(f"unknown filter/segment: {mode}")
...
def _limit_horizons(requested: List[int], preferred: List[int]) -> List[int]:
    out = [h for h in preferred if h in requested]
    return out if out else requested[:1]


def make_trial_grid(args: argparse.Namespace, feature_sets: Dict[str, List[str]]) -> List[StrengthTrialConfig]:
    horizons_requested = [int(x) for x in parse_csv_arg(args.horizons)]
    models = parse_csv_arg(args.models)
    if HAS_XGB is False:
        models = [m for m in models if not m.startswith("xgb")]

    # Segment XGB 핵심: 사용자가 요청한 기존 조건을 유지한다.
    # - horizon=20
    # - XGBoost
    # - retrain_every_n_days=20
    # - 전체 구간이 아니라 주요 구간별 train filter를 비교한다.
    if args.profile == "segment_xgb":
        horizons = _limit_horizons(horizons_requested, [20])
        ret_eps_ks = [0.20]
        strength_eps_values = [0.0]
        strength_methods = ["score_delta", "hybrid_delta"]
        feature_names = [n for n in ["strength_core", "compact_mixed"] if n in feature_sets]
        filters = [
            ("all", "all"),
            ("bull_trend", "all"),
            ("bear_stress", "all"),
        ...
        models = [m for m in models if m in ["xgb"]]

    elif args.profile == "segment_xgb_eval_same":
        horizons = _limit_horizons(horizons_requested, [20])
        ret_eps_ks = [0.20]
        strength_eps_values = [0.0]
        strength_methods = ["score_delta", "hybrid_delta"]
        ...
        ret_eps_ks = [0.20]
        strength_eps_values = [0.0]
        strength_methods = ["hybrid_delta"]
        feature_names = [n for n in ["strength_core"] if n in feature_sets]
        filters = [("all", "all"), ("major_only", "all")]
        drop_sideways_options = [False]
        models = [m for m in models if m in ["xgb"]]

    elif args.profile == "quick":
        horizons = _limit_horizons(horizons_requested, [40, 60])
        ret_eps_ks = [0.20]
        ...
        filters = [("all", "all")]
        drop_sideways_options = [False]
        models = [m for m in models if m in ["hgb", "extratrees", "logistic"]]

    elif args.profile == "focused":
        horizons = _limit_horizons(horizons_requested, [40, 60])
        ret_eps_ks = [0.20, 0.30]
        strength_eps_values = [0.0, 0.5]
        strength_methods = ["hybrid_delta"]
        ...
        # v1.0의 balanced는 9,720 trial로 지나치게 컸다.
        # v1.1 balanced는 focused보다 넓지만, 여전히 현실적인 범위로 제한한다.
        horizons = _limit_horizons(horizons_requested, [20, 40, 60])
        ret_eps_ks = [0.10, 0.20, 0.30]
        strength_eps_values = [0.0, 0.5]
        strength_methods = ["score_delta", "hybrid_delta"]
        feature_names = [n for n in ["strength_core", "trend_core", "trend_volume", "compact_mixed"] if n in feature_sets]
        filters = [("all", "all"), ("non_extreme_vol", "all")]
        drop_sideways_options = [False]
        models = [m for m in models if m in ["hgb", "extratrees", "logistic"]]

    else:  # full
        horizons = horizons_requested
    ...
    if not models:
        raise ValueError("empty model list after profile filtering. Use --models hgb,extratrees,logistic or include xgb explicitly.")
    if not feature_names:
        raise ValueError("empty feature set list after profile filtering")
...
def main() -> None:
    ...
    parser.add_argument("--profile", choices=["smoke", "segment_xgb", "segment_xgb_eval_same", "quick", "focused", "balanced", "full"], default="segment_xgb")
    ...
    parser.add_argument("--max-train-rows", type=int, default=1260)
    parser.add_argument("--max-trials", type=int, default=None)
    parser.add_argument("--n-jobs", type=int, default=-1)
    parser.add_argument("--random-state", type=int, default=42)
    parser.add_argument("--result-dir", default="results_direction_strength_segment_xgb")
    parser.add_argument("--dry-run", action="store_true", help="trial grid만 출력하고 종료")
    parser.add_argument("--save-every", type=int, default=10, help="N개 trial마다 partial CSV 저장")
    ...
    # 주요 구간 mask support를 먼저 출력한다. segment 학습에서 support가 너무 작으면 fold가 skip될 수 있다.
    segment_names = ["all", "bull_trend", "bear_stress", "recovery", "high_vol", "major_only", "low_vol_only", "non_extreme_vol"]
    for seg in segment_names:
        try:
            m = mask_by_filter(df, seg)
            log(f"segment {seg}: {int(m.sum())} rows ({100.0 * float(m.mean()):.2f}%)")
        except Exception:
            pass

    trials = make_trial_grid(args, feature_sets)
    log(f"trial_count={len(trials)}, profile={args.profile}, HAS_XGB={HAS_XGB}")
    log(f"models={parse_csv_arg(args.models)}, retrain_every_n_days={args.retrain_every_n_days}, max_train_rows={args.max_train_rows}")
    if args.dry_run:
        preview = [asdict(t) for t in trials[:20]]
        print(json.dumps({"trial_count": len(trials), "preview_first_20": preview}, ensure_ascii=False, indent=2))
        return
    ...
    for i, cfg in enumerate(trials, start=1):
        ...
        try:
            ...
            pred = None
        rows.append(row)
        if args.save_every and i % max(1, args.save_every) == 0:
            partial = pd.DataFrame(rows)
            partial.to_csv(out_dir / "direction_strength_trials_partial.csv", index=False, encoding="utf-8-sig")
    ...
    summary = {
        ...
        "notes": [
            ...
            "segment_xgb는 horizon=20, XGBoost, retrain_every_n_days=20 조건을 유지한 채 주요 구간별 train filter를 비교합니다.",
            "train_filter가 특정 구간이고 eval_filter=all이면 특정 구간으로 학습한 모델의 전체 구간 일반화 성능을 봅니다.",
            "segment_xgb_eval_same은 특정 구간으로 학습하고 같은 구간에서만 평가합니다. 이는 구간 특화 성능 확인용입니다.",
