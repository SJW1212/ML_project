"""
XGBoost v8.6.18 - Controlled Consensus Multi-Horizon Upside Strength Trigger Allocation
====================================================================
...
class Config:
    ...
    extreme_stock_weight: float = 0.38
    extreme_bond_weight: float = 0.43
    extreme_cash_weight: float = 0.19

    # Base bucket allocations: low-base regime
    normal_stock_weight: float = 0.76
    normal_bond_weight: float = 0.16
    normal_cash_weight: float = 0.08

    watch_stock_weight: float = 0.66
    watch_bond_weight: float = 0.23
    watch_cash_weight: float = 0.11

    high_vol_stock_weight: float = 0.58
    high_vol_bond_weight: float = 0.28
    high_vol_cash_weight: float = 0.14

    risk_off_stock_weight: float = 0.52
    risk_off_bond_weight: float = 0.32
    risk_off_cash_weight: float = 0.16
    ...
    result_dir: str = "results_xgb_consensus_upside_strength_v8_6_18"
    ...
    # v8.6.18 Controlled Consensus Multi-Horizon Upside Strength Trigger
    ...
    up_strength_weight_5d: float = 0.10
    up_strength_weight_10d: float = 0.30
    up_strength_weight_20d: float = 0.60
    ...
    direction_strength_feature_set: str = "compact_mixed"
    ...
    # v8.6.18 controlled consensus thresholds
    ...
    up_strength_single_5d_stock_weight: float = 0.78
    up_strength_single_10d_stock_weight: float = 0.82
    up_strength_single_20d_stock_weight: float = 0.84
    up_strength_pair_5d_10d_stock_weight: float = 0.86
    up_strength_pair_5d_20d_stock_weight: float = 0.88
    ...
    # v8.6.18: 100% 진입은 매우 강한 5D/10D/20D 합의에만 허용한다.
    up_strength_offensive_stock_weight_1: float = 0.84
    up_strength_offensive_stock_weight_2: float = 0.92
    up_strength_offensive_stock_weight_3: float = 0.96
    up_strength_low_vol_threshold_1: float = 0.76
    up_strength_low_vol_threshold_2: float = 0.70
    up_strength_low_vol_threshold_3: float = 0.62
    up_strength_bear_block_threshold: float = 0.75

    # v8.6.18 diagnostics / stale offensive de-risking
    # 기본값 True: 저비중 기본형에서는 이전 공격 비중이 오래 남으면 전략 의도가 깨진다.
    enable_stale_offensive_decay: bool = True
    stale_offensive_stock_gap_threshold: float = 0.10
    stale_offensive_up_strength_reset_threshold: float = 0.22
    stale_offensive_high_vol_threshold: float = 0.70
...
def build_features(ohlcv: pd.DataFrame, horizons: Sequence[int]) -> Tuple[pd.DataFrame, List[str]]:
    ...
    feature_cols = [
        ...
        "price_ma_10_gap", "price_ma_20_gap", "price_ma_60_gap", "price_ma_120_gap", "price_ma_200_gap",
...
def ensemble_weights(cfg: Config) -> Tuple[Dict[int, float], Dict[int, float]]:
    # v8.6.18에서 cfg.horizons에는 UP_STRENGTHENING 5D 라벨 생성을 위해 5가 포함될 수 있다.
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    """
    v8.6.18: allocation gate에서 사용할 하락 위험 점수.
...
def build_direction_strength_labels(df: pd.DataFrame, cfg: Config, horizon: Optional[int] = None) -> Tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    v8.6.18에서는 5D/10D/20D specialist를 각각 학습하므로 horizon 인자를 선택적으로 받는다.
...
def apply_direction_strength_specialist_policy(
    ...
    핵심 해석:
    - 5D/10D/20D UP_STRENGTHENING 확률을 각각 상승 예측 여부로 변환한다.
    - 20D 쪽으로 갈수록 점수 가중치를 높인다.
    - 5D 단독 신호는 CUSTOM 전환을 거의 만들지 않도록 약하게 처리한다.
    - 3개 horizon 모두 상승 예측이어도 기본 목표는 96%이고, 매우 강한 합의에서만 100%를 허용한다.
    - Down-risk head는 allocation에서 계속 제외하고 Stage1 high-vol을 위험 차단기로 사용한다.
    """
    ph = _row_float(row, "prob_high_vol")
    pdn_raw = _row_float(row, "prob_down", _row_float(row, "prob_down_risk"))
    pdn_alloc = allocation_downrisk_score(ph, pdn_raw, cfg)

    pus5 = _row_float(row, "prob_up_strengthening_5d")
    pus10 = _row_float(row, "prob_up_strengthening_10d")
    pus20 = _row_float(row, "prob_up_strengthening_20d", _row_float(row, "prob_up_strengthening"))
    pus_score = get_up_strength_score_from_row(row, cfg)
    puw20 = _row_float(row, "prob_up_weakening_20d", _row_float(row, "prob_up_weakening"))
    ...
    # Horizon별 상승 예측 threshold.
    # v8.6.18: 5D는 더 엄격하게, 20D는 유지해 중기 신호를 우선한다.
    pred_t5 = float(getattr(cfg, "up_strength_pred_threshold_5d", 0.30))
    pred_t10 = float(getattr(cfg, "up_strength_pred_threshold_10d", 0.27))
    pred_t20 = float(getattr(cfg, "up_strength_pred_threshold_20d", 0.25))
    pred5 = bool(pus5 >= pred_t5)
    pred10 = bool(pus10 >= pred_t10)
    pred20 = bool(pus20 >= pred_t20)
    consensus_count = int(pred5) + int(pred10) + int(pred20)
    consensus_pattern = f"5:{int(pred5)}|10:{int(pred10)}|20:{int(pred20)}"
    horizon_weights = get_up_strength_horizon_weights(cfg)
    consensus_weighted_score = float(
        horizon_weights.get(5, 0.0) * int(pred5)
        + horizon_weights.get(10, 0.0) * int(pred10)
        + horizon_weights.get(20, 0.0) * int(pred20)
    )
    prob_spread = float(max(pus5, pus10, pus20) - min(pus5, pus10, pus20))

    low_vol_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.76))
    low_vol_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.70))
    low_vol_3 = float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.62))
    block_bear = float(getattr(cfg, "up_strength_bear_block_threshold", 0.75))

    offensive_active = False
    offensive_tier = 0
    target_stock_from_consensus = base_stock
    strong_all3 = False
    five_day_only = bool(pred5 and not pred10 and not pred20)

    if regime in {"NORMAL", "WATCH"} and pbd < block_bear:
        # 1개 horizon만 상승 예측:
        # - 5D 단독은 단기 반등 노이즈로 간주해 기본 비중 수준에 가깝게 제한한다.
        # - 10D/20D 단독은 약한 공격만 허용한다.
        if consensus_count == 1 and ph < low_vol_1:
            if pred20:
                target_stock_from_consensus = max(
                    target_stock_from_consensus,
                    float(getattr(cfg, "up_strength_single_20d_stock_weight", 0.84)),
                )
                offensive_tier = max(offensive_tier, 1)
            elif pred10:
                target_stock_from_consensus = max(
                    target_stock_from_consensus,
                    float(getattr(cfg, "up_strength_single_10d_stock_weight", 0.82)),
                )
                offensive_tier = max(offensive_tier, 1)
            elif pred5:
                target_stock_from_consensus = max(
                    target_stock_from_consensus,
                    float(getattr(cfg, "up_strength_single_5d_stock_weight", 0.78)),
                )
                # NORMAL 기본 78%에서는 CUSTOM 전환을 만들지 않도록 tier를 올리지 않는다.
                if target_stock_from_consensus > base_stock + 0.02:
                    offensive_tier = max(offensive_tier, 1)

        # 2개 horizon 상승 예측: 10D+20D 조합을 가장 강하게 인정한다.
        elif consensus_count == 2 and ph < low_vol_2:
            if pred10 and pred20:
                target_stock_from_consensus = max(
                    target_stock_from_consensus,
                    float(getattr(cfg, "up_strength_pair_10d_20d_stock_weight", 0.92)),
                )
                offensive_tier = max(offensive_tier, 2)
            elif pred5 and pred20:
                target_stock_from_consensus = max(
                    target_stock_from_consensus,
                    float(getattr(cfg, "up_strength_pair_5d_20d_stock_weight", 0.88)),
                )
                offensive_tier = max(offensive_tier, 2)
            elif pred5 and pred10:
                target_stock_from_consensus = max(
                    target_stock_from_consensus,
                    float(getattr(cfg, "up_strength_pair_5d_10d_stock_weight", 0.86)),
                )
                offensive_tier = max(offensive_tier, 2)

        # 3개 horizon 모두 상승 예측:
        # - 기본은 96%.
        # - up_score와 각 horizon 확률이 충분히 높고 high-vol이 낮을 때만 100%.
        elif consensus_count == 3 and ph < low_vol_3:
            all3_base_stock = float(getattr(cfg, "up_strength_all3_base_stock_weight", getattr(cfg, "up_strength_offensive_stock_weight_3", 0.96)))
            strong_score_t = float(getattr(cfg, "up_strength_all3_strong_score_threshold", 0.45))
            strong5_t = float(getattr(cfg, "up_strength_all3_strong_5d_threshold", 0.30))
            strong10_t = float(getattr(cfg, "up_strength_all3_strong_10d_threshold", 0.38))
            strong20_t = float(getattr(cfg, "up_strength_all3_strong_20d_threshold", 0.38))
            strong_hv_t = float(getattr(cfg, "up_strength_all3_strong_high_vol_threshold", 0.55))
            strong_all3 = bool(
                pus_score >= strong_score_t
                and pus5 >= strong5_t
                and pus10 >= strong10_t
                and pus20 >= strong20_t
                and ph < strong_hv_t
            )
            target_stock_from_consensus = max(target_stock_from_consensus, all3_base_stock)
            if strong_all3:
                target_stock_from_consensus = max(
                    target_stock_from_consensus,
                    float(getattr(cfg, "up_strength_all3_strong_stock_weight", 1.00)),
                )
            offensive_tier = max(offensive_tier, 3)

        # v8.6.17의 score-only 보조 진입은 CUSTOM 과다를 유발했다.
        # v8.6.18에서는 합의 조건이 있는 경우에만 score를 보조로 사용한다.
        if consensus_count == 2 and pred10 and pred20 and ph < low_vol_2:
            if pus_score >= float(getattr(cfg, "up_strength_bonus_threshold_2", 0.35)):
                target_stock_from_consensus = max(
                    target_stock_from_consensus,
        ...
        # 기간 간 확률 불일치가 큰 경우에는 공격 강도를 완화한다.
        if prob_spread >= 0.35:
            if consensus_count >= 3 and not strong_all3:
                target_stock_from_consensus = min(target_stock_from_consensus, max(base_stock, 0.92))
            elif consensus_count == 2:
                target_stock_from_consensus = min(target_stock_from_consensus, max(base_stock, 0.86))
            elif consensus_count == 1:
                target_stock_from_consensus = min(target_stock_from_consensus, max(base_stock, 0.80))
        elif prob_spread >= 0.25:
            if consensus_count == 1:
                target_stock_from_consensus = min(target_stock_from_consensus, max(base_stock, 0.80))
            elif consensus_count == 2:
                target_stock_from_consensus = min(target_stock_from_consensus, max(base_stock, 0.90))

        # 5D 단독은 RISK_OFF/EXTREME 이전 NORMAL/WATCH에서 과잉 CUSTOM을 만들지 않도록 마지막으로 한 번 더 제한.
        if five_day_only:
            target_stock_from_consensus = min(target_stock_from_consensus, max(base_stock, float(getattr(cfg, "up_strength_single_5d_stock_weight", 0.78))))
            if target_stock_from_consensus <= base_stock + 0.02:
                offensive_tier = 0

        stock = max(stock, target_stock_from_consensus)
    ...
    stock = float(np.clip(stock - cut, 0.35, 1.00))
    ...
    meta = {
        ...
        "direction_strength_score": float(pus_score - pbd),
        "up_strength_score": float(pus_score),
        "up_strength_5d": float(pus5),
        "up_strength_10d": float(pus10),
        "up_strength_20d": float(pus20),
        "up_strength_pred_5d": bool(pred5),
        "up_strength_pred_10d": bool(pred10),
        "up_strength_pred_20d": bool(pred20),
        "up_strength_consensus_count": int(consensus_count),
        "up_strength_consensus_pattern": str(consensus_pattern),
        "up_strength_consensus_weighted_score": float(consensus_weighted_score),
        "up_strength_prob_spread": float(prob_spread),
        "up_strength_consensus_target_stock": float(target_stock_from_consensus),
        ...
        "policy_note": "controlled_consensus_multi_horizon_upside_strength_stage1_risk_gate",
    }
    return w, meta
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        if prev_w is None:
            ...
            # v8.6.18 optional: offensive target이 사라졌는데 이전 98~100% 비중이 과도하게 남는 문제를 실험적으로 제어한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_consensus_upside_strength_v8_6_18_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.6.18 Consensus Multi-Horizon Diagnostics 결과 요약")
...
def make_condition_candidate_configs(base_cfg: Config, grid_size: str = "standard") -> List[Tuple[str, Config]]:
    ...
    if grid_size == "compact":
        ...
        # v8.6.18 standard: v8.6.10의 과도한 방어/turnover를 줄이는 공격형 후보군
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_consensus_upside_strength_v8_6_18_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_consensus_upside_strength_v8_6_18_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_consensus_upside_strength_v8_6_18_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.18 Controlled Consensus Multi-Horizon Upside Strength Trigger + Objective Condition Search")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.18 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.18 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.18 기본 ON인 stale offensive decay 비활성화")
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_consensus_upside_strength_v8_6_18_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_consensus_upside_strength_v8_6_18_summary.json"
    ...
    cfg.result_dir = f"results_xgb_consensus_upside_strength_v8_6_18_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        cfg.no_trade_band = max(cfg.no_trade_band, 0.22)
        # v8.6.18: v8.6.13보다 기본 비중을 다시 낮춘 lower-base 구조.
        # v8.6.14보다 낮은 평균 주식 비중을 목표로 하되, 상승 감지 시 공격 전환 비중을 더 크게 둔다.
        cfg.normal_stock_weight = 0.78
        cfg.normal_bond_weight = 0.15
        cfg.normal_cash_weight = 0.07
        cfg.watch_stock_weight = 0.68
        cfg.watch_bond_weight = 0.22
        cfg.watch_cash_weight = 0.10
        cfg.risk_off_stock_weight = 0.52
        cfg.risk_off_bond_weight = 0.32
        cfg.risk_off_cash_weight = 0.16
        cfg.extreme_high_vol_threshold = 0.86
        cfg.extreme_downrisk_threshold = 0.86
        cfg.extreme_stock_weight = 0.38
        cfg.extreme_bond_weight = 0.43
        cfg.extreme_cash_weight = 0.19
        ...
        cfg.up_strength_bonus_threshold_1 = float(getattr(cfg, "up_strength_bonus_threshold_1", 0.24))
        cfg.up_strength_bonus_threshold_2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.34))
        cfg.up_strength_bonus_threshold_3 = float(getattr(cfg, "up_strength_bonus_threshold_3", 0.44))
        cfg.up_strength_low_vol_threshold_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.80))
        cfg.up_strength_low_vol_threshold_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.72))
        cfg.up_strength_low_vol_threshold_3 = float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.64))
        ...
        # v8.6.18: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
        cfg.stale_offensive_stock_gap_threshold = float(getattr(cfg, "stale_offensive_stock_gap_threshold", 0.09))
        cfg.stale_offensive_up_strength_reset_threshold = float(getattr(cfg, "stale_offensive_up_strength_reset_threshold", 0.18))
        cfg.stale_offensive_high_vol_threshold = float(getattr(cfg, "stale_offensive_high_vol_threshold", 0.68))
    ...
    # v8.6.18 direct low-base overrides after policy defaults.
    if getattr(args, "base_normal_stock", None) is not None:
        cfg.normal_stock_weight = float(args.base_normal_stock)
        rem = max(0.0, 1.0 - cfg.normal_stock_weight); cfg.normal_bond_weight = rem * 2/3; cfg.normal_cash_weight = rem * 1/3
    if getattr(args, "base_watch_stock", None) is not None:
    ...
    file_prefix = f"{safe_ticker}_xgb_consensus_upside_strength_v8_6_18"
