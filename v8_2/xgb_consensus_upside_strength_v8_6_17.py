"""
XGBoost v8.6.17 - Consensus Multi-Horizon Upside Strength Trigger Allocation
====================================================================
...
class Config:
    ...
    result_dir: str = "results_xgb_consensus_upside_strength_v8_6_17"
    ...
    # v8.6.17 Consensus Multi-Horizon Upside Strength Trigger
    ...
    # v8.6.17 multi-horizon upside score thresholds
    # up_strength_score = 0.10*P5D + 0.30*P10D + 0.60*P20D
    # v8.6.17: 5D/10D/20D 각각의 상승 예측 여부를 합의(consensus)로 해석한다.
    # 세 기간 모두 상승 예측이면 100% 주식까지 허용한다.
    ...
    up_strength_confirm_10d_threshold_3: float = 0.36
    up_strength_confirm_20d_threshold_3: float = 0.36
    up_strength_pred_threshold_5d: float = 0.25
    up_strength_pred_threshold_10d: float = 0.25
    up_strength_pred_threshold_20d: float = 0.25
    up_strength_single_5d_stock_weight: float = 0.80
    up_strength_single_10d_stock_weight: float = 0.84
    up_strength_single_20d_stock_weight: float = 0.86
    up_strength_pair_5d_10d_stock_weight: float = 0.88
    up_strength_pair_5d_20d_stock_weight: float = 0.90
    up_strength_pair_10d_20d_stock_weight: float = 0.94
    ...
    # 단기 상승: 86%, 단기+중기 확인: 92%, 강한 중기 상승: 98~100%
    up_strength_offensive_stock_weight_1: float = 0.86
    up_strength_offensive_stock_weight_2: float = 0.94
    up_strength_offensive_stock_weight_3: float = 1.00
    up_strength_low_vol_threshold_1: float = 0.78
    ...
    # v8.6.17 diagnostics / stale offensive de-risking
...
def ensemble_weights(cfg: Config) -> Tuple[Dict[int, float], Dict[int, float]]:
    # v8.6.17에서 cfg.horizons에는 UP_STRENGTHENING 5D 라벨 생성을 위해 5가 포함될 수 있다.
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    """
    v8.6.17: allocation gate에서 사용할 하락 위험 점수.
...
def build_direction_strength_labels(df: pd.DataFrame, cfg: Config, horizon: Optional[int] = None) -> Tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    v8.6.17에서는 5D/10D/20D specialist를 각각 학습하므로 horizon 인자를 선택적으로 받는다.
...
def apply_direction_strength_specialist_policy(
    ...
    - 5D/10D/20D 세 기간이 모두 상승 예측이면 100% 주식까지 허용한다.
    - 5D 단독 신호는 단기 반등 가능성이 커서 매우 제한적으로만 반영한다.
    ...
    # Horizon별 상승 예측 threshold. 확률 자체가 아니라 "상승 예측 여부"를 만든다.
    pred_t5 = float(getattr(cfg, "up_strength_pred_threshold_5d", getattr(cfg, "up_strength_bonus_threshold_1", 0.25)))
    pred_t10 = float(getattr(cfg, "up_strength_pred_threshold_10d", getattr(cfg, "up_strength_bonus_threshold_1", 0.25)))
    pred_t20 = float(getattr(cfg, "up_strength_pred_threshold_20d", getattr(cfg, "up_strength_bonus_threshold_1", 0.25)))
    ...
    low_vol_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.78))
    low_vol_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.72))
    ...
    if regime in {"NORMAL", "WATCH"} and pbd < block_bear:
        # 1개 horizon만 상승 예측: 매우 제한적 반영. 20D 단독 > 10D 단독 > 5D 단독.
        if consensus_count == 1 and ph < low_vol_1:
            if pred20:
                target_stock_from_consensus = max(target_stock_from_consensus, float(getattr(cfg, "up_strength_single_20d_stock_weight", 0.86)))
                offensive_tier = max(offensive_tier, 1)
            elif pred10:
                target_stock_from_consensus = max(target_stock_from_consensus, float(getattr(cfg, "up_strength_single_10d_stock_weight", 0.84)))
                offensive_tier = max(offensive_tier, 1)
            elif pred5:
                target_stock_from_consensus = max(target_stock_from_consensus, float(getattr(cfg, "up_strength_single_5d_stock_weight", 0.80)))
                offensive_tier = max(offensive_tier, 1)

        # 2개 horizon 상승 예측: 조합별 차등. 10D+20D 조합을 가장 강하게 인정한다.
        elif consensus_count == 2 and ph < low_vol_2:
            if pred10 and pred20:
                target_stock_from_consensus = max(target_stock_from_consensus, float(getattr(cfg, "up_strength_pair_10d_20d_stock_weight", 0.94)))
                offensive_tier = max(offensive_tier, 2)
            elif pred5 and pred20:
                target_stock_from_consensus = max(target_stock_from_consensus, float(getattr(cfg, "up_strength_pair_5d_20d_stock_weight", 0.90)))
                offensive_tier = max(offensive_tier, 2)
            elif pred5 and pred10:
                target_stock_from_consensus = max(target_stock_from_consensus, float(getattr(cfg, "up_strength_pair_5d_10d_stock_weight", 0.88)))
                offensive_tier = max(offensive_tier, 2)

        # 3개 horizon 모두 상승 예측: 사용자 의도대로 100% 주식까지 허용.
        elif consensus_count == 3 and ph < low_vol_3:
            target_stock_from_consensus = max(target_stock_from_consensus, float(getattr(cfg, "up_strength_offensive_stock_weight_3", 1.00)))
            offensive_tier = max(offensive_tier, 3)

        # 확률 가중 score가 높지만 예측 합의가 부족한 경우: 10D/20D 확인이 있을 때만 보조 반영.
        # 단순 비율 매핑이 아니라, 합의 기반 결과의 보조 안전장치로만 사용한다.
        if consensus_count < 3 and ph < low_vol_1:
            if (pred10 or pred20) and pus_score >= float(getattr(cfg, "up_strength_bonus_threshold_1", 0.25)):
                target_stock_from_consensus = max(target_stock_from_consensus, float(getattr(cfg, "up_strength_offensive_stock_weight_1", 0.86)))
                offensive_tier = max(offensive_tier, 1)
            if (pred10 and pred20) and ph < low_vol_2 and pus_score >= float(getattr(cfg, "up_strength_bonus_threshold_2", 0.35)):
                target_stock_from_consensus = max(target_stock_from_consensus, float(getattr(cfg, "up_strength_offensive_stock_weight_2", 0.94)))
                offensive_tier = max(offensive_tier, 2)

        # 기간 간 확률 불일치가 크고 세 기간 합의가 아닌 경우에는 공격 강도를 완화한다.
        if prob_spread >= 0.35 and consensus_count < 3:
            target_stock_from_consensus = min(target_stock_from_consensus, max(base_stock, 0.88))
        elif prob_spread >= 0.25 and consensus_count == 1:
            target_stock_from_consensus = min(target_stock_from_consensus, max(base_stock, 0.84))
    ...
    meta = {
        ...
        "policy_note": "consensus_multi_horizon_upside_strength_stage1_risk_gate",
    }
    return w, meta
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        if prev_w is None:
            ...
            # v8.6.17 optional: offensive target이 사라졌는데 이전 98~100% 비중이 과도하게 남는 문제를 실험적으로 제어한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_consensus_upside_strength_v8_6_17_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.6.17 Consensus Multi-Horizon Diagnostics 결과 요약")
...
def make_condition_candidate_configs(base_cfg: Config, grid_size: str = "standard") -> List[Tuple[str, Config]]:
    ...
    if grid_size == "compact":
        ...
        # v8.6.17 standard: v8.6.10의 과도한 방어/turnover를 줄이는 공격형 후보군
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_consensus_upside_strength_v8_6_17_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_consensus_upside_strength_v8_6_17_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_consensus_upside_strength_v8_6_17_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.17 Consensus Multi-Horizon Upside Strength Trigger + Objective Condition Search")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.17 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.17 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.17 기본 ON인 stale offensive decay 비활성화")
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_consensus_upside_strength_v8_6_17_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_consensus_upside_strength_v8_6_17_summary.json"
    ...
    cfg.result_dir = f"results_xgb_consensus_upside_strength_v8_6_17_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.17: v8.6.13보다 기본 비중을 다시 낮춘 lower-base 구조.
        ...
        # v8.6.17: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
    ...
    # v8.6.17 direct low-base overrides after policy defaults.
    if getattr(args, "base_normal_stock", None) is not None:
        cfg.normal_stock_weight = float(args.base_normal_stock)
        rem = max(0.0, 1.0 - cfg.normal_stock_weight); cfg.normal_bond_weight = rem * 2/3; cfg.normal_cash_weight = rem * 1/3
    if getattr(args, "base_watch_stock", None) is not None:
    ...
    file_prefix = f"{safe_ticker}_xgb_consensus_upside_strength_v8_6_17"
