...
실행 예시
    py xgb_multi_horizon_no_sideway_conservative_v8_6_24.py --speed-profile balanced --h10-down-only --condition-search
    py xgb_multi_horizon_no_sideway_conservative_v8_6_24.py --speed-profile fast --h10-down-only
    py xgb_multi_horizon_no_sideway_conservative_v8_6_24.py --speed-profile full --adaptive-label --h10-down-only
...
class Config:
    ...
    result_dir: str = "results_xgb_multi_horizon_no_sideway_conservative_v8_6_24"
    ...
    # v8.6.24 No-Sideway Conservative Multi-Horizon Upside Strength Trigger
    ...
    up_strength_pred_threshold_5d: float = 0.99  # v8.6.24: 5D 단독 trigger 비활성화
    ...
    # v8.6.24: 5D는 allocation trigger에서 제외하고, 10D는 보조 확인, 20D는 핵심 조건으로 사용한다.
    ...
    up_strength_full_stock_score_threshold: float = 0.45
    up_strength_full_stock_10d_threshold: float = 0.38
    up_strength_full_stock_20d_threshold: float = 0.38
...
def apply_direction_strength_specialist_policy(
    ...
    pred5 = bool(p5 >= t5)
    if bool(getattr(cfg, "up_strength_disable_5d_trigger", True)):
        # v8.6.24: 5D는 학습/진단에는 남기지만 allocation trigger에서는 제외한다.
        pred5 = False
    pred10 = bool(p10 >= t10)
    pred20 = bool(p20 >= t20)
    ...
    if regime not in {"RISK_OFF", "EXTREME_RISK"} and pbd < block_bear:
        # v8.6.24 conservative allocation conversion
        # - 5D is diagnostic only; it cannot trigger offensive allocation.
        # - Tier 1 requires 20D confirmation.
        # - Tier 2 requires both 10D and 20D confirmation and is capped lower than v8.6.23.
        # - Tier 3 requires strong score + 20D confirmation.
        # - Full 100% requires score, 10D, 20D, and low high-vol simultaneously.
        require_20d_t2 = bool(getattr(cfg, "up_strength_require_20d_for_tier2", True))
        require_20d_t3 = bool(getattr(cfg, "up_strength_require_20d_for_tier3", True))

        tier1_signal = bool(
            pus_score >= float(getattr(cfg, "up_strength_bonus_threshold_1", 0.25))
            and pred20
            and ph < low_vol_1
        )
        tier2_signal = bool(
            pus_score >= float(getattr(cfg, "up_strength_bonus_threshold_2", 0.35))
            and pred10
            and ((not require_20d_t2) or pred20)
            and ph < low_vol_2
        )
        tier3_signal = bool(
            pus_score >= float(getattr(cfg, "up_strength_bonus_threshold_3", 0.45))
            and ((not require_20d_t3) or p20 >= float(getattr(cfg, "up_strength_confirm_20d_threshold_3", 0.38)))
            and ph < low_vol_3
        )
        full_stock_signal = bool(
            pus_score >= float(getattr(cfg, "up_strength_full_stock_score_threshold", 0.45))
            and p10 >= float(getattr(cfg, "up_strength_full_stock_10d_threshold", 0.38))
            and p20 >= float(getattr(cfg, "up_strength_full_stock_20d_threshold", 0.38))
        ...
        # BEAR mid-trend에서는 full signal이 아닌 단순 Tier 1/2 spike를 제한한다.
        if trend_state == "BEAR" and offensive_tier in {1, 2} and not full_stock_signal:
            target_stock = min(target_stock, max(base_stock, 0.86))
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_multi_horizon_no_sideway_conservative_v8_6_24_diagnostics",
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_multi_horizon_no_sideway_conservative_v8_6_24_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_multi_horizon_no_sideway_conservative_v8_6_24_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_multi_horizon_no_sideway_conservative_v8_6_24_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.24 No-Sideway Conservative Multi-Horizon Upside Strength Trigger + Objective Condition Search")
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_multi_horizon_no_sideway_conservative_v8_6_24_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_multi_horizon_no_sideway_conservative_v8_6_24_summary.json"
    ...
    cfg.result_dir = f"results_xgb_multi_horizon_no_sideway_conservative_v8_6_24_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.24: no-sideway label은 유지하되 5D trigger를 제거하고 Tier 2를 약화한다.
    ...
    file_prefix = f"{safe_ticker}_xgb_multi_horizon_no_sideway_conservative_v8_6_24"
