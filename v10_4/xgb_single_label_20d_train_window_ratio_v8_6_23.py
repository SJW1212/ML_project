"""
XGBoost v8.6.23 - Horizon Train-Window Ratio 20D Pruned Feature Spike Upside Strength Allocation
====================================================================
...
class Config:
    ...
    result_dir: str = "results_xgb_single_label_20d_train_window_ratio_v8_6_23"
    ...
    # v8.6.23 Horizon Train-Window Ratio 20D Pruned Feature Spike Upside Strength Trigger
    ...
    # v8.6.23: 100% 진입은 매우 강한 5D/10D/20D 합의에만 허용한다.
...
def build_features(ohlcv: pd.DataFrame, horizons: Sequence[int]) -> Tuple[pd.DataFrame, List[str]]:
    ...
    # v8.6.23: 20D 단일 라벨을 예측하기 위한 5/10/20 과거 구간 피처.
...
def apply_direction_strength_specialist_policy(
    ...
    meta = {
        ...
        "policy_note": "single_label_20d_train_window_ratio_p20_stage1_risk_gate",
    }
    return w, meta
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_single_label_20d_train_window_ratio_v8_6_23_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_single_label_20d_train_window_ratio_v8_6_23_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_single_label_20d_train_window_ratio_v8_6_23_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_single_label_20d_train_window_ratio_v8_6_23_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.23 Horizon Train-Window Ratio 20D Pruned Feature Spike Upside Strength Trigger + Objective Condition Search")
    ...
    parser.add_argument("--rebalance-every", type=int, default=None, help="리밸런싱 주기 직접 지정. v8.6.23 기본 5거래일")
    parser.add_argument("--emergency-cooldown", type=int, default=None, help="긴급 리밸런싱 cooldown 직접 지정. v8.6.23 기본 5거래일")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.23 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.23 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.23 기본 ON인 stale offensive decay 비활성화")
    ...
    parser.add_argument("--no-horizon-train-window", action="store_true", help="v8.6.23 horizon별 train_rows = horizon*multiplier 모드 비활성화")
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_single_label_20d_train_window_ratio_v8_6_23_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_single_label_20d_train_window_ratio_v8_6_23_summary.json"
    ...
    cfg.result_dir = f"results_xgb_single_label_20d_train_window_ratio_v8_6_23_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.23: Single-Label 20D Pruned Feature Spike Upside Strength Trigger
        ...
        # v8.6.23: 20D 단일 라벨 spike. v8.6.19의 base 60%는 너무 낮아 CAGR이 눌렸으므로 상향한다.
        ...
        # v8.6.23: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
    ...
    print("[3/5] Walk-forward Stage1 + 기존 Up/Down + horizon train-window Direction-Strength Specialist 예측")
    ...
    file_prefix = f"{safe_ticker}_xgb_single_label_20d_train_window_ratio_v8_6_23"
