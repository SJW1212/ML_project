"""
XGBoost v8.6.22 - Single-Label 20D Rolling Tier Spike Upside Strength Allocation
====================================================================
...
class Config:
    ...
    result_dir: str = "results_xgb_single_label_20d_rolling_tier_spike_v8_6_22"
    ...
    # v8.6.22 Single-Label 20D Rolling Tier Spike Upside Strength Trigger
    ...
    multi_strength_horizons: Tuple[int, ...] = (20,)
    up_strength_weight_5d: float = 0.00
    up_strength_weight_10d: float = 0.00
    up_strength_weight_20d: float = 1.00
    ...
    # v8.6.22 controlled consensus thresholds
    # up_strength_score = 0.10*P5D + 0.30*P10D + 0.60*P20D
    # 핵심: 5D 단독 신호는 약하게, 10D/20D 합의를 더 강하게 반영한다.
    # 세 기간 모두 상승 예측이어도 기본은 96%, 매우 강한 합의에서만 100%를 허용한다.
    up_strength_bonus_threshold_1: float = 0.25
    up_strength_bonus_threshold_2: float = 0.35
    up_strength_bonus_threshold_3: float = 0.45
    up_strength_confirm_10d_threshold_2: float = 0.30
    up_strength_confirm_10d_threshold_3: float = 0.38
    up_strength_confirm_20d_threshold_3: float = 0.38
    up_strength_pred_threshold_5d: float = 0.30
    ...
    up_strength_single_20d_stock_weight: float = 0.82
    up_strength_pair_5d_10d_stock_weight: float = 0.92
    up_strength_pair_5d_20d_stock_weight: float = 0.92
    up_strength_pair_10d_20d_stock_weight: float = 0.92
    up_strength_all3_base_stock_weight: float = 1.00
    ...
    direction_strength_max_stock_bonus: float = 0.35
    ...
    # v8.6.22: 100% 진입은 매우 강한 5D/10D/20D 합의에만 허용한다.
    up_strength_offensive_stock_weight_1: float = 0.82
    up_strength_offensive_stock_weight_2: float = 0.92
    up_strength_offensive_stock_weight_3: float = 1.00
    up_strength_low_vol_threshold_1: float = 0.82
    up_strength_low_vol_threshold_2: float = 0.76
    up_strength_low_vol_threshold_3: float = 0.68
    up_strength_bear_block_threshold: float = 0.75

    # v8.6.22 diagnostics / stale offensive de-risking
    ...
    # v8.6.22 Rolling Tier Optimizer
    # - 20D UP_STRENGTHENING probability(p20)를 사용하되, Tier 1/2/3 및 100% 실행 조건을
    #   전체 기간 고정값으로 두지 않고 과거 rolling window에서만 선택한다.
    # - 누수 방지를 위해 예측일 i에서는 i 이전의 realized return 구간만 최적화에 사용한다.
    use_rolling_tier_optimization: bool = True
    tier_optimize_every_n_days: int = 20
    tier_rolling_window: int = 756
    tier_min_window: int = 504
    tier_score_cagr_weight: float = 1.30
    tier_score_sharpe_weight: float = 0.30
    tier_score_calmar_weight: float = 0.30
    tier_score_mdd_weight: float = 0.60
    tier_score_turnover_weight: float = 0.04
    tier_score_tier3_penalty_weight: float = 0.03
    tier_target_tier3_rate: float = 0.08
    tier_grid_t1: Tuple[float, ...] = (0.24, 0.26)
    tier_grid_t2: Tuple[float, ...] = (0.34, 0.36)
    tier_grid_t3: Tuple[float, ...] = (0.42, 0.45)
    tier_grid_full_threshold: Tuple[float, ...] = (0.48, 0.50)
    tier_grid_stock1: Tuple[float, ...] = (0.80, 0.82)
    tier_grid_stock2: Tuple[float, ...] = (0.88, 0.90)
    tier_grid_stock3: Tuple[float, ...] = (0.96, 0.98)
    tier_grid_full_stock: Tuple[float, ...] = (1.00,)
    tier_grid_full_high_vol: Tuple[float, ...] = (0.68, 0.72)
...
def build_features(ohlcv: pd.DataFrame, horizons: Sequence[int]) -> Tuple[pd.DataFrame, List[str]]:
    ...
    # v8.6.22: 20D 단일 라벨을 예측하기 위한 5/10/20 과거 구간 피처.
...
def ensemble_weights(cfg: Config) -> Tuple[Dict[int, float], Dict[int, float]]:
    # v8.6.22에서 cfg.horizons에는 UP_STRENGTHENING 5D 라벨 생성을 위해 5가 포함될 수 있다.
...
def recompute_multi_horizon_strength_score(pred_df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    hs = get_multi_strength_horizons(cfg)
    vals: Dict[int, pd.Series] = {}
    for h in hs:
        col = f"prob_up_strengthening_{h}d"
        if col in pred_df.columns:
            vals[h] = pred_df[col].astype(float).clip(0.0, 1.0)
    if vals:
        weights = get_up_strength_horizon_weights(cfg)
        score = pd.Series(0.0, index=pred_df.index, dtype=float)
        for h, s in vals.items():
        ...
        if 5 in vals and 20 in vals:
            pred_df["prob_up_strengthening_spread_5d_20d"] = (vals[5] - vals[20]).clip(-1.0, 1.0)
    return pred_df
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            if bool(getattr(cfg, "use_direction_strength_specialist", True)):
                try:
                    ...
                    # Bear specialist는 기존처럼 20D/legacy horizon 기준 진단용으로만 유지한다.
                    y_strength_bear, valid_strength_bear, _ = build_direction_strength_labels(
                        strength_train_df, cfg, horizon=legacy_h
                    )
                    bear_strength_models = fit_direction_strength_specialist(
                        strength_train_df,
                        feature_cols,
                        y_strength_bear,
                        valid_strength_bear,
                        str(getattr(cfg, "bear_strength_train_filter", "bear_stress")),
                        cfg,
                    )
        ...
        if bool(getattr(cfg, "use_direction_strength_specialist", True)):
            ...
            bear_strength_probs = predict_direction_strength_one(bear_strength_models, row_df)
        else:
        ...
        out.update({
            ...
            "prob_up_weakening": up_strength_probs.get("UP_WEAKENING", 0.0),
            "prob_down_strengthening": up_strength_probs.get("DOWN_STRENGTHENING", 0.0),
            "prob_down_weakening": up_strength_probs.get("DOWN_WEAKENING", 0.0),
            "prob_sideways_strength": up_strength_probs.get("SIDEWAYS", 0.0),
            ...
            **{f"prob_up_weakening_{int(sh)}d": up_strength_probs_by_h.get(int(sh), {}).get("UP_WEAKENING", 0.0) for sh in multi_strength_horizons},
            **{f"actual_direction_strength_{int(sh)}d": str(strength_full_by_h[int(sh)][2].loc[date, "direction_strength_label"]) if date in strength_full_by_h[int(sh)][2].index else "" for sh in multi_strength_horizons},
            "prob_bear_up_strengthening": bear_strength_probs.get("UP_STRENGTHENING", 0.0),
            "prob_bear_up_weakening": bear_strength_probs.get("UP_WEAKENING", 0.0),
            "prob_bear_down_strengthening": bear_strength_probs.get("DOWN_STRENGTHENING", 0.0),
            "prob_bear_down_weakening": bear_strength_probs.get("DOWN_WEAKENING", 0.0),
            "prob_bear_sideways_strength": bear_strength_probs.get("SIDEWAYS", 0.0),
    ...
    if cfg.use_prob_ewma:
        prob_cols = [
            ...
            or c.startswith("prob_sideways_strength")
        ]
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    """
    v8.6.22: allocation gate에서 사용할 하락 위험 점수.
...
def build_strength_feature_sets(feature_cols: Sequence[str]) -> Dict[str, List[str]]:
    ...
    # v8.6.22: 20D single-label specialist 전용 축소 피처셋.
...
def build_direction_strength_labels(df: pd.DataFrame, cfg: Config, horizon: Optional[int] = None) -> Tuple[pd.Series, pd.Series, pd.DataFrame]:
    """5-class 방향·추세강도 라벨. trend_delta는 label 생성에만 사용하고 feature에는 넣지 않는다.

    v8.6.22에서는 5D/10D/20D specialist를 각각 학습하므로 horizon 인자를 선택적으로 받는다.
    horizon이 None이면 기존 legacy 설정(cfg.direction_strength_horizon)을 사용한다.
    """
    ...
    up = r_h >= ret_eps
    down = r_h <= -ret_eps
    y_label.loc[valid & up & (trend_delta > strength_eps)] = "UP_STRENGTHENING"
    y_label.loc[valid & up & (trend_delta <= strength_eps)] = "UP_WEAKENING"
    y_label.loc[valid & down & (trend_delta < -strength_eps)] = "DOWN_STRENGTHENING"
    y_label.loc[valid & down & (trend_delta >= -strength_eps)] = "DOWN_WEAKENING"
...
def fit_direction_strength_specialist(
    ...
    if not cols:
        return {"model": None, "cols": [], "available": False, "train_rows": 0, "classes": []}
    mask = valid_strength & mask_by_strength_filter(train_df, train_filter)
    idx = train_df.index[mask.values]
    max_rows = getattr(cfg, "direction_strength_max_train_rows", None)
    if max_rows is not None:
        idx = idx[-int(max_rows):]
    min_rows = int(getattr(cfg, "direction_strength_min_train_rows", 300))
    if len(idx) < min_rows:
        return {"model": None, "cols": cols, "available": False, "train_rows": int(len(idx)), "classes": []}
    ...
    if len(orig_classes) < 2:
        return {"model": None, "cols": cols, "available": False, "train_rows": int(len(idx)), "classes": orig_classes.tolist()}
    ...
    return {"model": model, "cols": cols, "available": True, "train_rows": int(len(idx)), "classes": orig_classes.tolist()}
...
def apply_direction_strength_specialist_policy(
    ...
    핵심 해석:
    - 예측 라벨은 20D UP_STRENGTHENING 하나로 고정한다.
    - 모델 입력 피처에는 과거 5D/10D/20D 수익률·추세·변동성·거래량·가속도 피처를 사용한다.
    - 별도의 5D/10D 라벨 예측을 합의시키지 않는다. 즉, horizon 간 라벨 충돌을 제거한다.
    - p20 하나로 82% / 92% / 100% spike를 결정한다.
    - v8.6.19보다 base stock을 높여 비추적 구간의 장기 상승장 누락을 줄인다.
    """
    ph = _row_float(row, "prob_high_vol")
    pdn_raw = _row_float(row, "prob_down", _row_float(row, "prob_down_risk"))
    pdn_alloc = allocation_downrisk_score(ph, pdn_raw, cfg)

    p20 = _row_float(row, "prob_up_strengthening_20d", _row_float(row, "prob_up_strengthening"))
    pus_score = p20
    pbd = _row_float(row, "prob_bear_down_strengthening")
    pbdw = _row_float(row, "prob_bear_down_weakening")
    trend_score, trend_state = compute_mid_trend_score(row)
    ...
    # p20 단일 확률 threshold. 기존 이름을 재사용해 CLI 호환성을 유지한다.
    t1 = float(getattr(cfg, "up_strength_bonus_threshold_1", 0.25))
    t2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.35))
    t3 = float(getattr(cfg, "up_strength_bonus_threshold_3", 0.45))

    low_vol_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.82))
    low_vol_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.76))
    low_vol_3 = float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.68))
    block_bear = float(getattr(cfg, "up_strength_bear_block_threshold", 0.75))

    pred20_t1 = bool(p20 >= t1)
    pred20_t2 = bool(p20 >= t2)
    pred20_t3 = bool(p20 >= t3)
    offensive_tier = 0
    target_stock_from_p20 = base_stock
    offensive_active = False

    # RISK_OFF / EXTREME_RISK에서는 공격 전환을 막는다.
    # NORMAL/WATCH/HIGH_VOL에서만 p20 spike를 허용한다.
    if regime not in {"RISK_OFF", "EXTREME_RISK"} and pbd < block_bear:
        if pred20_t3 and ph < low_vol_3:
            target_stock_from_p20 = max(
                target_stock_from_p20,
                float(getattr(cfg, "up_strength_offensive_stock_weight_3", 1.00)),
            )
            offensive_tier = 3
        elif pred20_t2 and ph < low_vol_2:
            target_stock_from_p20 = max(
                target_stock_from_p20,
                float(getattr(cfg, "up_strength_offensive_stock_weight_2", 0.92)),
            )
            offensive_tier = 2
        elif pred20_t1 and ph < low_vol_1:
            target_stock_from_p20 = max(
                target_stock_from_p20,
                float(getattr(cfg, "up_strength_offensive_stock_weight_1", 0.82)),
            )
            offensive_tier = 1

        # 중기 추세가 BEAR이고 p20이 임계값만 간신히 넘은 경우에는 약한 spike를 제한한다.
        if trend_state == "BEAR" and offensive_tier == 1 and p20 < (t1 + 0.05):
            target_stock_from_p20 = min(target_stock_from_p20, max(base_stock, 0.78))

        stock = max(stock, target_stock_from_p20)
        bonus = max(0.0, stock - base_stock)
        offensive_active = bool(offensive_tier > 0 and bonus > 1e-12)

    if bool(getattr(cfg, "use_bear_specialist_cut", False)):
        if ph >= 0.65 and pbd >= float(getattr(cfg, "bear_down_strength_cut_threshold_1", 0.55)):
            cut += 0.02
        if ph >= 0.75 and pbd >= float(getattr(cfg, "bear_down_strength_cut_threshold_2", 0.65)):
            cut += 0.02
        if ph >= 0.65 and pbdw >= 0.65 and pbd < 0.40:
            cut = max(0.0, cut - 0.01)

    max_bonus = float(getattr(cfg, "direction_strength_max_stock_bonus", 0.35))
    ...
    if regime in {"RISK_OFF", "EXTREME_RISK"}:
        stock = min(stock, base_stock)
        bonus = 0.0
        offensive_active = False
        offensive_tier = 0
        target_stock_from_p20 = base_stock
    ...
    meta = {
        ...
        "mid_trend_score": trend_score,
        "mid_trend_state": trend_state,
        "defensive_risk_score": float(np.clip(0.85 * ph + 0.15 * pdn_alloc, 0.0, 1.0)),
        "direction_strength_score": float(p20 - pbd),
        "up_strength_score": float(p20),
        "up_strength_5d": 0.0,
        "up_strength_10d": 0.0,
        "up_strength_20d": float(p20),
        "up_strength_pred_5d": False,
        "up_strength_pred_10d": False,
        "up_strength_pred_20d": bool(pred20_t1),
        "up_strength_consensus_count": int(offensive_tier),
        "up_strength_consensus_pattern": f"20D:p={p20:.4f}|tier={offensive_tier}",
        "up_strength_consensus_weighted_score": float(p20),
        "up_strength_prob_spread": 0.0,
        "up_strength_consensus_target_stock": float(target_stock_from_p20),
        "up_strength_strong_all3": bool(offensive_tier == 3),
        "up_strength_five_day_only": False,
        ...
        "p20_tier": int(offensive_tier),
        "policy_note": "single_label_20d_rolling_tier_spike_p20_stage1_risk_gate",
    }
    return w, meta


def default_tier_config_from_cfg(cfg: Config) -> Dict[str, object]:
    """현재 Config의 고정 tier 설정을 rolling optimizer 후보와 동일한 dict 형식으로 변환한다."""
    return {
        "name": "tier_static_default",
        "t1": float(getattr(cfg, "up_strength_bonus_threshold_1", 0.25)),
        "t2": float(getattr(cfg, "up_strength_bonus_threshold_2", 0.35)),
        "t3": float(getattr(cfg, "up_strength_bonus_threshold_3", 0.45)),
        "full_threshold": float(getattr(cfg, "up_strength_bonus_threshold_3", 0.45)),
        "stock1": float(getattr(cfg, "up_strength_offensive_stock_weight_1", 0.82)),
        "stock2": float(getattr(cfg, "up_strength_offensive_stock_weight_2", 0.92)),
        "stock3": float(getattr(cfg, "up_strength_offensive_stock_weight_3", 1.00)),
        "full_stock": 1.00,
        "low_vol1": float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.82)),
        "low_vol2": float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.76)),
        "low_vol3": float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.68)),
        "full_high_vol": float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.68)),
        "bear_block": float(getattr(cfg, "up_strength_bear_block_threshold", 0.75)),
        "score": np.nan,
        "opt_cagr": np.nan,
        "opt_mdd": np.nan,
        "opt_sharpe": np.nan,
        "opt_calmar": np.nan,
        "opt_turnover": np.nan,
        "opt_tier3_rate": np.nan,
    }


def build_rolling_tier_grid(cfg: Config) -> List[Dict[str, object]]:
    """과최적화와 속도 문제를 줄이기 위해 작고 단조 제약이 있는 후보만 생성한다."""
    grid: List[Dict[str, object]] = []
    idx = 0
    for t1 in tuple(getattr(cfg, "tier_grid_t1", (0.24, 0.26))):
        for t2 in tuple(getattr(cfg, "tier_grid_t2", (0.34, 0.36))):
            for t3 in tuple(getattr(cfg, "tier_grid_t3", (0.42, 0.45))):
                for ft in tuple(getattr(cfg, "tier_grid_full_threshold", (0.48, 0.50))):
                    if not (float(t1) < float(t2) < float(t3) <= float(ft)):
                        continue
                    for s1 in tuple(getattr(cfg, "tier_grid_stock1", (0.80, 0.82))):
                        for s2 in tuple(getattr(cfg, "tier_grid_stock2", (0.88, 0.90))):
                            for s3 in tuple(getattr(cfg, "tier_grid_stock3", (0.96, 0.98))):
                                for fs in tuple(getattr(cfg, "tier_grid_full_stock", (1.00,))):
                                    if not (float(s1) <= float(s2) <= float(s3) <= float(fs)):
                                        continue
                                    for fhv in tuple(getattr(cfg, "tier_grid_full_high_vol", (0.68, 0.72))):
                                        name = (
                                            f"rtier_{idx:04d}_"
                                            f"t{float(t1):.2f}-{float(t2):.2f}-{float(t3):.2f}-f{float(ft):.2f}_"
                                            f"s{float(s1):.2f}-{float(s2):.2f}-{float(s3):.2f}-{float(fs):.2f}_"
                                            f"hv{float(fhv):.2f}"
                                        )
                                        grid.append({
                                            "name": name,
                                            ...
                                            "low_vol2": float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.76)),
                                            "low_vol3": float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.68)),
                                            "full_high_vol": float(fhv),
                                            "bear_block": float(getattr(cfg, "up_strength_bear_block_threshold", 0.75)),
                                        })
                                        idx += 1
    return grid or [default_tier_config_from_cfg(cfg)]


def apply_direction_strength_specialist_policy_with_tier_config(
    base_w: Tuple[float, float, float],
    regime: str,
    row: pd.Series,
    cfg: Config,
    ...
    """
    v8.6.22 rolling tier version of the 20D single-label spike policy.

    tier_cfg is selected from past-only rolling optimization. It controls:
    - Tier 1/2/3 thresholds
    - Tier 1/2/3 target stock weights
    - full-stock threshold and high-vol cap
    """
    ph = _row_float(row, "prob_high_vol")
    pdn_raw = _row_float(row, "prob_down", _row_float(row, "prob_down_risk"))
    pdn_alloc = allocation_downrisk_score(ph, pdn_raw, cfg)
    ...
    t1 = float(tier_cfg.get("t1", getattr(cfg, "up_strength_bonus_threshold_1", 0.25)))
    t2 = float(tier_cfg.get("t2", getattr(cfg, "up_strength_bonus_threshold_2", 0.35)))
    t3 = float(tier_cfg.get("t3", getattr(cfg, "up_strength_bonus_threshold_3", 0.45)))
    ft = float(tier_cfg.get("full_threshold", t3))
    s1 = float(tier_cfg.get("stock1", getattr(cfg, "up_strength_offensive_stock_weight_1", 0.82)))
    s2 = float(tier_cfg.get("stock2", getattr(cfg, "up_strength_offensive_stock_weight_2", 0.92)))
    s3 = float(tier_cfg.get("stock3", getattr(cfg, "up_strength_offensive_stock_weight_3", 1.00)))
    fs = float(tier_cfg.get("full_stock", 1.00))
    low_vol_1 = float(tier_cfg.get("low_vol1", getattr(cfg, "up_strength_low_vol_threshold_1", 0.82)))
    low_vol_2 = float(tier_cfg.get("low_vol2", getattr(cfg, "up_strength_low_vol_threshold_2", 0.76)))
    low_vol_3 = float(tier_cfg.get("low_vol3", getattr(cfg, "up_strength_low_vol_threshold_3", 0.68)))
    full_hv = float(tier_cfg.get("full_high_vol", low_vol_3))
    block_bear = float(tier_cfg.get("bear_block", getattr(cfg, "up_strength_bear_block_threshold", 0.75)))
    ...
    if regime not in {"RISK_OFF", "EXTREME_RISK"} and pbd < block_bear:
        if p20 >= ft and ph < full_hv:
            target_stock_from_p20 = max(target_stock_from_p20, fs)
            offensive_tier = 3
            full_stock_signal = True
        elif pred20_t3 and ph < low_vol_3:
            target_stock_from_p20 = max(target_stock_from_p20, s3)
            offensive_tier = 3
        elif pred20_t2 and ph < low_vol_2:
            target_stock_from_p20 = max(target_stock_from_p20, s2)
            offensive_tier = 2
        elif pred20_t1 and ph < low_vol_1:
            target_stock_from_p20 = max(target_stock_from_p20, s1)
            offensive_tier = 1

        if trend_state == "BEAR" and offensive_tier == 1 and p20 < (t1 + 0.05):
            target_stock_from_p20 = min(target_stock_from_p20, max(base_stock, min(s1, 0.78)))
    ...
    offensive_active = bool(offensive_tier > 0 and max(0.0, stock - base_stock) > 1e-12)
    w = _redistribute_after_stock_change(stock, base_w)
    meta = {
        "policy_overlay": float(w[0] - base_w[0]),
        "mid_trend_score": trend_score,
        ...
        "up_strength_pred_20d": bool(pred20_t1),
        "up_strength_consensus_count": int(offensive_tier),
        "up_strength_consensus_pattern": f"20D:p={p20:.4f}|tier={offensive_tier}|cfg={tier_cfg.get('name', 'tier_unknown')}",
        "up_strength_consensus_weighted_score": float(p20),
        "up_strength_prob_spread": 0.0,
        "up_strength_consensus_target_stock": float(target_stock_from_p20),
        "up_strength_strong_all3": bool(offensive_tier == 3),
        ...
        "p20_up_strengthening": float(p20),
        "p20_tier": int(offensive_tier),
        "p20_full_stock_signal": bool(full_stock_signal),
        "policy_note": "single_label_20d_rolling_tier_spike_p20_stage1_risk_gate",
    }
    return w, meta


def tier_optimizer_score(stats: Dict[str, float], cfg: Config) -> float:
    annual_turnover = float(stats.get("annual_turnover", stats.get("avg_turnover", 0.0) * 252.0))
    tier3_rate = float(stats.get("tier3_rate", 0.0))
    tier3_excess = max(0.0, tier3_rate - float(getattr(cfg, "tier_target_tier3_rate", 0.08)))
    return float(
        float(getattr(cfg, "tier_score_cagr_weight", 1.30)) * float(stats.get("cagr", 0.0))
        + float(getattr(cfg, "tier_score_sharpe_weight", 0.30)) * float(stats.get("sharpe", 0.0))
        + float(getattr(cfg, "tier_score_calmar_weight", 0.30)) * float(stats.get("calmar", 0.0))
        - float(getattr(cfg, "tier_score_mdd_weight", 0.60)) * abs(float(stats.get("mdd", 0.0)))
        - float(getattr(cfg, "tier_score_turnover_weight", 0.04)) * annual_turnover
        - float(getattr(cfg, "tier_score_tier3_penalty_weight", 0.03)) * tier3_excess
    )


def simulate_tier_config_on_hist(
    hist_df: pd.DataFrame,
    gate_cfg: Dict[str, float],
    tier_cfg: Dict[str, object],
    cfg: Config,
) -> Dict[str, float]:
    """최근 과거 구간에서 후보 tier config를 평가한다. hist_df는 반드시 현재 시점 이전 행만 포함해야 한다."""
    prev_w: Optional[Tuple[float, float, float]] = None
    rets: List[float] = []
    turnovers: List[float] = []
    stock_weights: List[float] = []
    tier3_flags: List[bool] = []
    full_flags: List[bool] = []
    last_emergency_i = -10**9

    for j, (_, row) in enumerate(hist_df.iterrows()):
        ...
        signal_regime = classify_gate(ph, pdn, gate_cfg)
        base_signal_w = apply_continuous_adjustment(base_weight_for_regime(signal_regime, gate_cfg), ph, pdn, cfg)
        signal_w, meta = apply_direction_strength_specialist_policy_with_tier_config(base_signal_w, signal_regime, row, cfg, tier_cfg)
        ...
        emergency = bool(raw_emergency and (j - last_emergency_i >= cfg.emergency_cooldown_days))
        scheduled = (j % cfg.rebalance_every_n_days == 0)
        ...
        if prev_w is None:
            ...
            stale_decay = (
                ...
                and stale_gap >= float(getattr(cfg, "stale_offensive_stock_gap_threshold", 0.12))
                and (
                    stale_up_prob < float(getattr(cfg, "stale_offensive_up_strength_reset_threshold", 0.20))
                    or ph >= float(getattr(cfg, "stale_offensive_high_vol_threshold", 0.72))
                )
            )
            w = signal_w if stale_decay else prev_w
        else:
            total_delta_to_signal = sum(abs(signal_w[k] - prev_w[k]) for k in range(3))
            w = prev_w if total_delta_to_signal < gate_cfg["no_trade_band"] else signal_w

        turnover = 0.0 if prev_w is None else sum(abs(w[k] - prev_w[k]) for k in range(3))
        gross = w[0] * float(row["stock_next_return"]) + w[1] * float(row["bond_next_return"]) + w[2] * float(row["cash_next_return"])
        net = gross - float(cfg.transaction_cost_rate) * turnover
        rets.append(float(net))
        turnovers.append(float(turnover))
        stock_weights.append(float(w[0]))
        tier3_flags.append(bool(int(meta.get("offensive_tier", 0)) >= 3))
        full_flags.append(bool(meta.get("p20_full_stock_signal", False)))
    ...
    stats["annual_turnover"] = float(stats["avg_turnover"] * 252.0)
    stats["avg_stock_weight"] = float(np.mean(stock_weights)) if stock_weights else 0.0
    stats["tier3_rate"] = float(np.mean(tier3_flags)) if tier3_flags else 0.0
    stats["full_stock_signal_rate"] = float(np.mean(full_flags)) if full_flags else 0.0
    stats["score"] = tier_optimizer_score(stats, cfg)
    return stats


def select_best_tier_config(
    hist_df: pd.DataFrame,
    gate_cfg: Dict[str, float],
    tier_grid: List[Dict[str, object]],
    cfg: Config,
) -> Dict[str, object]:
    best_cfg = default_tier_config_from_cfg(cfg)
    best_stats: Dict[str, float] = {"score": -np.inf}
    for cand in tier_grid:
        st = simulate_tier_config_on_hist(hist_df, gate_cfg, cand, cfg)
        if float(st.get("score", -np.inf)) > float(best_stats.get("score", -np.inf)):
            best_cfg = dict(cand)
            best_stats = st
    best_cfg.update({
        "score": float(best_stats.get("score", np.nan)),
        "opt_cagr": float(best_stats.get("cagr", np.nan)),
        "opt_mdd": float(best_stats.get("mdd", np.nan)),
        "opt_sharpe": float(best_stats.get("sharpe", np.nan)),
        "opt_calmar": float(best_stats.get("calmar", np.nan)),
        "opt_turnover": float(best_stats.get("annual_turnover", np.nan)),
        "opt_tier3_rate": float(best_stats.get("tier3_rate", np.nan)),
        "opt_full_stock_signal_rate": float(best_stats.get("full_stock_signal_rate", np.nan)),
    })
    return best_cfg
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    current_g = default_g
    usage: Dict[str, int] = {}
    tier_grid = build_rolling_tier_grid(cfg)
    current_tier_cfg: Dict[str, object] = default_tier_config_from_cfg(cfg)
    ...
    for i, row in pred_df.iterrows():
        ...
        if (
            bool(getattr(cfg, "use_rolling_tier_optimization", True))
            and i >= int(getattr(cfg, "tier_min_window", 504))
            and i % int(getattr(cfg, "tier_optimize_every_n_days", 20)) == 0
        ):
            hist_tier = pred_df.iloc[max(0, i - int(getattr(cfg, "tier_rolling_window", 756))):i].copy()
            current_tier_cfg = select_best_tier_config(hist_tier, current_g, tier_grid, cfg)
        ...
        if (
            str(getattr(cfg, "policy_mode", "base")).lower() in {"direction_strength_specialist", "strength_specialist", "ds_specialist"}
            and bool(getattr(cfg, "use_rolling_tier_optimization", True))
        ):
            signal_w, policy_meta = apply_direction_strength_specialist_policy_with_tier_config(
                base_signal_w, signal_regime, row, cfg, current_tier_cfg
            )
        else:
        ...
        if prev_w is None:
            ...
            # v8.6.22 optional: offensive target이 사라졌는데 이전 98~100% 비중이 과도하게 남는 문제를 실험적으로 제어한다.
        ...
        out.update({
            ...
            "rolling_tier_config": str(current_tier_cfg.get("name", "tier_static_default")),
            "selected_tier1_threshold": float(current_tier_cfg.get("t1", np.nan)),
            "selected_tier2_threshold": float(current_tier_cfg.get("t2", np.nan)),
            "selected_tier3_threshold": float(current_tier_cfg.get("t3", np.nan)),
            "selected_full_stock_threshold": float(current_tier_cfg.get("full_threshold", np.nan)),
            "selected_full_high_vol_threshold": float(current_tier_cfg.get("full_high_vol", np.nan)),
            "selected_tier1_stock": float(current_tier_cfg.get("stock1", np.nan)),
            "selected_tier2_stock": float(current_tier_cfg.get("stock2", np.nan)),
            "selected_tier3_stock": float(current_tier_cfg.get("stock3", np.nan)),
            "selected_full_stock": float(current_tier_cfg.get("full_stock", np.nan)),
            "tier_config_score": float(current_tier_cfg.get("score", np.nan)) if pd.notna(current_tier_cfg.get("score", np.nan)) else np.nan,
            "tier_config_cagr": float(current_tier_cfg.get("opt_cagr", np.nan)) if pd.notna(current_tier_cfg.get("opt_cagr", np.nan)) else np.nan,
            "tier_config_mdd": float(current_tier_cfg.get("opt_mdd", np.nan)) if pd.notna(current_tier_cfg.get("opt_mdd", np.nan)) else np.nan,
            "tier_config_sharpe": float(current_tier_cfg.get("opt_sharpe", np.nan)) if pd.notna(current_tier_cfg.get("opt_sharpe", np.nan)) else np.nan,
            "tier_config_calmar": float(current_tier_cfg.get("opt_calmar", np.nan)) if pd.notna(current_tier_cfg.get("opt_calmar", np.nan)) else np.nan,
            "tier_config_turnover": float(current_tier_cfg.get("opt_turnover", np.nan)) if pd.notna(current_tier_cfg.get("opt_turnover", np.nan)) else np.nan,
            "tier_config_tier3_rate": float(current_tier_cfg.get("opt_tier3_rate", np.nan)) if pd.notna(current_tier_cfg.get("opt_tier3_rate", np.nan)) else np.nan,
            ...
            "p20_tier": int(policy_meta.get("p20_tier", policy_meta.get("offensive_tier", 0))),
            "p20_full_stock_signal": bool(policy_meta.get("p20_full_stock_signal", False)),
...
def classification_metrics(pred_df: pd.DataFrame, cfg: Config) -> Dict[str, object]:
    ...
    if "actual_direction_strength" in pred_df.columns:
        ...
        if "prob_bear_down_strengthening" in pred_df.columns:
            y_ds = (actual_strength == "DOWN_STRENGTHENING").astype(int).values
            p_ds = pred_df["prob_bear_down_strengthening"].astype(float).clip(0.0, 1.0).values
            metrics["direction_strength_bear_down_strengthening"] = binary_cls_metrics(
                y_ds, p_ds, float(getattr(cfg, "bear_down_strength_cut_threshold_1", 0.45)), "DOWN_STRENGTHENING"
            )
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_single_label_20d_rolling_tier_spike_v8_6_22_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
        ...
        "rolling_tier_optimizer": {
            "enabled": bool(getattr(cfg, "use_rolling_tier_optimization", True)),
            "optimize_every_n_days": int(getattr(cfg, "tier_optimize_every_n_days", 20)),
            "rolling_window": int(getattr(cfg, "tier_rolling_window", 756)),
            "min_window": int(getattr(cfg, "tier_min_window", 504)),
            "candidate_count": int(len(build_rolling_tier_grid(cfg))),
            "usage_top10": pred_df.get("rolling_tier_config", pd.Series("", index=pred_df.index)).value_counts().head(10).to_dict(),
            "latest_config": str(pred_df.get("rolling_tier_config", pd.Series("", index=pred_df.index)).iloc[-1]) if len(pred_df) else "",
            "avg_selected_tier1_threshold": float(pred_df.get("selected_tier1_threshold", pd.Series(np.nan, index=pred_df.index)).astype(float).mean()),
            "avg_selected_tier2_threshold": float(pred_df.get("selected_tier2_threshold", pd.Series(np.nan, index=pred_df.index)).astype(float).mean()),
            "avg_selected_tier3_threshold": float(pred_df.get("selected_tier3_threshold", pd.Series(np.nan, index=pred_df.index)).astype(float).mean()),
            "avg_selected_full_stock_threshold": float(pred_df.get("selected_full_stock_threshold", pd.Series(np.nan, index=pred_df.index)).astype(float).mean()),
            "avg_selected_tier1_stock": float(pred_df.get("selected_tier1_stock", pd.Series(np.nan, index=pred_df.index)).astype(float).mean()),
            "avg_selected_tier2_stock": float(pred_df.get("selected_tier2_stock", pd.Series(np.nan, index=pred_df.index)).astype(float).mean()),
            "avg_selected_tier3_stock": float(pred_df.get("selected_tier3_stock", pd.Series(np.nan, index=pred_df.index)).astype(float).mean()),
            "avg_selected_full_stock": float(pred_df.get("selected_full_stock", pd.Series(np.nan, index=pred_df.index)).astype(float).mean()),
        },
        ...
        "latest_prediction": {
            ...
            "prob_up_weakening": round(float(latest.get("prob_up_weakening", 0.0)) * 100, 2),
            "prob_bear_down_strengthening": round(float(latest.get("prob_bear_down_strengthening", 0.0)) * 100, 2),
            "prob_bear_down_weakening": round(float(latest.get("prob_bear_down_weakening", 0.0)) * 100, 2),
            ...
            "rolling_tier_config": str(latest.get("rolling_tier_config", "")),
            "selected_tier1_threshold": round(float(latest.get("selected_tier1_threshold", np.nan)) * 100, 2) if pd.notna(latest.get("selected_tier1_threshold", np.nan)) else None,
            "selected_tier2_threshold": round(float(latest.get("selected_tier2_threshold", np.nan)) * 100, 2) if pd.notna(latest.get("selected_tier2_threshold", np.nan)) else None,
            "selected_tier3_threshold": round(float(latest.get("selected_tier3_threshold", np.nan)) * 100, 2) if pd.notna(latest.get("selected_tier3_threshold", np.nan)) else None,
            "selected_full_stock_threshold": round(float(latest.get("selected_full_stock_threshold", np.nan)) * 100, 2) if pd.notna(latest.get("selected_full_stock_threshold", np.nan)) else None,
            "selected_tier1_stock": round(float(latest.get("selected_tier1_stock", np.nan)) * 100, 2) if pd.notna(latest.get("selected_tier1_stock", np.nan)) else None,
            "selected_tier2_stock": round(float(latest.get("selected_tier2_stock", np.nan)) * 100, 2) if pd.notna(latest.get("selected_tier2_stock", np.nan)) else None,
            "selected_tier3_stock": round(float(latest.get("selected_tier3_stock", np.nan)) * 100, 2) if pd.notna(latest.get("selected_tier3_stock", np.nan)) else None,
            "selected_full_stock": round(float(latest.get("selected_full_stock", np.nan)) * 100, 2) if pd.notna(latest.get("selected_full_stock", np.nan)) else None,
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.6.22 Single-Label 20D Rolling Tier Spike Diagnostics 결과 요약")
...
def make_condition_candidate_configs(base_cfg: Config, grid_size: str = "standard") -> List[Tuple[str, Config]]:
    ...
    if grid_size == "compact":
        ...
        # v8.6.22 standard: v8.6.10의 과도한 방어/turnover를 줄이는 공격형 후보군
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_single_label_20d_rolling_tier_spike_v8_6_22_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_single_label_20d_rolling_tier_spike_v8_6_22_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_single_label_20d_rolling_tier_spike_v8_6_22_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.22 Single-Label 20D Rolling Tier Spike Upside Strength Trigger + Objective Condition Search")
    ...
    parser.add_argument("--no-rolling-tier-opt", action="store_true", help="v8.6.22 기본 ON인 Tier 1/2/3/100% rolling 최적화를 비활성화")
    parser.add_argument("--tier-optimize-every", type=int, default=None, help="Tier rolling optimizer 재최적화 주기. 기본 20거래일")
    parser.add_argument("--tier-opt-window", type=int, default=None, help="Tier rolling optimizer 과거 평가 window. 기본 756거래일")
    parser.add_argument("--tier-opt-min-window", type=int, default=None, help="Tier rolling optimizer 최소 평가 window. 기본 504거래일")
    ...
    parser.add_argument("--rebalance-every", type=int, default=None, help="리밸런싱 주기 직접 지정. v8.6.22 기본 5거래일")
    parser.add_argument("--emergency-cooldown", type=int, default=None, help="긴급 리밸런싱 cooldown 직접 지정. v8.6.22 기본 5거래일")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.22 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.22 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.22 기본 ON인 stale offensive decay 비활성화")
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_single_label_20d_rolling_tier_spike_v8_6_22_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_single_label_20d_rolling_tier_spike_v8_6_22_summary.json"
    ...
    if getattr(args, "no_rolling_tier_opt", False):
        cfg.use_rolling_tier_optimization = False
    if getattr(args, "tier_optimize_every", None) is not None:
        cfg.tier_optimize_every_n_days = int(args.tier_optimize_every)
    if getattr(args, "tier_opt_window", None) is not None:
        cfg.tier_rolling_window = int(args.tier_opt_window)
    if getattr(args, "tier_opt_min_window", None) is not None:
        cfg.tier_min_window = int(args.tier_opt_min_window)
    ...
    cfg.result_dir = f"results_xgb_single_label_20d_rolling_tier_spike_v8_6_22_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        cfg.no_trade_band = max(cfg.no_trade_band, 0.11)
    elif cfg.policy_mode in {"direction_strength_specialist", "strength_specialist", "ds_specialist"}:
        # v8.6.22: Single-Label 20D Rolling Tier Spike Upside Strength Trigger
        # - 기본 주식 비중을 중간 수준으로 두고, 상승 강화 조건을 널널하게 적용해 더 자주, 더 크게 공격 전환한다.
        ...
        # v8.6.22: 20D 단일 라벨 spike. v8.6.19의 base 60%는 너무 낮아 CAGR이 눌렸으므로 상향한다.
        ...
        cfg.up_strength_bonus_threshold_1 = float(getattr(cfg, "up_strength_bonus_threshold_1", 0.25))
        cfg.up_strength_bonus_threshold_2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.35))
        ...
        cfg.up_strength_low_vol_threshold_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.76))
        cfg.up_strength_low_vol_threshold_3 = float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.68))
        cfg.up_strength_bear_block_threshold = float(getattr(cfg, "up_strength_bear_block_threshold", 0.75))
        cfg.direction_strength_max_stock_bonus = max(float(getattr(cfg, "direction_strength_max_stock_bonus", 0.35)), 0.35)
        cfg.enable_stale_offensive_decay = bool(getattr(cfg, "enable_stale_offensive_decay", True))
        # v8.6.22: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
        ...
        cfg.multi_strength_horizons = (20,)
        cfg.up_strength_weight_5d = 0.0
        cfg.up_strength_weight_10d = 0.0
        cfg.up_strength_weight_20d = 1.0
        cfg.direction_strength_feature_set = "horizon_5_10_20_pruned"
        cfg.use_rolling_tier_optimization = bool(getattr(cfg, "use_rolling_tier_optimization", True))
    ...
    # v8.6.22 direct low-base overrides after policy defaults.
    if getattr(args, "base_normal_stock", None) is not None:
        cfg.normal_stock_weight = float(args.base_normal_stock)
        rem = max(0.0, 1.0 - cfg.normal_stock_weight); cfg.normal_bond_weight = rem * 2/3; cfg.normal_cash_weight = rem * 1/3
    if getattr(args, "base_watch_stock", None) is not None:
    ...
    print(f"    rolling_tier_opt: {getattr(cfg, 'use_rolling_tier_optimization', False)}")
    ...
    print("[3/5] Walk-forward Stage1 + 기존 Up/Down + 5D/10D/20D Direction-Strength Specialist 예측")
    ...
    file_prefix = f"{safe_ticker}_xgb_single_label_20d_rolling_tier_spike_v8_6_22"
