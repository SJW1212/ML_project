"""
XGBoost v8.6.20 - Single-Label 20D Feature Spike Upside Strength Allocation
====================================================================
...
class Config:
    ...
    result_dir: str = "results_xgb_single_label_20d_feature_spike_v8_6_20"
    ...
    # v8.6.20 Single-Label 20D Feature Spike Upside Strength Trigger
    ...
    direction_strength_feature_set: str = "horizon_5_10_20"
    ...
    # v8.6.20 controlled consensus thresholds
    ...
    # v8.6.20: 100% 진입은 매우 강한 5D/10D/20D 합의에만 허용한다.
    ...
    # v8.6.20 diagnostics / stale offensive de-risking
...
def build_features(ohlcv: pd.DataFrame, horizons: Sequence[int]) -> Tuple[pd.DataFrame, List[str]]:
    ...
    # v8.6.20: 20D 단일 라벨을 예측하기 위한 5/10/20 과거 구간 피처.
    ...
    feature_cols = [
        ...
        "return_5d_minus_10d", "return_5d_minus_20d", "return_10d_minus_20d",
        ...
        "volume_change", "volume_ratio_5", "volume_ratio_10", "volume_ratio_20",
        "volume_zscore_5", "volume_zscore_10", "volume_zscore_20",
        "volume_ratio_5_minus_20", "volume_ratio_10_minus_20",
        "down_volume_ratio_5", "down_volume_ratio_10", "down_volume_ratio_20",
        "high_volume_down_ratio_5", "high_volume_down_ratio_10", "high_volume_down_ratio_20",
...
def ensemble_weights(cfg: Config) -> Tuple[Dict[int, float], Dict[int, float]]:
    # v8.6.20에서 cfg.horizons에는 UP_STRENGTHENING 5D 라벨 생성을 위해 5가 포함될 수 있다.
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    """
    v8.6.20: allocation gate에서 사용할 하락 위험 점수.
...
def build_strength_feature_sets(feature_cols: Sequence[str]) -> Dict[str, List[str]]:
    ...
    horizon_5_10_20 = keep([
        "return_5d", "return_10d", "return_20d",
        "return_5d_minus_10d", "return_5d_minus_20d", "return_10d_minus_20d",
        "price_ma_5_gap", "price_ma_10_gap", "price_ma_20_gap", "ma_gap_5_20",
        "trend_slope_5", "trend_slope_10", "trend_slope_20",
        "positive_return_ratio_5", "positive_return_ratio_10", "positive_return_ratio_20",
        "trend_consistency_20",
        "large_up_day_ratio_5", "large_up_day_ratio_10", "large_up_day_ratio_20",
        "large_down_day_ratio_5", "large_down_day_ratio_10", "large_down_day_ratio_20",
        "drawdown_5", "drawdown_10", "drawdown_20",
        "price_position_5", "price_position_10", "price_position_20",
        "price_position_5_minus_20", "price_position_10_minus_20",
        "close_to_5d_high", "close_to_10d_high", "close_to_20d_high",
        ...
        "atr_ratio_5_20", "atr_ratio_10_20", "atr_accel_5",
        "realized_vol_5", "realized_vol_10", "realized_vol_20",
        "realized_vol_ratio_5_20", "realized_vol_ratio_10_20",
        "ewma_vol_5", "ewma_vol_10", "ewma_vol_20",
        "downside_vol_5", "downside_vol_10", "downside_vol_20",
        "semi_vol_5", "semi_vol_10", "semi_vol_20",
        "bb_width_20", "bb_width_rank_252", "ulcer_index_20",
    ])
    return {
        ...
        "horizon_5_10_20": horizon_5_10_20,
    }
...
def build_direction_strength_labels(df: pd.DataFrame, cfg: Config, horizon: Optional[int] = None) -> Tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    v8.6.20에서는 5D/10D/20D specialist를 각각 학습하므로 horizon 인자를 선택적으로 받는다.
...
def apply_direction_strength_specialist_policy(
    ...
    meta = {
        ...
        "policy_note": "single_label_20d_feature_spike_p20_stage1_risk_gate",
    }
    return w, meta
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        if prev_w is None:
            ...
            # v8.6.20 optional: offensive target이 사라졌는데 이전 98~100% 비중이 과도하게 남는 문제를 실험적으로 제어한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_single_label_20d_feature_spike_v8_6_20_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.6.20 Single-Label 20D Feature Spike Diagnostics 결과 요약")
...
def make_condition_candidate_configs(base_cfg: Config, grid_size: str = "standard") -> List[Tuple[str, Config]]:
    ...
    if grid_size == "compact":
        ...
        # v8.6.20 standard: v8.6.10의 과도한 방어/turnover를 줄이는 공격형 후보군
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_single_label_20d_feature_spike_v8_6_20_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_single_label_20d_feature_spike_v8_6_20_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_single_label_20d_feature_spike_v8_6_20_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.20 Single-Label 20D Feature Spike Upside Strength Trigger + Objective Condition Search")
    ...
    parser.add_argument("--rebalance-every", type=int, default=None, help="리밸런싱 주기 직접 지정. v8.6.20 기본 5거래일")
    parser.add_argument("--emergency-cooldown", type=int, default=None, help="긴급 리밸런싱 cooldown 직접 지정. v8.6.20 기본 5거래일")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.20 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.20 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.20 기본 ON인 stale offensive decay 비활성화")
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_single_label_20d_feature_spike_v8_6_20_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_single_label_20d_feature_spike_v8_6_20_summary.json"
    ...
    cfg.result_dir = f"results_xgb_single_label_20d_feature_spike_v8_6_20_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.20: 20D 단일 라벨 spike. v8.6.19의 base 60%는 너무 낮아 CAGR이 눌렸으므로 상향한다.
        ...
        # v8.6.20: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
        ...
        cfg.direction_strength_feature_set = "horizon_5_10_20"
    ...
    # v8.6.20 direct low-base overrides after policy defaults.
    if getattr(args, "base_normal_stock", None) is not None:
        cfg.normal_stock_weight = float(args.base_normal_stock)
        rem = max(0.0, 1.0 - cfg.normal_stock_weight); cfg.normal_bond_weight = rem * 2/3; cfg.normal_cash_weight = rem * 1/3
    if getattr(args, "base_watch_stock", None) is not None:
    ...
    file_prefix = f"{safe_ticker}_xgb_single_label_20d_feature_spike_v8_6_20"
