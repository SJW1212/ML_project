"""
XGBoost ATR-aware H10 State Switch Strategy - Stage1 + Down-risk OVR
======================================================
...
- prob_high_vol / prob_down_risk를 중심으로 배분 함수 계산
- 평가 지표는 Stage1 / Down-risk OVR / Final Cascade로 분리

실행
    py xgb_atr_h10_v5_state_switch_strategy.py
...
class Config:
    ...
    # 상태전환형 전략에서는 emergency는 "Risk-off 진입"에만 사용한다.
    emergency_down_threshold: float = 0.45
    emergency_combined_down_threshold: float = 0.35
    emergency_combined_high_vol_threshold: float = 0.40
    emergency_high_vol_threshold: float = 0.75

    # --------------------------------------------------------
    # State-switch allocation
    # --------------------------------------------------------
    # NORMAL: 평상시 고주식 비중. 실제 프로젝트에서는 이 비중을 별도 최적화 대상으로 둔다.
    normal_stock_weight: float = 0.88
    normal_bond_weight: float = 0.08
    normal_cash_weight: float = 0.04

    # RISK_OFF: 하락고변동 위험 감지 시 방어 포트폴리오.
    risk_off_stock_weight: float = 0.40
    risk_off_bond_weight: float = 0.35
    risk_off_cash_weight: float = 0.25

    # RECOVERY: 하락위험 해소 직후 정상 복귀 전 완충 포트폴리오.
    recovery_stock_weight: float = 0.65
    recovery_bond_weight: float = 0.22
    recovery_cash_weight: float = 0.13

    # 상태 진입/해제. 진입 threshold는 높게, 해제 threshold는 낮게 두는 hysteresis 구조.
    state_enter_down_threshold: float = 0.45
    state_enter_combined_down_threshold: float = 0.35
    state_enter_combined_high_vol_threshold: float = 0.40
    state_enter_high_vol_threshold: float = 0.75
    state_exit_down_threshold: float = 0.25
    state_exit_high_vol_threshold: float = 0.45
    state_exit_confirm_periods: int = 2

    # 정기 리밸런싱 주기. 단, Risk-off 진입은 emergency로 즉시 허용.
    state_rebalance_every_n_days: int = 10
    ...
    result_dir: str = "results_xgb_atr_h10_v5_state_switch_strategy"
...
def _normalize_weights(stock: float, bond: float, cash: float) -> Tuple[float, float, float]:
    ...
    b = max(0.0, float(bond))
    c = max(0.0, float(cash))
    total = s + b + c
    if total <= 0:
        return 1.0, 0.0, 0.0
    return s / total, b / total, c / total


def build_allocation_grid(cfg: Config) -> List[Dict[str, float]]:
    """
    상태전환형 allocation grid.

    핵심:
    - NORMAL 포트폴리오는 별도 최적화 대상으로 둔다.
    - RISK_OFF는 하락고변동 위험 진입 시 명확히 주식 비중을 낮춘다.
    - RECOVERY는 위험 해소 후 정상 복귀 전 완충 상태로 사용한다.
    - 연속 확률에 따라 매번 비중을 흔드는 방식이 아니라, 상태가 바뀔 때만 거래한다.
    """
    normal_candidates = [
        (0.86, 0.10, 0.04),
        (0.88, 0.08, 0.04),
    ...
    threshold_profiles = [
        # enter_down, combined_down, combined_high, enter_high, exit_down, exit_high
        (0.45, 0.35, 0.40, 0.75, 0.25, 0.45),
        (0.50, 0.40, 0.45, 0.80, 0.25, 0.45),
    ]

    grid: List[Dict[str, float]] = []
    idx = 0
    for nw in normal_candidates:
        for rw in risk_off_candidates:
            for recw in recovery_candidates:
                for th in threshold_profiles:
                    grid.append({
                        "name": f"h10_state_switch_cfg_{idx:04d}",
                        "normal_stock_weight": nw[0],
                        "normal_bond_weight": nw[1],
                        "normal_cash_weight": nw[2],
                        "risk_off_stock_weight": rw[0],
                        "risk_off_bond_weight": rw[1],
                        "risk_off_cash_weight": rw[2],
                        "recovery_stock_weight": recw[0],
                        "recovery_bond_weight": recw[1],
                        "recovery_cash_weight": recw[2],
                        "state_enter_down_threshold": th[0],
                        "state_enter_combined_down_threshold": th[1],
                        "state_enter_combined_high_vol_threshold": th[2],
                        "state_enter_high_vol_threshold": th[3],
                        "state_exit_down_threshold": th[4],
                        "state_exit_high_vol_threshold": th[5],
                        "state_exit_confirm_periods": cfg.state_exit_confirm_periods,
                        "state_rebalance_every_n_days": cfg.state_rebalance_every_n_days,
                    })
                    idx += 1
    return grid
...
def default_alloc_config(cfg: Config) -> Dict[str, float]:
    return {
        ...
        "recovery_stock_weight": cfg.recovery_stock_weight,
        "recovery_bond_weight": cfg.recovery_bond_weight,
        "recovery_cash_weight": cfg.recovery_cash_weight,
        "state_enter_down_threshold": cfg.state_enter_down_threshold,
        "state_enter_combined_down_threshold": cfg.state_enter_combined_down_threshold,
        "state_enter_combined_high_vol_threshold": cfg.state_enter_combined_high_vol_threshold,
        "state_enter_high_vol_threshold": cfg.state_enter_high_vol_threshold,
        "state_exit_down_threshold": cfg.state_exit_down_threshold,
        "state_exit_high_vol_threshold": cfg.state_exit_high_vol_threshold,
        "state_exit_confirm_periods": cfg.state_exit_confirm_periods,
        "state_rebalance_every_n_days": cfg.state_rebalance_every_n_days,
    }


def regime_weights(state: str, cfg_dict: Dict[str, float]) -> Tuple[float, float, float]:
    if state == "RISK_OFF":
    ...
    if state == "RECOVERY":
        return _normalize_weights(
            cfg_dict["recovery_stock_weight"],
            cfg_dict["recovery_bond_weight"],
            cfg_dict["recovery_cash_weight"],
        )
    return _normalize_weights(
...
def risk_off_enter_signal(prob_high_vol: float, prob_down_risk: float, cfg_dict: Dict[str, float]) -> bool:
    ph = float(prob_high_vol)
    pdn = float(prob_down_risk)
    return bool(
        pdn >= cfg_dict["state_enter_down_threshold"]
        or (
            pdn >= cfg_dict["state_enter_combined_down_threshold"]
            and ph >= cfg_dict["state_enter_combined_high_vol_threshold"]
        )
        or ph >= cfg_dict["state_enter_high_vol_threshold"]
    )


def risk_off_exit_signal(prob_high_vol: float, prob_down_risk: float, cfg_dict: Dict[str, float]) -> bool:
    ph = float(prob_high_vol)
    pdn = float(prob_down_risk)
    return bool(
        pdn <= cfg_dict["state_exit_down_threshold"]
        and ph <= cfg_dict["state_exit_high_vol_threshold"]
    )
...
def next_regime_state(
    ...
    """
    NORMAL / RISK_OFF / RECOVERY 상태전환 로직.

    반환값:
    - next_state
    - next_exit_confirm_count
    - state_changed
    """
    state = prev_state or "NORMAL"
    enter = risk_off_enter_signal(prob_high_vol, prob_down_risk, cfg_dict)
    exit_sig = risk_off_exit_signal(prob_high_vol, prob_down_risk, cfg_dict)
    confirm_need = int(cfg_dict.get("state_exit_confirm_periods", 2))

    if state == "NORMAL":
        if enter:
            return "RISK_OFF", 0, True
        return "NORMAL", 0, False

    if state == "RISK_OFF":
        if enter:
            return "RISK_OFF", 0, False
        if exit_sig:
            exit_confirm_count += 1
            if exit_confirm_count >= confirm_need:
                return "RECOVERY", 0, True
            return "RISK_OFF", exit_confirm_count, False
        return "RISK_OFF", 0, False
    ...
    if state == "RECOVERY":
        if enter:
            return "RISK_OFF", 0, True
        if exit_sig:
            return "NORMAL", 0, True
        return "RECOVERY", 0, False

    return "NORMAL", 0, state != "NORMAL"
...
def simulate_with_config(pred_df: pd.DataFrame, cfg_dict: Dict[str, float], cfg: Config) -> Dict[str, float]:
    ...
    for i, (_, row) in enumerate(pred_df.iterrows()):
        ...
        pdn = float(row.get("prob_down_risk", row.get("prob_down_high_vol", 0.0)))

        scheduled = (i % int(cfg_dict.get("state_rebalance_every_n_days", cfg.state_rebalance_every_n_days)) == 0)
        emergency_enter = risk_off_enter_signal(ph, pdn, cfg_dict) and state != "RISK_OFF"
        should_eval_state = prev_w is None or scheduled or emergency_enter

        if should_eval_state:
            state, exit_confirm_count, changed = next_regime_state(state, ph, pdn, cfg_dict, exit_confirm_count)
            if changed:
                state_changes += 1
            w = regime_weights(state, cfg_dict)
        else:
            w = prev_w if prev_w is not None else regime_weights(state, cfg_dict)
    ...
    stats["state_change_ratio"] = float(state_changes / len(pred_df)) if len(pred_df) else 0.0
    return stats
...
def score_config(stats: Dict[str, float], cfg: Config, avg_stock_weight: Optional[float] = None) -> float:
    ...
    stock_penalty = 0.0
    if avg_stock < target - 0.08:
        stock_penalty += cfg.opt_low_stock_penalty_weight * (target - 0.08 - avg_stock) ** 2
    if avg_stock > target + 0.08:
        stock_penalty += 1.00 * (avg_stock - target - 0.08) ** 2
...
def apply_rolling_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        scheduled_rebalance = (i % int(current_cfg.get("state_rebalance_every_n_days", cfg.state_rebalance_every_n_days)) == 0)
        emergency_enter = risk_off_enter_signal(ph, pdn, current_cfg) and state != "RISK_OFF"
        should_rebalance = (prev_w is None) or scheduled_rebalance or emergency_enter

        prev_state = state
        state_changed = False
        if should_rebalance:
            state, exit_confirm_count, state_changed = next_regime_state(state, ph, pdn, current_cfg, exit_confirm_count)
            w = regime_weights(state, current_cfg)
        else:
            w = prev_w if prev_w is not None else regime_weights(state, current_cfg)
        ...
        out.update({
            "regime_state": state,
            "prev_regime_state": prev_state,
            "state_changed": bool(state_changed),
            "exit_confirm_count": int(exit_confirm_count),
            "stock_weight": w[0],
            "bond_weight": w[1],
            "cash_weight": w[2],
            "turnover": turnover,
            ...
            "emergency_rebalance": bool(emergency_enter and should_rebalance),
        })
        rows.append(out)
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], cfg_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    summary = {
        ...
        "model_type": "xgb_atr_h10_state_switch_stage1_downrisk_ovr",
        "config": asdict(cfg),
        "period": {
        ...
        "label_design": {
            ...
            "allocation_mode": "NORMAL / RISK_OFF / RECOVERY state-switch",
        ...
        "turnover": {
            ...
            "state_change_ratio": float(pred_df.get("state_changed", pd.Series([False] * len(pred_df))).mean()),
        },
        "regime_state_distribution": {
            k: float(v) for k, v in pred_df.get("regime_state", pd.Series(["UNKNOWN"] * len(pred_df))).value_counts(normalize=True).to_dict().items()
        },
        "performance": perf,
        **cls,
        ...
        "latest_prediction": {
            ...
            "regime_state": str(latest.get("regime_state", "UNKNOWN")),
            "target_allocation": {
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost ATR-aware H10 State-switch 결과 요약")
    print("Stage1 + Down-risk OVR + NORMAL/RISK_OFF/RECOVERY Allocation")
    print("==============================")
    ...
    print(f"상태 전환 비율: {t.get('state_change_ratio', 0.0) * 100:.2f}%")
    if "regime_state_distribution" in summary:
        dist = {k: round(v * 100, 2) for k, v in summary["regime_state_distribution"].items()}
        print(f"상태 분포: {dist}")
...
def main() -> None:
    ...
    pred_path = result_dir / "qqq_xgb_atr_h10_state_switch_predictions.csv"
    summary_path = result_dir / "qqq_xgb_atr_h10_state_switch_summary.json"
    latest_path = result_dir / "qqq_xgb_atr_h10_state_switch_latest.json"
