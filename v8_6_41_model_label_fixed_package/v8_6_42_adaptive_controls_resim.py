...
class AdaptiveControlsConfig:
    ...
    broad_index_max_stock: float = 1.00
    ...
    version: str = "v8.6.42_adaptive_controls"
...
def build_cross_asset_context_scores(combined: pd.DataFrame, cfg: AdaptiveControlsConfig) -> pd.DataFrame:
    """Fast expanding context score across tickers without lookahead.

    Uses only rows that occur before the current row in Date/ticker order. The policy table is not manual:
    it is inferred from historical realized returns by context_key = asset_class × mid_trend_state × ph_bin.
    If a context has insufficient history, it falls back to asset_class history, then global history.
    """
    work = combined.sort_values(["Date", "ticker"]).reset_index(drop=True).copy()
    work["ph_bin"] = work["ph_rank_adaptive"].apply(ph_bin)
    work["context_key"] = work["asset_class"].astype(str) + "|" + work["mid_trend_state"].astype(str) + "|" + work["ph_bin"].astype(str)
    ret = pd.to_numeric(work["stock_next_return"], errors="coerce").fillna(0.0).astype(float)
    down = (ret < 0).astype(float)

    def _past_stats(key_col: str) -> Tuple[pd.Series, pd.Series, pd.Series]:
        gb = work.groupby(key_col, sort=False)
        cnt = gb.cumcount().astype(float)
        sum_ret = ret.groupby(work[key_col], sort=False).cumsum() - ret
        sum_down = down.groupby(work[key_col], sort=False).cumsum() - down
        mean_ret = sum_ret / cnt.replace(0.0, np.nan)
        down_freq = sum_down / cnt.replace(0.0, np.nan)
        return cnt, mean_ret, down_freq

    ctx_cnt, ctx_mean, ctx_down = _past_stats("context_key")
    ac_cnt, ac_mean, ac_down = _past_stats("asset_class")
    global_cnt = pd.Series(np.arange(len(work), dtype=float), index=work.index)
    global_sum = ret.cumsum() - ret
    global_down_sum = down.cumsum() - down
    global_mean = global_sum / global_cnt.replace(0.0, np.nan)
    global_down = global_down_sum / global_cnt.replace(0.0, np.nan)

    use_ctx = ctx_cnt >= cfg.context_min_rows
    use_ac = (~use_ctx) & (ac_cnt >= cfg.context_global_min_rows)

    mean = pd.Series(0.0, index=work.index, dtype=float)
    down_freq = pd.Series(0.5, index=work.index, dtype=float)
    rows = pd.Series(0, index=work.index, dtype=int)
    scope = pd.Series("global", index=work.index, dtype=object)

    mean.loc[use_ctx] = ctx_mean.loc[use_ctx]
    down_freq.loc[use_ctx] = ctx_down.loc[use_ctx]
    rows.loc[use_ctx] = ctx_cnt.loc[use_ctx].astype(int)
    scope.loc[use_ctx] = "asset_class_trend_phbin"

    mean.loc[use_ac] = ac_mean.loc[use_ac]
    down_freq.loc[use_ac] = ac_down.loc[use_ac]
    rows.loc[use_ac] = ac_cnt.loc[use_ac].astype(int)
    scope.loc[use_ac] = "asset_class"

    use_global = ~(use_ctx | use_ac)
    valid_global = use_global & (global_cnt > 0)
    mean.loc[valid_global] = global_mean.loc[valid_global]
    down_freq.loc[valid_global] = global_down.loc[valid_global]
    rows.loc[valid_global] = global_cnt.loc[valid_global].astype(int)

    mean = mean.fillna(0.0)
    down_freq = down_freq.fillna(0.5)
    # Tail penalty is intentionally omitted in the fast expanding version; down frequency acts as drawdown proxy.
    score = cfg.context_return_scale * mean - cfg.context_down_penalty * (down_freq - 0.5)
    adjust = score.clip(-1.0, 1.0) * float(cfg.context_adjust_max)

    work["context_rows"] = rows
    work["context_scope"] = scope
    work["context_mean_return"] = mean
    work["context_down_freq"] = down_freq
    work["context_tail10"] = np.nan
    work["context_score"] = score
    work["context_adjust"] = adjust
    return work
...
def add_signal_weights(df: pd.DataFrame, cfg: AdaptiveControlsConfig) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        # upside overlay: adaptive and guarded
        if cfg.enable_upside_overlay and up >= cfg.up_tier1 and risk < 0.78:
            target = 0.82
            tier = 1
