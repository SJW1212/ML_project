"""
XGBoost v8.6.26 - Volatility-Base Return-Improve Strong-Override No-Sideway Strength Allocation
====================================================================
...
실행 예시
    py xgb_vol_base_return_improve_v8_6_26.py --speed-profile balanced --h10-down-only --condition-search
    py xgb_vol_base_return_improve_v8_6_26.py --speed-profile fast --h10-down-only
    py xgb_vol_base_return_improve_v8_6_26.py --speed-profile full --adaptive-label --h10-down-only
...
class Config:
    ...
    result_dir: str = "results_xgb_vol_base_return_improve_v8_6_26"
    ...
    # v8.6.26 base allocation: 기본 상태는 high-vol 확률 기반 연속/계단형 비중으로 산출한다.
    # v8.6.26 return-improve: 극저변동 구간(<15%)만 83%로 상향하고 15~25%는 78% 유지.
    use_vol_probability_base_allocation: bool = True
    vol_base_stock_lt_15: float = 0.83
    ...
    # v8.6.26 execution: Tier3/Full은 schedule/no-trade-band에 묶이지 않고 즉시 목표 비중으로 점프한다.
    ...
    # v8.6.26 Volatility-Base Strong-Override Multi-Horizon Upside Strength Trigger
    ...
    up_strength_bonus_threshold_1: float = 0.34
    up_strength_bonus_threshold_2: float = 0.99  # v8.6.26: Tier2 삭제
    ...
    up_strength_confirm_20d_threshold_1: float = 0.34
    up_strength_confirm_20d_threshold_3: float = 0.38
    up_strength_pred_threshold_5d: float = 0.99  # v8.6.26: 5D 단독 trigger 비활성화
    ...
    up_strength_pair_10d_20d_stock_weight: float = 0.88  # Tier2 삭제 상태에서는 기본 미사용
    ...
    # v8.6.26: 5D는 allocation trigger에서 제외하고, 10D는 보조 확인, 20D는 핵심 조건으로 사용한다.
    ...
    up_strength_low_vol_threshold_3: float = 0.66
...
def base_weight_from_vol_probability(prob_high_vol: float, cfg: Config) -> Tuple[float, float, float]:
    ...
    ph = float(np.clip(prob_high_vol, 0.0, 1.0))
    if ph < 0.15:
        stock = float(getattr(cfg, "vol_base_stock_lt_15", 0.83))
    elif ph < 0.25:
...
def apply_direction_strength_specialist_policy(
    ...
    """
    v8.6.26 Volatility-Base Strong-Override Upside Strength Trigger.
    ...
    low_vol_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.60))
    low_vol_3 = float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.66))
    ...
    tier1_signal = False
    tier2_signal = False  # v8.6.26: 삭제 유지. diagnostic compatibility용.
    tier3_signal = False
    ...
    # v8.6.26: 강한 상승 override는 regime bucket보다 실제 high-vol cap을 우선한다.
    # ph cap과 bear block을 통과하지 못하면 공격 전환하지 않는다.
    if pbd < block_bear:
        tier1_signal = bool(
            pus_score >= float(getattr(cfg, "up_strength_bonus_threshold_1", 0.34))
            and p20 >= float(getattr(cfg, "up_strength_confirm_20d_threshold_1", 0.34))
            and ph < low_vol_1
        )
        if not bool(getattr(cfg, "disable_tier2_signal", True)):
            tier2_signal = bool(
                pus_score >= float(getattr(cfg, "up_strength_bonus_threshold_2", 0.99))
                and pred10
                and pred20
                and ph < float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.76))
            )
        tier3_signal = bool(
            pus_score >= float(getattr(cfg, "up_strength_bonus_threshold_3", 0.44))
        ...
        # Tier 2는 기본값에서 삭제. 켜더라도 88% 제한.
        if tier2_signal:
            target_stock = max(target_stock, float(getattr(cfg, "up_strength_pair_10d_20d_stock_weight", 0.88)))
    ...
    meta = {
        ...
        "policy_note": "vol_probability_base_tier3_full_strong_override",
    }
    return w, meta
    ...
    if "actual_direction_strength" in pred_df.columns:
        ...
        if "prob_up_strengthening" in pred_df.columns:
            ...
            metrics["direction_strength_up_strengthening"] = binary_cls_metrics(
                y_us, p_us, float(getattr(cfg, "up_strength_bonus_threshold_1", 0.34)), "UP_STRENGTHENING"
            )
        ...
        for sh in get_multi_strength_horizons(cfg):
            ...
            if actual_col in pred_df.columns and prob_col in pred_df.columns:
                ...
                metrics[f"direction_strength_up_strengthening_{int(sh)}d"] = binary_cls_metrics(
                    y_h, p_h, float(getattr(cfg, "up_strength_bonus_threshold_1", 0.34)), f"UP_STRENGTHENING_{int(sh)}D"
                )
        if "prob_up_strengthening_score" in pred_df.columns and "actual_direction_strength_20d" in pred_df.columns:
            ...
            metrics["direction_strength_up_score_vs_20d"] = binary_cls_metrics(
                y_score, p_score, float(getattr(cfg, "up_strength_bonus_threshold_1", 0.34)), "UP_STRENGTHENING_SCORE"
            )
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_vol_base_return_improve_v8_6_26_diagnostics",
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_vol_base_return_improve_v8_6_26_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_vol_base_return_improve_v8_6_26_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_vol_base_return_improve_v8_6_26_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.26 Volatility-Base Strong-Override Multi-Horizon Upside Strength Trigger + Objective Condition Search")
    ...
    parser.add_argument("--rebalance-every", type=int, default=None, help="리밸런싱 주기 직접 지정. v8.6.26 기본 5거래일")
    parser.add_argument("--emergency-cooldown", type=int, default=None, help="긴급 리밸런싱 cooldown 직접 지정. v8.6.26 기본 5거래일")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.26 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.26 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.26 기본 ON인 stale offensive decay 비활성화")
    ...
    parser.add_argument("--no-horizon-train-window", action="store_true", help="v8.6.26 horizon별 train_rows = horizon*multiplier 모드 비활성화")
    ...
    parser.add_argument("--up-confirm-20d-threshold-1", type=float, default=None, help="Tier1 진입용 20D UP_STRENGTHENING 확인 threshold")
    ...
    parser.add_argument("--disable-vol-base-allocation", action="store_true", help="v8.6.26 high-vol 확률 기반 기본 비중을 끄고 regime bucket 기본 비중 사용")
    parser.add_argument("--vol-base-stock-lt-15", type=float, default=None, help="prob_high_vol < 15% 구간 기본 주식 비중. v8.6.26 기본 0.83")
    parser.add_argument("--vol-base-stock-lt-25", type=float, default=None, help="15% <= prob_high_vol < 25% 구간 기본 주식 비중. v8.6.26 기본 0.78")
    ...
    parser.add_argument("--enable-tier2", action="store_true", help="기본 삭제된 Tier2를 실험적으로 다시 활성화")
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_vol_base_return_improve_v8_6_26_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_vol_base_return_improve_v8_6_26_summary.json"
    ...
    cfg.result_dir = f"results_xgb_vol_base_return_improve_v8_6_26_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.26: 기본 상태는 prob_high_vol 기반 포트폴리오, 강한 신호만 즉시 override한다.
        cfg.use_vol_probability_base_allocation = True
        cfg.vol_base_stock_lt_15 = 0.83
        ...
        cfg.up_strength_bonus_threshold_1 = float(getattr(cfg, "up_strength_bonus_threshold_1", 0.34))
        cfg.up_strength_bonus_threshold_2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.99))
        ...
        cfg.up_strength_low_vol_threshold_3 = float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.66))
        ...
        # v8.6.26: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
    ...
    if getattr(args, "up_confirm_20d_threshold_1", None) is not None:
        cfg.up_strength_confirm_20d_threshold_1 = float(args.up_confirm_20d_threshold_1)
    ...
    if getattr(args, "vol_base_stock_lt_15", None) is not None:
        cfg.vol_base_stock_lt_15 = float(args.vol_base_stock_lt_15)
    if getattr(args, "vol_base_stock_lt_25", None) is not None:
        cfg.vol_base_stock_lt_25 = float(args.vol_base_stock_lt_25)
    ...
    file_prefix = f"{safe_ticker}_xgb_vol_base_return_improve_v8_6_26"
