"""
XGBoost ATR-aware Risk-first Model v4 - Stage1 + Down-risk OVR
======================================================
...
실행
    py xgb_atr_stage1_downrisk_ovr_v4.py
...
class Config:
    ...
    down_soft_threshold: float = 0.25
    down_hard_threshold: float = 0.35
    down_extreme_threshold: float = 0.50
    ...
    pred_down_high_vol_threshold: float = 0.25
    ...
    opt_turnover_weight: float = 0.35
    opt_target_avg_stock_weight: float = 0.75
    opt_low_stock_penalty_weight: float = 0.40

    result_dir: str = "results_xgb_atr_stage1_downrisk_ovr_v4"
            ...
            for high_w in high_weight_list:
                for down_w in down_weight_list:
                    for ntb in no_trade_list:
                        grid.append({
                            "name": f"xgb_v4_downrisk_cfg_{idx:04d}",
...
def default_alloc_config(cfg: Config) -> Dict[str, float]:
    return {
        "name": "default_xgb_atr_stage1_downrisk_ovr_v4",
...
def apply_rolling_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        emergency_rebalance = (
            float(row.get("prob_down_risk", 0.0)) >= cfg.down_hard_threshold
            or float(row.get("prob_high_vol", 0.0)) >= 0.60
        )
...
def classification_metrics(pred_df: pd.DataFrame) -> Dict[str, object]:
    ...
    return {
        ...
        # v3와 비교용. Stage2 방향 모델은 v4에서 제거됨.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], cfg_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    summary = {
        ...
        "model_type": "xgb_atr_stage1_downrisk_ovr_risk_first_v4",
        "config": asdict(cfg),
        "period": {
        ...
        "label_design": {
            ...
            "removed": "고변동 내부 상승고변동 / 하락고변동 Stage2 방향 분류는 v4에서 제거",
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost ATR-aware v4 결과 요약")
...
def main() -> None:
    ...
    pred_path = result_dir / "qqq_xgb_atr_stage1_downrisk_ovr_v4_predictions.csv"
    summary_path = result_dir / "qqq_xgb_atr_stage1_downrisk_ovr_v4_summary.json"
    latest_path = result_dir / "qqq_xgb_atr_stage1_downrisk_ovr_v4_latest.json"
