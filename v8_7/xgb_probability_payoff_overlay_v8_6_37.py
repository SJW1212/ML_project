"""
XGBoost v8.6.37 - Normal Base Boost + Probability Payoff Overlay
====================================================================

목적
- QQQ/IEF/BIL 동적 자산배분 전략
- v8.6.37: 5D+10D+high-vol ShortMidConfirm 보조 신호 추가
...
실행 예시
    py xgb_probability_payoff_overlay_v8_6_37.py --speed-profile balanced --h10-down-only
    py xgb_probability_payoff_overlay_v8_6_37.py --speed-profile fast --h10-down-only
    py xgb_probability_payoff_overlay_v8_6_37.py --speed-profile full --adaptive-label --h10-down-only
...
class Config:
    ...
    result_dir: str = "results_xgb_probability_payoff_overlay_v8_6_37"
    ...
    # v8.6.37 base allocation: 기본 상태는 high-vol 확률 기반 연속/계단형 비중으로 산출한다.
    ...
    # v8.6.37 Normal-Unrestricted Base
    ...
    # v8.6.37 Probability Payoff Overlay
    payoff_overlay_enabled: bool = True
    payoff_overlay_horizon: int = 20
    payoff_overlay_train_rows: int = 756
    payoff_overlay_min_samples: int = 60
    payoff_overlay_bin_width: float = 0.20
    payoff_overlay_cvar_quantile: float = 0.20
    payoff_overlay_reward_mean_weight: float = 0.50
    payoff_overlay_reward_q75_weight: float = 0.50
    payoff_overlay_rr_cut_1: float = 0.70
    payoff_overlay_rr_cut_2: float = 0.90
    payoff_overlay_rr_cut_3: float = 1.10
    payoff_overlay_mult_1: float = 0.78
    payoff_overlay_mult_2: float = 0.86
    payoff_overlay_mult_3: float = 0.93
    payoff_overlay_min_stock_weight: float = 0.30
    payoff_overlay_apply_to_offensive: bool = True
    payoff_overlay_force_rr_cut_rebalance: bool = False
    payoff_overlay_drawdown_guard_enabled: bool = True
    payoff_overlay_dd1: float = -0.04
    payoff_overlay_dd2: float = -0.07
    payoff_overlay_dd3: float = -0.10
    payoff_overlay_dd1_high_vol: float = 0.45
    payoff_overlay_dd2_high_vol: float = 0.50
    payoff_overlay_dd3_high_vol: float = 0.60
    payoff_overlay_dd1_mult: float = 0.92
    payoff_overlay_dd2_mult: float = 0.85
    payoff_overlay_dd3_mult: float = 0.70
    payoff_overlay_force_dd_rebalance: bool = True

    # v8.6.37 execution: Tier3/Full은 schedule/no-trade-band에 묶이지 않고 즉시 목표 비중으로 점프한다.
    ...
    # v8.6.37 Separate PortfolioPolicyModel
    ...
    # v8.6.37 Volatility-Base Strong-Override Multi-Horizon Upside Strength Trigger
    ...
    # v8.6.37: prob_bear_down_strengthening 제거. 하락 강화도 상승 강화와 동일하게 5D/10D/20D term structure로 계산한다.
    ...
    up_strength_bonus_threshold_2: float = 0.38  # v8.6.37: Tier2 활성화 기본값
    ...
    up_strength_pred_threshold_5d: float = 0.99  # v8.6.37: 5D 단독 trigger 비활성화
    ...
    # v8.6.37: 5D는 allocation trigger에서 제외하고, 10D는 보조 확인, 20D는 핵심 조건으로 사용한다.
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            if bool(getattr(cfg, "use_direction_strength_specialist", True)):
                try:
                    ...
                    # v8.6.37: 별도 bear-filter specialist 제거.
        ...
        if bool(getattr(cfg, "use_direction_strength_specialist", True)):
            ...
            bear_strength_probs = {name: 0.0 for name in DIRECTION_STRENGTH_LABELS}  # removed in v8.6.37
        else:
        ...
        out.update({
            ...
            **{f"prob_up_weakening_{int(sh)}d": 0.0 for sh in multi_strength_horizons},
            # v8.6.37: 하락 강화도 상승 강화와 동일한 horizon별 term structure로 산출한다.
...
def apply_direction_strength_specialist_policy(
    ...
    """
    v8.6.37 Volatility-Base Strong-Override Upside Strength Trigger.
    ...
    """
    ph = _row_float(row, "prob_high_vol")
    # v8.6.37: weak 확률(prob_up/prob_down/prob_bear_down_strengthening)은 allocation에서 제거한다.
    ...
    # v8.6.37: 강한 상승 override는 regime bucket보다 실제 high-vol cap을 우선한다.
    ...
    if True:
        ...
        # v8.6.37 Normal-Unrestricted policy.
        ...
        # v8.6.37 ShortMidConfirm optional allocation actions.
    ...
    # v8.6.37: bear/down 약한 확률 기반 cut은 제거한다.
...
def _prob_bin_value(value: float, width: float) -> int:
    width = float(width) if float(width) > 0 else 0.20
    if pd.isna(value):
        value = 0.0
...
def _payoff_col_name(cfg: Config) -> str:
    h = int(getattr(cfg, "payoff_overlay_horizon", 20))
    return f"future_return_{h}d"


def _payoff_hist_slice(pred_df: pd.DataFrame, i: int, cfg: Config) -> pd.DataFrame:
    h = int(getattr(cfg, "payoff_overlay_horizon", 20))
    end = max(0, int(i) - h)
    if end <= 0:
        return pred_df.iloc[0:0].copy()
    train_rows = int(getattr(cfg, "payoff_overlay_train_rows", 756))
    start = max(0, end - train_rows) if train_rows > 0 else 0
    return pred_df.iloc[start:end].copy()


def _payoff_stats_from_returns(ret: pd.Series, cfg: Config) -> Dict[str, float]:
    r = ret.dropna().astype(float)
    if len(r) == 0:
        return {"sample_count": 0, "mean_return": np.nan, "q75_return": np.nan, "q20_return": np.nan, "cvar_loss": np.nan, "win_rate": np.nan, "reward": np.nan, "risk": np.nan, "rr": np.nan}
    q = float(np.clip(getattr(cfg, "payoff_overlay_cvar_quantile", 0.20), 0.01, 0.50))
    mean_ret = float(r.mean())
    q75 = float(r.quantile(0.75))
    q20 = float(r.quantile(q))
    tail = r[r <= q20]
    cvar = float(tail.mean()) if len(tail) else q20
    reward = float(getattr(cfg, "payoff_overlay_reward_mean_weight", 0.50)) * mean_ret + float(getattr(cfg, "payoff_overlay_reward_q75_weight", 0.50)) * q75
    reward = float(max(0.0, reward))
    risk = abs(cvar) if cvar < 0 else max(abs(q20), float(r.std()), 1e-6)
    rr = float(reward / (risk + 1e-9))
    return {"sample_count": int(len(r)), "mean_return": mean_ret, "q75_return": q75, "q20_return": q20, "cvar_loss": cvar, "win_rate": float((r > 0).mean()), "reward": reward, "risk": float(risk), "rr": rr}


def lookup_probability_payoff_stats(hist: pd.DataFrame, row: pd.Series, cfg: Config) -> Dict[str, object]:
    ret_col = _payoff_col_name(cfg)
    min_samples = int(getattr(cfg, "payoff_overlay_min_samples", 60))
    if hist.empty or ret_col not in hist.columns:
        stats = _payoff_stats_from_returns(pd.Series(dtype=float), cfg)
        stats.update({"bucket_level": "none", "bucket_key": ""})
        return stats
    width = float(getattr(cfg, "payoff_overlay_bin_width", 0.20))
    ph = _row_float(row, "prob_high_vol", 0.0)
    up = _row_float(row, "prob_up_strengthening_score", _row_float(row, "prob_up_strengthening", 0.0))
    dn = _row_float(row, "prob_down_strengthening_score", 0.0)
    hv_bin = _prob_bin_value(ph, width)
    up_bin = _prob_bin_value(up, width)
    dn_bin = _prob_bin_value(dn, width)
    h = hist.copy()
    h = h[pd.notna(h[ret_col])].copy()
    if h.empty:
        stats = _payoff_stats_from_returns(pd.Series(dtype=float), cfg)
        stats.update({"bucket_level": "none", "bucket_key": ""})
        return stats
    h["_hv_bin"] = h.get("prob_high_vol", pd.Series(0.0, index=h.index)).astype(float).clip(0, 0.999999).map(lambda x: _prob_bin_value(x, width))
    h["_up_bin"] = h.get("prob_up_strengthening_score", h.get("prob_up_strengthening", pd.Series(0.0, index=h.index))).astype(float).clip(0, 0.999999).map(lambda x: _prob_bin_value(x, width))
    h["_dn_bin"] = h.get("prob_down_strengthening_score", pd.Series(0.0, index=h.index)).astype(float).clip(0, 0.999999).map(lambda x: _prob_bin_value(x, width))
    candidates = [
        ("hv_up_down", (h["_hv_bin"].eq(hv_bin) & h["_up_bin"].eq(up_bin) & h["_dn_bin"].eq(dn_bin)), f"hv{hv_bin}|up{up_bin}|dn{dn_bin}"),
        ("hv_up", (h["_hv_bin"].eq(hv_bin) & h["_up_bin"].eq(up_bin)), f"hv{hv_bin}|up{up_bin}"),
        ("hv", h["_hv_bin"].eq(hv_bin), f"hv{hv_bin}"),
        ("global", pd.Series(True, index=h.index), "global"),
    ]
    for level, mask, key in candidates:
        part = h.loc[mask, ret_col].dropna().astype(float)
        if len(part) >= min_samples or level == "global":
            stats = _payoff_stats_from_returns(part, cfg)
            stats.update({"bucket_level": level, "bucket_key": key})
            return stats
    stats = _payoff_stats_from_returns(pd.Series(dtype=float), cfg)
    stats.update({"bucket_level": "none", "bucket_key": ""})
    return stats
...
def _multiplier_from_payoff_rr(rr: float, cfg: Config) -> Tuple[float, str]:
    if pd.isna(rr):
        return 1.0, "no_stats"
    rr = float(rr)
    if rr < float(getattr(cfg, "payoff_overlay_rr_cut_1", 0.70)):
        return float(getattr(cfg, "payoff_overlay_mult_1", 0.78)), "rr_cut_1"
    if rr < float(getattr(cfg, "payoff_overlay_rr_cut_2", 0.90)):
        return float(getattr(cfg, "payoff_overlay_mult_2", 0.86)), "rr_cut_2"
    if rr < float(getattr(cfg, "payoff_overlay_rr_cut_3", 1.10)):
        return float(getattr(cfg, "payoff_overlay_mult_3", 0.93)), "rr_cut_3"
    return 1.0, "rr_ok"


def apply_probability_payoff_overlay(signal_w: Tuple[float, float, float], base_signal_w: Tuple[float, float, float], row: pd.Series, hist: pd.DataFrame, cfg: Config, current_portfolio_drawdown: float) -> Tuple[Tuple[float, float, float], Dict[str, object]]:
    if not bool(getattr(cfg, "payoff_overlay_enabled", True)):
        return signal_w, {"payoff_overlay_enabled": False, "payoff_overlay_action": "disabled", "payoff_overlay_multiplier": 1.0, "payoff_overlay_force_rebalance": False}
    stats = lookup_probability_payoff_stats(hist, row, cfg)
    rr = float(stats.get("rr", np.nan)) if not pd.isna(stats.get("rr", np.nan)) else np.nan
    rr_mult, rr_action = _multiplier_from_payoff_rr(rr, cfg)
    ph = _row_float(row, "prob_high_vol", 0.0)
    dd_mult = 1.0
    dd_action = "dd_ok"
    force_rebalance = False
    dd = float(current_portfolio_drawdown)
    if bool(getattr(cfg, "payoff_overlay_drawdown_guard_enabled", True)):
        if dd <= float(getattr(cfg, "payoff_overlay_dd3", -0.10)) and ph >= float(getattr(cfg, "payoff_overlay_dd3_high_vol", 0.60)):
            dd_mult = min(dd_mult, float(getattr(cfg, "payoff_overlay_dd3_mult", 0.70))); dd_action = "dd_guard_3"; force_rebalance = bool(getattr(cfg, "payoff_overlay_force_dd_rebalance", True))
        elif dd <= float(getattr(cfg, "payoff_overlay_dd2", -0.07)) and ph >= float(getattr(cfg, "payoff_overlay_dd2_high_vol", 0.50)):
            dd_mult = min(dd_mult, float(getattr(cfg, "payoff_overlay_dd2_mult", 0.85))); dd_action = "dd_guard_2"; force_rebalance = bool(getattr(cfg, "payoff_overlay_force_dd_rebalance", True))
        elif dd <= float(getattr(cfg, "payoff_overlay_dd1", -0.04)) and ph >= float(getattr(cfg, "payoff_overlay_dd1_high_vol", 0.45)):
            dd_mult = min(dd_mult, float(getattr(cfg, "payoff_overlay_dd1_mult", 0.92))); dd_action = "dd_guard_1"; force_rebalance = bool(getattr(cfg, "payoff_overlay_force_dd_rebalance", True))
    multiplier = float(np.clip(min(rr_mult, dd_mult), 0.0, 1.0))
    stock_before = float(signal_w[0])
    min_stock = float(getattr(cfg, "payoff_overlay_min_stock_weight", 0.30))
    stock_after = float(np.clip(stock_before * multiplier, min_stock, 1.0))
    if stock_before <= float(base_signal_w[0]) + 1e-12 and dd_action == "dd_ok":
        stock_after = stock_before
        multiplier = 1.0
        rr_action = "base_or_defensive_skip"
    w_after = _redistribute_after_stock_change(stock_after, signal_w)
    if bool(getattr(cfg, "payoff_overlay_force_rr_cut_rebalance", False)) and multiplier < 0.999 and dd_action == "dd_ok":
        force_rebalance = True
    action = "none"
    if stock_after < stock_before - 1e-12:
        action = dd_action if dd_action != "dd_ok" else rr_action
    return w_after, {
        "payoff_overlay_enabled": True,
        "payoff_overlay_action": str(action),
        "payoff_overlay_rr_action": str(rr_action),
        "payoff_overlay_dd_action": str(dd_action),
        "payoff_overlay_multiplier": float(multiplier),
        "payoff_overlay_force_rebalance": bool(force_rebalance),
        "payoff_overlay_stock_before": float(stock_before),
        "payoff_overlay_stock_after": float(w_after[0]),
        "payoff_overlay_stock_cut": float(stock_before - w_after[0]),
        "payoff_overlay_portfolio_dd_pre": float(dd),
        "payoff_overlay_horizon": int(getattr(cfg, "payoff_overlay_horizon", 20)),
        "payoff_overlay_bucket_level": str(stats.get("bucket_level", "")),
        "payoff_overlay_bucket_key": str(stats.get("bucket_key", "")),
        "payoff_overlay_sample_count": int(stats.get("sample_count", 0) or 0),
        "payoff_overlay_mean_return": float(stats.get("mean_return", np.nan)),
        "payoff_overlay_q75_return": float(stats.get("q75_return", np.nan)),
        "payoff_overlay_q20_return": float(stats.get("q20_return", np.nan)),
        "payoff_overlay_cvar_loss": float(stats.get("cvar_loss", np.nan)),
        "payoff_overlay_win_rate": float(stats.get("win_rate", np.nan)),
        "payoff_overlay_reward": float(stats.get("reward", np.nan)),
        "payoff_overlay_risk": float(stats.get("risk", np.nan)),
        "payoff_overlay_rr": float(rr) if not pd.isna(rr) else np.nan,
    }
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    equity_net = float(cfg.initial_capital)
    equity_peak_net = float(cfg.initial_capital)
    ...
    for i, row in pred_df.iterrows():
        ...
        payoff_hist = _payoff_hist_slice(pred_df, i, cfg)
        current_portfolio_dd = float(equity_net / equity_peak_net - 1.0) if equity_peak_net > 0 else 0.0
        signal_w, payoff_meta = apply_probability_payoff_overlay(
            signal_w=signal_w,
            base_signal_w=base_signal_w,
            row=row,
            hist=payoff_hist,
            cfg=cfg,
            current_portfolio_drawdown=current_portfolio_dd,
        )
        policy_meta.update(payoff_meta)
        ...
        risk_reduction_override = bool(policy_meta.get("payoff_overlay_force_rebalance", False))
        if strong_offensive_override or risk_reduction_override:
            rebalance_due = True
        ...
        if prev_w is None:
            ...
            if risk_reduction_override:
                w = signal_w
                executed_regime = signal_regime
                hold_reason = "payoff_overlay_risk_cut"
                trade_executed = True
            elif strong_offensive_override:
                w = signal_w
                executed_regime = signal_regime
        ...
        out.update({
            ...
            "payoff_overlay_risk_cut_override": bool(risk_reduction_override),
            ...
            "payoff_overlay_enabled": bool(policy_meta.get("payoff_overlay_enabled", False)),
            "payoff_overlay_action": str(policy_meta.get("payoff_overlay_action", "")),
            "payoff_overlay_rr_action": str(policy_meta.get("payoff_overlay_rr_action", "")),
            "payoff_overlay_dd_action": str(policy_meta.get("payoff_overlay_dd_action", "")),
            "payoff_overlay_multiplier": float(policy_meta.get("payoff_overlay_multiplier", 1.0)),
            "payoff_overlay_stock_before": float(policy_meta.get("payoff_overlay_stock_before", np.nan)),
            "payoff_overlay_stock_after": float(policy_meta.get("payoff_overlay_stock_after", np.nan)),
            "payoff_overlay_stock_cut": float(policy_meta.get("payoff_overlay_stock_cut", 0.0)),
            "payoff_overlay_portfolio_dd_pre": float(policy_meta.get("payoff_overlay_portfolio_dd_pre", 0.0)),
            "payoff_overlay_bucket_level": str(policy_meta.get("payoff_overlay_bucket_level", "")),
            "payoff_overlay_bucket_key": str(policy_meta.get("payoff_overlay_bucket_key", "")),
            "payoff_overlay_sample_count": int(policy_meta.get("payoff_overlay_sample_count", 0)),
            "payoff_overlay_mean_return": float(policy_meta.get("payoff_overlay_mean_return", np.nan)),
            "payoff_overlay_q75_return": float(policy_meta.get("payoff_overlay_q75_return", np.nan)),
            "payoff_overlay_q20_return": float(policy_meta.get("payoff_overlay_q20_return", np.nan)),
            "payoff_overlay_cvar_loss": float(policy_meta.get("payoff_overlay_cvar_loss", np.nan)),
            "payoff_overlay_win_rate": float(policy_meta.get("payoff_overlay_win_rate", np.nan)),
            "payoff_overlay_reward": float(policy_meta.get("payoff_overlay_reward", np.nan)),
            "payoff_overlay_risk": float(policy_meta.get("payoff_overlay_risk", np.nan)),
            "payoff_overlay_rr": float(policy_meta.get("payoff_overlay_rr", np.nan)),
            "force_rebalance_signal": bool(policy_meta.get("force_rebalance", False) or policy_meta.get("payoff_overlay_force_rebalance", False)),
        ...
        equity_net = float(equity_net * (1.0 + net))
        equity_peak_net = float(max(equity_peak_net, equity_net))
        prev_w = w
...
def classification_metrics(pred_df: pd.DataFrame, cfg: Config) -> Dict[str, object]:
    ...
    # v8.6.37: prob_up/prob_down은 약한 출력으로 판정되어 summary 핵심 성능에서 제외한다.
    ...
    if "actual_direction_strength" in pred_df.columns:
        ...
        # v8.6.37: prob_bear_down_strengthening은 allocation/summary 성능에서 제외한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_probability_payoff_overlay_v8_6_37_diagnostics",
        ...
        "direction_strength_specialist": {
            ...
            "payoff_overlay_enabled": bool(getattr(cfg, "payoff_overlay_enabled", True)),
            "payoff_overlay_action_distribution_pct": pred_df.get("payoff_overlay_action", pd.Series("", index=pred_df.index)).astype(str).value_counts(normalize=True).mul(100).round(2).to_dict(),
            "payoff_overlay_bucket_level_distribution_pct": pred_df.get("payoff_overlay_bucket_level", pd.Series("", index=pred_df.index)).astype(str).value_counts(normalize=True).mul(100).round(2).to_dict(),
            "payoff_overlay_avg_rr": float(pred_df.get("payoff_overlay_rr", pd.Series(np.nan, index=pred_df.index)).astype(float).mean()),
            "payoff_overlay_avg_multiplier": float(pred_df.get("payoff_overlay_multiplier", pd.Series(1.0, index=pred_df.index)).astype(float).mean()),
            "payoff_overlay_avg_stock_cut": float(pred_df.get("payoff_overlay_stock_cut", pd.Series(0.0, index=pred_df.index)).astype(float).mean()),
            "payoff_overlay_cut_rate": float((pred_df.get("payoff_overlay_stock_cut", pd.Series(0.0, index=pred_df.index)).astype(float) > 1e-12).mean()),
            "payoff_overlay_risk_cut_override_rate": float(pred_df.get("payoff_overlay_risk_cut_override", pd.Series(False, index=pred_df.index)).astype(bool).mean()),
        ...
        "latest_prediction": {
            ...
            "payoff_overlay_enabled": bool(latest.get("payoff_overlay_enabled", False)),
            "payoff_overlay_action": str(latest.get("payoff_overlay_action", "")),
            "payoff_overlay_rr_action": str(latest.get("payoff_overlay_rr_action", "")),
            "payoff_overlay_dd_action": str(latest.get("payoff_overlay_dd_action", "")),
            "payoff_overlay_rr": round(float(latest.get("payoff_overlay_rr", np.nan)), 4) if pd.notna(latest.get("payoff_overlay_rr", np.nan)) else None,
            "payoff_overlay_multiplier": round(float(latest.get("payoff_overlay_multiplier", 1.0)), 4),
            "payoff_overlay_sample_count": int(latest.get("payoff_overlay_sample_count", 0)),
            "payoff_overlay_bucket_level": str(latest.get("payoff_overlay_bucket_level", "")),
            "payoff_overlay_bucket_key": str(latest.get("payoff_overlay_bucket_key", "")),
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_probability_payoff_overlay_v8_6_37_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_probability_payoff_overlay_v8_6_37_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_probability_payoff_overlay_v8_6_37_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.37 Probability-Payoff Overlay Volatility-Base Multi-Horizon Upside Strength Trigger")
    ...
    parser.add_argument("--rebalance-every", type=int, default=None, help="리밸런싱 주기 직접 지정. v8.6.37 기본 5거래일")
    parser.add_argument("--emergency-cooldown", type=int, default=None, help="긴급 리밸런싱 cooldown 직접 지정. v8.6.37 기본 5거래일")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist", "portfolio_model"], default=None, help="v8.6.37 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.37 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.37 기본 ON인 stale offensive decay 비활성화")
    ...
    parser.add_argument("--no-horizon-train-window", action="store_true", help="v8.6.37 horizon별 train_rows = horizon*multiplier 모드 비활성화")
    ...
    parser.add_argument("--disable-normal-unrestricted", action="store_true", help="v8.6.37 정상 상태 비중 제한 완화 정책 비활성화")
    ...
    parser.add_argument("--disable-payoff-overlay", action="store_true", help="v8.6.37 확률별 손익비 기반 비중 감속 overlay 비활성화")
    parser.add_argument("--payoff-horizon", type=int, default=None, help="손익비 계산용 미래수익률 horizon. 기본 20")
    parser.add_argument("--payoff-train-rows", type=int, default=None, help="손익비 table 계산용 과거 OOS 샘플 수. 기본 756")
    parser.add_argument("--payoff-min-samples", type=int, default=None, help="확률 bin별 최소 샘플 수. 기본 60")
    parser.add_argument("--payoff-bin-width", type=float, default=None, help="확률 bin 폭. 기본 0.20")
    parser.add_argument("--payoff-rr-cut-1", type=float, default=None, help="강한 감속 RR threshold. 기본 0.70")
    parser.add_argument("--payoff-rr-cut-2", type=float, default=None, help="중간 감속 RR threshold. 기본 0.90")
    parser.add_argument("--payoff-rr-cut-3", type=float, default=None, help="약한 감속 RR threshold. 기본 1.10")
    parser.add_argument("--payoff-mult-1", type=float, default=None, help="RR cut 1 주식 비중 multiplier. 기본 0.78")
    parser.add_argument("--payoff-mult-2", type=float, default=None, help="RR cut 2 주식 비중 multiplier. 기본 0.86")
    parser.add_argument("--payoff-mult-3", type=float, default=None, help="RR cut 3 주식 비중 multiplier. 기본 0.93")
    parser.add_argument("--payoff-min-stock", type=float, default=None, help="payoff overlay 적용 후 최소 주식 비중. 기본 0.30")
    parser.add_argument("--payoff-force-rr-rebalance", action="store_true", help="RR 감속 발생 시 schedule/no-trade-band 우회")
    parser.add_argument("--disable-payoff-dd-guard", action="store_true", help="portfolio drawdown guard 비활성화")
    parser.add_argument("--payoff-dd1", type=float, default=None, help="drawdown guard 1 threshold. 기본 -0.04")
    parser.add_argument("--payoff-dd2", type=float, default=None, help="drawdown guard 2 threshold. 기본 -0.07")
    parser.add_argument("--payoff-dd3", type=float, default=None, help="drawdown guard 3 threshold. 기본 -0.10")
    parser.add_argument("--payoff-dd1-high-vol", type=float, default=None, help="drawdown guard 1 high-vol threshold. 기본 0.45")
    parser.add_argument("--payoff-dd2-high-vol", type=float, default=None, help="drawdown guard 2 high-vol threshold. 기본 0.50")
    parser.add_argument("--payoff-dd3-high-vol", type=float, default=None, help="drawdown guard 3 high-vol threshold. 기본 0.60")
    parser.add_argument("--payoff-dd1-mult", type=float, default=None, help="drawdown guard 1 multiplier. 기본 0.92")
    parser.add_argument("--payoff-dd2-mult", type=float, default=None, help="drawdown guard 2 multiplier. 기본 0.85")
    parser.add_argument("--payoff-dd3-mult", type=float, default=None, help="drawdown guard 3 multiplier. 기본 0.70")
    parser.add_argument("--disable-payoff-force-dd-rebalance", action="store_true", help="drawdown guard 발생 시 강제 리밸런싱 비활성화")
    parser.add_argument("--disable-vol-base-allocation", action="store_true", help="v8.6.37 high-vol 확률 기반 기본 비중을 끄고 regime bucket 기본 비중 사용")
    ...
    parser.add_argument("--disable-tier2", action="store_true", help="Tier2 비활성화. v8.6.37 기본값은 활성화")
...
def drop_weak_probability_output_columns(pred_df: pd.DataFrame) -> pd.DataFrame:
    """
    v8.6.37 external-output filter.
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_probability_payoff_overlay_v8_6_37_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_probability_payoff_overlay_v8_6_37_summary.json"
    ...
    cfg.result_dir = f"results_xgb_probability_payoff_overlay_v8_6_37_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.37: 기본 상태는 prob_high_vol 기반 포트폴리오, 강한 신호만 즉시 override한다.
        cfg.use_vol_probability_base_allocation = True
        # v8.6.37: 정상/저변동 상태 기본 비중 상향.
        ...
        # v8.6.37: Tier2를 기본 활성화한다. 기존 실패 구간 재검증을 위해 CLI로 조건/비중 조정 가능.
        ...
        # v8.6.37: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
    ...
    if getattr(args, "disable_payoff_overlay", False):
        cfg.payoff_overlay_enabled = False
    if getattr(args, "payoff_horizon", None) is not None:
        cfg.payoff_overlay_horizon = int(args.payoff_horizon)
    if getattr(args, "payoff_train_rows", None) is not None:
        cfg.payoff_overlay_train_rows = int(args.payoff_train_rows)
    if getattr(args, "payoff_min_samples", None) is not None:
        cfg.payoff_overlay_min_samples = int(args.payoff_min_samples)
    if getattr(args, "payoff_bin_width", None) is not None:
        cfg.payoff_overlay_bin_width = float(args.payoff_bin_width)
    if getattr(args, "payoff_rr_cut_1", None) is not None:
        cfg.payoff_overlay_rr_cut_1 = float(args.payoff_rr_cut_1)
    if getattr(args, "payoff_rr_cut_2", None) is not None:
        cfg.payoff_overlay_rr_cut_2 = float(args.payoff_rr_cut_2)
    if getattr(args, "payoff_rr_cut_3", None) is not None:
        cfg.payoff_overlay_rr_cut_3 = float(args.payoff_rr_cut_3)
    if getattr(args, "payoff_mult_1", None) is not None:
        cfg.payoff_overlay_mult_1 = float(args.payoff_mult_1)
    if getattr(args, "payoff_mult_2", None) is not None:
        cfg.payoff_overlay_mult_2 = float(args.payoff_mult_2)
    if getattr(args, "payoff_mult_3", None) is not None:
        cfg.payoff_overlay_mult_3 = float(args.payoff_mult_3)
    if getattr(args, "payoff_min_stock", None) is not None:
        cfg.payoff_overlay_min_stock_weight = float(args.payoff_min_stock)
    if getattr(args, "payoff_force_rr_rebalance", False):
        cfg.payoff_overlay_force_rr_cut_rebalance = True
    if getattr(args, "disable_payoff_dd_guard", False):
        cfg.payoff_overlay_drawdown_guard_enabled = False
    if getattr(args, "payoff_dd1", None) is not None:
        cfg.payoff_overlay_dd1 = float(args.payoff_dd1)
    if getattr(args, "payoff_dd2", None) is not None:
        cfg.payoff_overlay_dd2 = float(args.payoff_dd2)
    if getattr(args, "payoff_dd3", None) is not None:
        cfg.payoff_overlay_dd3 = float(args.payoff_dd3)
    if getattr(args, "payoff_dd1_high_vol", None) is not None:
        cfg.payoff_overlay_dd1_high_vol = float(args.payoff_dd1_high_vol)
    if getattr(args, "payoff_dd2_high_vol", None) is not None:
        cfg.payoff_overlay_dd2_high_vol = float(args.payoff_dd2_high_vol)
    if getattr(args, "payoff_dd3_high_vol", None) is not None:
        cfg.payoff_overlay_dd3_high_vol = float(args.payoff_dd3_high_vol)
    if getattr(args, "payoff_dd1_mult", None) is not None:
        cfg.payoff_overlay_dd1_mult = float(args.payoff_dd1_mult)
    if getattr(args, "payoff_dd2_mult", None) is not None:
        cfg.payoff_overlay_dd2_mult = float(args.payoff_dd2_mult)
    if getattr(args, "payoff_dd3_mult", None) is not None:
        cfg.payoff_overlay_dd3_mult = float(args.payoff_dd3_mult)
    if getattr(args, "disable_payoff_force_dd_rebalance", False):
        cfg.payoff_overlay_force_dd_rebalance = False
    ...
    required_horizons = (
        set(tuple(cfg.horizons))
        | {int(getattr(cfg, "direction_strength_horizon", 20))}
        | set(get_multi_strength_horizons(cfg))
        | {int(getattr(cfg, "payoff_overlay_horizon", 20))}
    )
    ...
    file_prefix = f"{safe_ticker}_xgb_probability_payoff_overlay_v8_6_37"
