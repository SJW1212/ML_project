"""
XGBoost v8.6.35 - Volatility-Base + Selective Strength Combo Policy
====================================================================

목적
- QQQ/IEF/BIL 동적 자산배분 전략
- v8.6.35: 5D/10D/20D 전체 조합은 진단으로 유지하되, allocation action은 검증된 5D+10D 및 5D+10D+20D 중심으로 제한
...
실행 예시
    py xgb_strength_combo_selective_v8_6_35.py --speed-profile balanced --h10-down-only --condition-search
    py xgb_strength_combo_selective_v8_6_35.py --speed-profile fast --h10-down-only
    py xgb_strength_combo_selective_v8_6_35.py --speed-profile full --adaptive-label --h10-down-only
...
class Config:
    ...
    result_dir: str = "results_xgb_strength_combo_selective_v8_6_35"
    ...
    # v8.6.35 base allocation: 기본 상태는 high-vol 확률 기반 연속/계단형 비중으로 산출한다.
    ...
    # v8.6.35 execution: Tier3/Full은 schedule/no-trade-band에 묶이지 않고 즉시 목표 비중으로 점프한다.
    ...
    # v8.6.35 Separate PortfolioPolicyModel
    ...
    # v8.6.35 Volatility-Base Strong-Override Multi-Horizon Upside Strength Trigger
    ...
    # v8.6.35: prob_bear_down_strengthening 제거. 하락 강화도 상승 강화와 동일하게 5D/10D/20D term structure로 계산한다.
    ...
    up_strength_bonus_threshold_2: float = 0.38  # v8.6.35: Tier2 활성화 기본값
    ...
    up_strength_pred_threshold_5d: float = 0.99  # v8.6.35: 5D 단독 trigger 비활성화
    ...
    # v8.6.35: 5D/10D/20D 단독과 모든 2-way/3-way 조합을 strength_combo ladder에서 사용한다.
    ...
    strength_combo_policy_mode: str = "selective_quality"  # off, diagnostic, max_weight, selective_quality
    ...
    strength_combo_single_5d_stock_weight: float = 0.78
    strength_combo_single_10d_stock_weight: float = 0.80
    strength_combo_single_20d_stock_weight: float = 0.80
    strength_combo_pair_5d_10d_stock_weight: float = 0.84
    strength_combo_pair_5d_20d_stock_weight: float = 0.80
    strength_combo_pair_10d_20d_stock_weight: float = 0.82
    strength_combo_all3_stock_weight: float = 0.90
    strength_combo_force_all3_rebalance: bool = False
    # v8.6.35: Tier2를 완전 제거하지 않고 soft-weight로 낮춘 뒤, 검증된 confirm에서만 강화한다.
    tier2_soft_weight_enabled: bool = True
    tier2_base_stock_weight: float = 0.82
    tier2_confirm_stock_weight: float = 0.88
    tier2_strong_confirm_stock_weight: float = 0.90
    # v8.6.35: weak combo 단독 action 방지. 진단 컬럼은 계속 저장한다.
    strength_combo_action_scope: str = "quality_only"  # quality_only, base_and_tier1, all

    # v8.6.35 TierWeightOptimizer
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            if bool(getattr(cfg, "use_direction_strength_specialist", True)):
                try:
                    ...
                    # v8.6.35: 별도 bear-filter specialist 제거.
        ...
        if bool(getattr(cfg, "use_direction_strength_specialist", True)):
            ...
            bear_strength_probs = {name: 0.0 for name in DIRECTION_STRENGTH_LABELS}  # removed in v8.6.35
        else:
        ...
        out.update({
            ...
            **{f"prob_up_weakening_{int(sh)}d": 0.0 for sh in multi_strength_horizons},
            # v8.6.35: 하락 강화도 상승 강화와 동일한 horizon별 term structure로 산출한다.
...
def apply_direction_strength_specialist_policy(
    ...
    """
    v8.6.35 Volatility-Base Strong-Override Upside Strength Trigger.
    ...
    """
    ph = _row_float(row, "prob_high_vol")
    # v8.6.35: weak 확률(prob_up/prob_down/prob_bear_down_strengthening)은 allocation에서 제거한다.
    ...
    # v8.6.35: 5D/10D/20D 단독 및 모든 조합을 별도 진단 신호로 계산한다.
    combo_mode = str(getattr(cfg, "strength_combo_policy_mode", "selective_quality")).lower()
    ...
    combo_candidates: List[Tuple[str, bool, float, int]] = [
        ...
        ("5D+10D+20D", combo_all3_signal, float(getattr(cfg, "strength_combo_all3_stock_weight", 0.90)), 3),
    ]
    ...
    # v8.6.35: 강한 상승 override는 regime bucket보다 실제 high-vol cap을 우선한다.
    ...
    if True:
        ...
        # v8.6.35 ShortMidConfirm optional allocation actions.
        ...
        # v8.6.35 Strength Combo Policy.
        # 모든 조합은 진단 컬럼으로 저장하지만, 기본 selective_quality 모드에서는
        # 검증상 약했던 5D 단독/20D 단독/5D+20D/10D+20D action을 막는다.
        if combo_enabled and combo_mode == "max_weight" and active_combo_candidates:
            # 비교용 레거시 모드: v8.6.34와 동일하게 모든 combo 중 최대 비중을 적용한다.
            ...
            if best_combo_tier >= 3 and bool(getattr(cfg, "strength_combo_force_all3_rebalance", False)):
                force_rebalance = True
        elif combo_enabled and combo_mode in {"selective", "selective_quality", "quality"}:
            combo_scope = str(getattr(cfg, "strength_combo_action_scope", "quality_only")).lower()
            # 기본 quality_only/base_and_tier1에서는 기존 Tier2/Tier3/Full을 침범하지 않고 보조 승격만 수행한다.
            allow_combo_action = combo_scope == "all" or (
                combo_scope in {"quality_only", "base_and_tier1"}
                and not (tier2_signal or tier3_signal or full_stock_signal)
            )
            if allow_combo_action and combo_all3_signal:
                target_stock = max(target_stock, float(getattr(cfg, "strength_combo_all3_stock_weight", 0.90)))
                offensive_tier = max(offensive_tier, 2)
                combo_policy_action = "combo_selective_5D_10D_20D"
                if bool(getattr(cfg, "strength_combo_force_all3_rebalance", False)):
                    force_rebalance = True
            elif allow_combo_action and combo_5d_10d_signal:
                target_stock = max(target_stock, float(getattr(cfg, "strength_combo_pair_5d_10d_stock_weight", 0.84)))
                offensive_tier = max(offensive_tier, 1)
                combo_policy_action = "combo_selective_5D_10D"

        # Tier 2: v8.6.35 soft-weight.
        # 기존 Tier2를 완전 제거하면 성과가 낮아졌으므로 유지하되, 기본 비중은 낮추고
        # ShortMid/Strong/All3 confirmation이 붙을 때만 88~90%까지 강화한다.
        if tier2_signal:
            if bool(getattr(cfg, "tier2_soft_weight_enabled", True)):
                tier2_weight = float(getattr(cfg, "tier2_base_stock_weight", 0.82))
                if short_mid_confirm or combo_5d_10d_signal:
                    tier2_weight = max(tier2_weight, float(getattr(cfg, "tier2_confirm_stock_weight", 0.88)))
                if short_mid_strong_confirm or combo_all3_signal:
                    tier2_weight = max(tier2_weight, float(getattr(cfg, "tier2_strong_confirm_stock_weight", 0.90)))
            else:
            ...
            if short_mid_mode in {"tier2_add", "tier2_replace"} and short_mid_action_confirm:
                tier2_weight = max(tier2_weight, float(getattr(cfg, "short_mid_tier2_stock_weight", tier2_weight)))
    ...
    # v8.6.35: bear/down 약한 확률 기반 cut은 제거한다.
    ...
    meta = {
        ...
        "tier2_soft_weight_enabled": bool(getattr(cfg, "tier2_soft_weight_enabled", True)),
        "tier2_base_stock_weight": float(getattr(cfg, "tier2_base_stock_weight", 0.82)),
        "tier2_confirm_stock_weight": float(getattr(cfg, "tier2_confirm_stock_weight", 0.88)),
        "tier2_strong_confirm_stock_weight": float(getattr(cfg, "tier2_strong_confirm_stock_weight", 0.90)),
        ...
        "policy_note": "vol_base_selective_strength_combo_enabled",
    }
    return w, meta
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        out.update({
            ...
            "strength_combo_policy_mode": str(policy_meta.get("strength_combo_policy_mode", getattr(cfg, "strength_combo_policy_mode", "selective_quality"))),
            ...
            "tier2_soft_weight_enabled": bool(policy_meta.get("tier2_soft_weight_enabled", getattr(cfg, "tier2_soft_weight_enabled", True))),
            "tier2_base_stock_weight": float(policy_meta.get("tier2_base_stock_weight", getattr(cfg, "tier2_base_stock_weight", 0.82))),
            "tier2_confirm_stock_weight": float(policy_meta.get("tier2_confirm_stock_weight", getattr(cfg, "tier2_confirm_stock_weight", 0.88))),
            "tier2_strong_confirm_stock_weight": float(policy_meta.get("tier2_strong_confirm_stock_weight", getattr(cfg, "tier2_strong_confirm_stock_weight", 0.90))),
...
def classification_metrics(pred_df: pd.DataFrame, cfg: Config) -> Dict[str, object]:
    ...
    # v8.6.35: prob_up/prob_down은 약한 출력으로 판정되어 summary 핵심 성능에서 제외한다.
    ...
    if "actual_direction_strength" in pred_df.columns:
        ...
        # v8.6.35: prob_bear_down_strengthening은 allocation/summary 성능에서 제외한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_strength_combo_selective_v8_6_35_diagnostics",
        ...
        "direction_strength_specialist": {
            ...
            "strength_combo_policy_mode": str(getattr(cfg, "strength_combo_policy_mode", "selective_quality")),
            "strength_combo_action_scope": str(getattr(cfg, "strength_combo_action_scope", "quality_only")),
            "tier2_soft_weight_enabled": bool(getattr(cfg, "tier2_soft_weight_enabled", True)),
            "tier2_base_stock_weight": float(getattr(cfg, "tier2_base_stock_weight", 0.82)),
            "tier2_confirm_stock_weight": float(getattr(cfg, "tier2_confirm_stock_weight", 0.88)),
            "tier2_strong_confirm_stock_weight": float(getattr(cfg, "tier2_strong_confirm_stock_weight", 0.90)),
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_strength_combo_selective_v8_6_35_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_strength_combo_selective_v8_6_35_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_strength_combo_selective_v8_6_35_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.35 Volatility-Base Strong-Override Multi-Horizon Upside Strength Trigger + Objective Condition Search")
    ...
    parser.add_argument("--rebalance-every", type=int, default=None, help="리밸런싱 주기 직접 지정. v8.6.35 기본 5거래일")
    parser.add_argument("--emergency-cooldown", type=int, default=None, help="긴급 리밸런싱 cooldown 직접 지정. v8.6.35 기본 5거래일")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist", "portfolio_model"], default=None, help="v8.6.35 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.35 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.35 기본 ON인 stale offensive decay 비활성화")
    ...
    parser.add_argument("--no-horizon-train-window", action="store_true", help="v8.6.35 horizon별 train_rows = horizon*multiplier 모드 비활성화")
    ...
    parser.add_argument("--disable-vol-base-allocation", action="store_true", help="v8.6.35 high-vol 확률 기반 기본 비중을 끄고 regime bucket 기본 비중 사용")
    ...
    parser.add_argument("--disable-tier2", action="store_true", help="기존 Tier2 비활성화. v8.6.35 기본값은 soft-weight 유지")
    parser.add_argument("--enable-tier2", action="store_true", help="기존 10D+20D Tier2 조건을 활성화. v8.6.35 기본값은 이미 활성화")
    ...
    parser.add_argument("--strength-combo-mode", choices=["off", "diagnostic", "max_weight", "selective_quality", "selective", "quality"], default=None, help="5D/10D/20D 조합 사용 방식. 기본 selective_quality")
    ...
    parser.add_argument("--combo-single-5d-stock", type=float, default=None, help="5D 단독 조합 목표 주식 비중. 기본 0.78, selective_quality에서는 action 미사용")
    parser.add_argument("--combo-single-10d-stock", type=float, default=None, help="10D 단독 조합 목표 주식 비중. 기본 0.80, selective_quality에서는 action 미사용")
    parser.add_argument("--combo-single-20d-stock", type=float, default=None, help="20D 단독 조합 목표 주식 비중. 기본 0.80, selective_quality에서는 action 미사용")
    parser.add_argument("--combo-5d-10d-stock", type=float, default=None, help="5D+10D 조합 목표 주식 비중. 기본 0.84")
    parser.add_argument("--combo-5d-20d-stock", type=float, default=None, help="5D+20D 조합 목표 주식 비중. 기본 0.80, selective_quality에서는 action 미사용")
    parser.add_argument("--combo-10d-20d-stock", type=float, default=None, help="10D+20D 조합 목표 주식 비중. 기본 0.82, selective_quality에서는 action 미사용")
    parser.add_argument("--combo-all3-stock", type=float, default=None, help="5D+10D+20D 조합 목표 주식 비중. 기본 0.90")
    parser.add_argument("--strength-combo-action-scope", type=str, default=None, choices=["quality_only", "base_and_tier1", "all"], help="v8.6.35 selective combo action scope")
    parser.add_argument("--disable-tier2-soft-weight", action="store_true", help="Disable v8.6.35 soft Tier2 weighting and use legacy Tier2 weight")
    parser.add_argument("--tier2-base-stock", type=float, default=None, help="Soft Tier2 base stock weight")
    parser.add_argument("--tier2-confirm-stock", type=float, default=None, help="Soft Tier2 stock weight when 5D+10D/ShortMid confirms")
    parser.add_argument("--tier2-strong-confirm-stock", type=float, default=None, help="Soft Tier2 stock weight when strong/all3 confirms")
...
def drop_weak_probability_output_columns(pred_df: pd.DataFrame) -> pd.DataFrame:
    """
    v8.6.35 external-output filter.
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_strength_combo_selective_v8_6_35_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_strength_combo_selective_v8_6_35_summary.json"
    ...
    cfg.result_dir = f"results_xgb_strength_combo_selective_v8_6_35_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.35: 기본 상태는 prob_high_vol 기반 포트폴리오, 강한 신호만 즉시 override한다.
        ...
        # v8.6.35: 기존 Tier2는 완전 제거하지 않고 soft-weight로 유지한다. 필요하면 --disable-tier2로 제거한다.
        ...
        # v8.6.35: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
    ...
    if getattr(args, "strength_combo_action_scope", None) is not None:
        cfg.strength_combo_action_scope = str(args.strength_combo_action_scope)
    if getattr(args, "disable_tier2_soft_weight", False):
        cfg.tier2_soft_weight_enabled = False
    if getattr(args, "tier2_base_stock", None) is not None:
        cfg.tier2_base_stock_weight = float(args.tier2_base_stock)
    if getattr(args, "tier2_confirm_stock", None) is not None:
        cfg.tier2_confirm_stock_weight = float(args.tier2_confirm_stock)
    if getattr(args, "tier2_strong_confirm_stock", None) is not None:
        cfg.tier2_strong_confirm_stock_weight = float(args.tier2_strong_confirm_stock)
    ...
    file_prefix = f"{safe_ticker}_xgb_strength_combo_selective_v8_6_35"
