...
class Config:
    ...
    result_dir: str = "results_xgb_up_strength_offensive_v8_6_11"
    ...
    # v8.6.11 Offensive Recovery Fix
    ...
    # v8.6.11 offensive thresholds
    up_strength_bonus_threshold_1: float = 0.40
    ...
    up_strength_offensive_stock_weight_1: float = 0.97
    up_strength_offensive_stock_weight_2: float = 1.00
    up_strength_low_vol_threshold_1: float = 0.60
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    """
    v8.6.11: allocation gate에서 사용할 하락 위험 점수.
...
def apply_direction_strength_specialist_policy(
    ...
    low_vol_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.45))
    low_vol_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.35))
    t1 = float(getattr(cfg, "up_strength_bonus_threshold_1", 0.50))
    ...
    # v8.6.11: NORMAL뿐 아니라 low-risk WATCH에서도 공격 overlay 허용.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_up_strength_offensive_v8_6_11_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def make_condition_candidate_configs(base_cfg: Config, grid_size: str = "standard") -> List[Tuple[str, Config]]:
    ...
    if grid_size == "compact":
        ...
        # v8.6.11 standard: v8.6.10의 과도한 방어/turnover를 줄이는 공격형 후보군
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_up_strength_offensive_v8_6_11_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_up_strength_offensive_v8_6_11_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_up_strength_offensive_v8_6_11_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.11 Offensive Recovery Fix + Objective Condition Search")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.11 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.11 up-strength offensive overlay 모델 학습/사용 비활성화")
...
def main() -> None:
    ...
    cfg.result_dir = f"results_xgb_up_strength_offensive_v8_6_11_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        cfg.no_trade_band = max(cfg.no_trade_band, 0.16)
        ...
        cfg.up_strength_bonus_threshold_1 = float(getattr(cfg, "up_strength_bonus_threshold_1", 0.40))
        cfg.up_strength_bonus_threshold_2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.55))
        cfg.up_strength_low_vol_threshold_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.60))
    ...
    pred_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_11_predictions.csv"
    summary_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_11_summary.json"
    latest_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_11_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_11_stage1_feature_importance.csv"
    importance_up_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_11_up_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_11_downrisk_feature_importance.csv"
    importance_down_price_trend_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_11_downrisk_price_trend_feature_importance.csv"
    importance_down_price_volume_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_11_downrisk_price_volume_feature_importance.csv"
    importance_down_volatility_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_11_downrisk_volatility_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_11_condition_search.csv"
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_up_strength_offensive_v8_6_11_{name}.csv"
