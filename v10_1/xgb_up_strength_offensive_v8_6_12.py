...
class Config:
    ...
    extreme_stock_weight: float = 0.60
    extreme_bond_weight: float = 0.26
    extreme_cash_weight: float = 0.14
    ...
    watch_stock_weight: float = 0.90
    watch_bond_weight: float = 0.075
    watch_cash_weight: float = 0.025

    high_vol_stock_weight: float = 0.82
    high_vol_bond_weight: float = 0.125
    high_vol_cash_weight: float = 0.055

    risk_off_stock_weight: float = 0.68
    risk_off_bond_weight: float = 0.208
    risk_off_cash_weight: float = 0.112
    ...
    result_dir: str = "results_xgb_up_strength_offensive_v8_6_12"
    ...
    # Offensive target weights
    up_strength_offensive_stock_weight_1: float = 0.98
    up_strength_offensive_stock_weight_2: float = 1.00
    up_strength_low_vol_threshold_1: float = 0.65
    up_strength_low_vol_threshold_2: float = 0.45
    up_strength_bear_block_threshold: float = 0.75

    # v8.6.12 diagnostics / optional stale offensive de-risking
    # 기본값 False: v8.6.11에서 수익률에 기여했을 수 있는 scheduled hold 효과를 기본으로 훼손하지 않는다.
    # True: 이전 공격 비중이 현재 target보다 과도하게 높고 상승 강화 신호가 사라졌을 때 중간 리밸런싱한다.
    enable_stale_offensive_decay: bool = False
    stale_offensive_stock_gap_threshold: float = 0.055
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    """
    v8.6.12: allocation gate에서 사용할 하락 위험 점수.
...
def apply_direction_strength_specialist_policy(
    ...
    설계 의도:
    - 하락 탐지 branch는 allocation에서 기본 제외/축소한다.
    - Stage1 high-vol 확률은 위험 차단기로 유지한다.
    - UP_STRENGTHENING 확률이 높으면 NORMAL/low-vol 구간에서 주식 비중을 97~100%로 올린다.
    - bear specialist cut은 기본 OFF다. 켜더라도 high-vol + 매우 높은 bear_down_strengthening에서만 약하게 쓴다.
    """
    ph = _row_float(row, "prob_high_vol")
    ...
    # Offensive overlay:
    # v8.6.9에서 mid_trend_state == BEAR 조건이 보너스를 0회로 막았으므로 제거한다.
    # 다만 Stage1 high-vol이 낮고 bear specialist가 극단적으로 높지 않은 구간에서만 공격한다.
    low_vol_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.65))
    low_vol_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.35))
    t1 = float(getattr(cfg, "up_strength_bonus_threshold_1", 0.36))
    t2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.60))
    block_bear = float(getattr(cfg, "up_strength_bear_block_threshold", 0.60))

    offensive_active = False
    # v8.6.12: NORMAL뿐 아니라 low-risk WATCH에서도 공격 overlay 허용.
    # 목적은 v8.6.10의 up_bonus_activation_rate 2~3% 문제를 10~20%대로 올리는 것이다.
    if regime in {"NORMAL", "WATCH"} and ph < low_vol_1 and pbd < block_bear:
        if pus >= t1:
            stock = max(stock, float(getattr(cfg, "up_strength_offensive_stock_weight_1", 0.97)))
            bonus = max(bonus, stock - base_stock)
            offensive_active = True
        if ph < low_vol_2 and pus >= t2 and puw < 0.75:
            stock = max(stock, float(getattr(cfg, "up_strength_offensive_stock_weight_2", 1.00)))
            bonus = max(bonus, stock - base_stock)
            offensive_active = True
    ...
    max_bonus = float(getattr(cfg, "direction_strength_max_stock_bonus", 0.08))
    ...
    meta = {
        ...
        "policy_note": "up_strength_offensive_overlay_stage1_risk_gate",
    }
    return w, meta
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        if prev_w is None:
            ...
            # v8.6.12 optional: offensive target이 사라졌는데 이전 98~100% 비중이 과도하게 남는 문제를 실험적으로 제어한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_up_strength_offensive_v8_6_12_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def make_condition_candidate_configs(base_cfg: Config, grid_size: str = "standard") -> List[Tuple[str, Config]]:
    ...
    if grid_size == "compact":
        ...
        # v8.6.12 standard: v8.6.10의 과도한 방어/turnover를 줄이는 공격형 후보군
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_up_strength_offensive_v8_6_12_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_up_strength_offensive_v8_6_12_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_up_strength_offensive_v8_6_12_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.12 Offensive Fine Tune + Objective Condition Search")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.12 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.12 up-strength offensive overlay 모델 학습/사용 비활성화")
...
def main() -> None:
    ...
    cfg.result_dir = f"results_xgb_up_strength_offensive_v8_6_12_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.12: offensive recovery fix
        # - v8.6.10의 과도한 EXTREME_RISK/turnover를 줄인다.
        # - Stage1 high-vol은 유지하되 RISK_OFF/EXTREME 진입 임계값을 올린다.
        # - UP_STRENGTHENING은 NORMAL뿐 아니라 낮은 WATCH에서도 공격 overlay를 허용한다.
        ...
        cfg.no_trade_band = max(cfg.no_trade_band, 0.17)
        cfg.normal_stock_weight = 0.92
        cfg.normal_bond_weight = 0.06
        cfg.normal_cash_weight = 0.02
        cfg.watch_stock_weight = 0.90
        cfg.watch_bond_weight = 0.075
        cfg.watch_cash_weight = 0.025
        cfg.risk_off_stock_weight = 0.68
        cfg.risk_off_bond_weight, cfg.risk_off_cash_weight = _risk_off_bond_cash_from_stock(cfg.risk_off_stock_weight)
        cfg.extreme_high_vol_threshold = 0.86
        cfg.extreme_downrisk_threshold = 0.86
        cfg.extreme_stock_weight = 0.60
        cfg.extreme_bond_weight, cfg.extreme_cash_weight = _risk_off_bond_cash_from_stock(cfg.extreme_stock_weight)
        ...
        cfg.up_strength_bonus_threshold_2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.55))
        cfg.up_strength_low_vol_threshold_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.65))
        cfg.up_strength_low_vol_threshold_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.45))
        cfg.up_strength_bear_block_threshold = float(getattr(cfg, "up_strength_bear_block_threshold", 0.75))
        cfg.direction_strength_max_stock_bonus = max(float(getattr(cfg, "direction_strength_max_stock_bonus", 0.10)), 0.10)
    ...
    pred_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_12_predictions.csv"
    summary_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_12_summary.json"
    latest_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_12_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_12_stage1_feature_importance.csv"
    importance_up_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_12_up_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_12_downrisk_feature_importance.csv"
    importance_down_price_trend_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_12_downrisk_price_trend_feature_importance.csv"
    importance_down_price_volume_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_12_downrisk_price_volume_feature_importance.csv"
    importance_down_volatility_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_12_downrisk_volatility_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_up_strength_offensive_v8_6_12_condition_search.csv"
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_up_strength_offensive_v8_6_12_{name}.csv"
