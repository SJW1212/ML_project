"""
XGBoost v8.6.16 - Multi-Horizon Upside Strength Trigger Allocation
====================================================================
...
class Config:
    ...
    result_dir: str = "results_xgb_multi_horizon_upside_strength_v8_6_16"
    ...
    # v8.6.16 Multi-Horizon Upside Strength Trigger
    ...
    up_strength_weight_5d: float = 0.25
    up_strength_weight_10d: float = 0.35
    up_strength_weight_20d: float = 0.40
    ...
    # v8.6.16 multi-horizon upside score thresholds
    # up_strength_score = 0.25*P5D + 0.35*P10D + 0.40*P20D
    # 5D 단독 상승은 작은 공격만 허용하고, 10D/20D 확인 시 큰 공격으로 전환한다.
    ...
    up_strength_offensive_stock_weight_3: float = 0.98
    up_strength_low_vol_threshold_1: float = 0.78
    up_strength_low_vol_threshold_2: float = 0.72
    up_strength_low_vol_threshold_3: float = 0.65
    up_strength_bear_block_threshold: float = 0.75

    # v8.6.16 diagnostics / stale offensive de-risking
...
def ensemble_weights(cfg: Config) -> Tuple[Dict[int, float], Dict[int, float]]:
    # v8.6.16에서 cfg.horizons에는 UP_STRENGTHENING 5D 라벨 생성을 위해 5가 포함될 수 있다.
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    """
    v8.6.16: allocation gate에서 사용할 하락 위험 점수.
...
def build_direction_strength_labels(df: pd.DataFrame, cfg: Config, horizon: Optional[int] = None) -> Tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    v8.6.16에서는 5D/10D/20D specialist를 각각 학습하므로 horizon 인자를 선택적으로 받는다.
...
def apply_direction_strength_specialist_policy(
    ...
    설계 의도:
    - 기본 주식 비중은 v8.6.15보다 낮게 유지하되, 5D/10D/20D 상승 강화 확률의 기간 구조를 본다.
    - 5D만 높으면 단기 반등으로 보고 작은 공격만 허용한다.
    - 10D/20D가 함께 높을 때만 92~98% 이상의 큰 공격 전환을 허용한다.
    - Down-risk head는 allocation에서 계속 제외하고, Stage1 high-vol만 위험 차단기로 사용한다.
    """
    ph = _row_float(row, "prob_high_vol")
    ...
    low_vol_3 = float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.65))
    ...
    confirm10_t2 = float(getattr(cfg, "up_strength_confirm_10d_threshold_2", 0.30))
    confirm10_t3 = float(getattr(cfg, "up_strength_confirm_10d_threshold_3", 0.36))
    confirm20_t3 = float(getattr(cfg, "up_strength_confirm_20d_threshold_3", 0.36))
    ...
    if regime in {"NORMAL", "WATCH"} and pbd < block_bear:
        # Tier 1: score 또는 단기 5D가 충분히 높으면 소폭 공격.
        if ph < low_vol_1 and (pus_score >= t1 or pus5 >= max(0.20, t1 - 0.03)):
            stock = max(stock, float(getattr(cfg, "up_strength_offensive_stock_weight_1", 0.86)))
            bonus = max(bonus, stock - base_stock)
            offensive_active = True
            offensive_tier = max(offensive_tier, 1)
        # Tier 2: 10D 확인이 필요하다. 5D 단독 스파이크는 여기서 차단한다.
        if ph < low_vol_2 and pus_score >= t2 and pus10 >= confirm10_t2 and puw20 < 0.82:
            stock = max(stock, float(getattr(cfg, "up_strength_offensive_stock_weight_2", 0.92)))
            bonus = max(bonus, stock - base_stock)
            offensive_active = True
            offensive_tier = max(offensive_tier, 2)
        # Tier 3: 10D와 20D가 함께 높을 때만 강한 공격.
        if ph < low_vol_3 and pus_score >= t3 and pus10 >= confirm10_t3 and pus20 >= confirm20_t3 and puw20 < 0.78:
            stock = max(stock, float(getattr(cfg, "up_strength_offensive_stock_weight_3", 0.98)))
            bonus = max(bonus, stock - base_stock)
            offensive_active = True
    ...
    meta = {
        ...
        "policy_note": "multi_horizon_upside_strength_stage1_risk_gate",
    }
    return w, meta
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        if prev_w is None:
            ...
            # v8.6.16 optional: offensive target이 사라졌는데 이전 98~100% 비중이 과도하게 남는 문제를 실험적으로 제어한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_multi_horizon_upside_strength_v8_6_16_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def make_condition_candidate_configs(base_cfg: Config, grid_size: str = "standard") -> List[Tuple[str, Config]]:
    ...
    if grid_size == "compact":
        ...
        # v8.6.16 standard: v8.6.10의 과도한 방어/turnover를 줄이는 공격형 후보군
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_multi_horizon_upside_strength_v8_6_16_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_multi_horizon_upside_strength_v8_6_16_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_multi_horizon_upside_strength_v8_6_16_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.16 Multi-Horizon Upside Strength Trigger + Objective Condition Search")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.16 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.16 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.16 기본 ON인 stale offensive decay 비활성화")
...
def main() -> None:
    ...
    cfg.result_dir = f"results_xgb_multi_horizon_upside_strength_v8_6_16_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.16: v8.6.13보다 기본 비중을 다시 낮춘 lower-base 구조.
        ...
        # v8.6.16: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
    ...
    # v8.6.16 direct low-base overrides after policy defaults.
    if getattr(args, "base_normal_stock", None) is not None:
        cfg.normal_stock_weight = float(args.base_normal_stock)
        rem = max(0.0, 1.0 - cfg.normal_stock_weight); cfg.normal_bond_weight = rem * 2/3; cfg.normal_cash_weight = rem * 1/3
    if getattr(args, "base_watch_stock", None) is not None:
    ...
    pred_path = result_dir / "qqq_xgb_multi_horizon_upside_strength_v8_6_16_predictions.csv"
    summary_path = result_dir / "qqq_xgb_multi_horizon_upside_strength_v8_6_16_summary.json"
    latest_path = result_dir / "qqq_xgb_multi_horizon_upside_strength_v8_6_16_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_multi_horizon_upside_strength_v8_6_16_stage1_feature_importance.csv"
    importance_up_path = result_dir / "qqq_xgb_multi_horizon_upside_strength_v8_6_16_up_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_multi_horizon_upside_strength_v8_6_16_downrisk_feature_importance.csv"
    importance_down_price_trend_path = result_dir / "qqq_xgb_multi_horizon_upside_strength_v8_6_16_downrisk_price_trend_feature_importance.csv"
    importance_down_price_volume_path = result_dir / "qqq_xgb_multi_horizon_upside_strength_v8_6_16_downrisk_price_volume_feature_importance.csv"
    importance_down_volatility_path = result_dir / "qqq_xgb_multi_horizon_upside_strength_v8_6_16_downrisk_volatility_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_multi_horizon_upside_strength_v8_6_16_condition_search.csv"
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_multi_horizon_upside_strength_v8_6_16_{name}.csv"
