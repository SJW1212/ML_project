"""
XGBoost v8.6.2 Direction-No-Vol - Directional Up/Down without Volatility Features
====================================================================
...
class Config:
    ...
    # v8.6.2 Direction-No-Vol
    # - 방향성 Up/Down 모델에서는 변동성/ATR/Range 피처를 전부 제외한다.
    # - Stage1 고변동 모델은 전체 피처를 계속 사용한다.
    # - Down 방향성 앙상블은 price_trend + price_volume branch만 사용한다.
    # - volatility/high_vol branch 가중치는 기본 0으로 둔다.
    down_price_trend_weight: float = 0.40
    down_price_volume_weight: float = 0.30
    down_volatility_weight: float = 0.00
    ...
    result_dir: str = "results_xgb_v8_6_2_direction_no_vol"
...
def normalize_downrisk_branch_weights(cfg: Config) -> Dict[str, float]:
    raw = {
        ...
        # 방향성 탐지에서는 변동성 branch를 사용하지 않는다.
        "volatility": 0.0,
        "high_vol": 0.0,
    }
    total = sum(max(0.0, v) for v in raw.values())
    if total <= 0:
        return {"price_trend": 0.60, "price_volume": 0.40, "volatility": 0.00, "high_vol": 0.00}
    return {k: max(0.0, v) / total for k, v in raw.items()}


def build_direction_down_feature_sets(feature_cols: Sequence[str]) -> Dict[str, List[str]]:
    """방향성 하락 모델용 피처셋.

    중요:
    - 변동성/ATR/Range/Ulcer/BB/Keltner/Squeeze 계열은 제외한다.
    - Stage1 고변동 모델에는 전체 feature_cols가 그대로 들어가므로, 고변동 탐지 성능은 유지한다.
    - Up-model과 Down-model은 가격·추세 및 가격·거래량 정보만 사용한다.
    """
    sets = build_downrisk_feature_sets(feature_cols)
    return {
        "price_trend": sets.get("price_trend", []),
        "price_volume": sets.get("price_volume", []),
    }
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    - Down-model volatility: 사용하지 않음. 방향성 탐지에서 변동성 피처를 완전히 제외
    ...
    # 방향성 모델에서는 변동성 branch를 완전히 제외한다.
    # Stage1은 X_train_full=feature_cols를 사용하므로 고변동 탐지용 변동성 피처는 유지된다.
    down_feature_sets = build_direction_down_feature_sets(feature_cols)
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            for h in cfg.horizons:
                ...
                down_models: Dict[str, Optional[Pipeline]] = {"price_trend": None, "price_volume": None}
                down_available: Dict[str, bool] = {"price_trend": False, "price_volume": False}
        ...
        prob_up_ens = 0.0
        prob_down_ens = 0.0
        prob_down_branch_ens: Dict[str, float] = {"price_trend": 0.0, "price_volume": 0.0}
        ...
        for h in cfg.horizons:
            ...
            if bool(cfg.use_multi_branch_downrisk):
                p_down = (
                    ...
                    + branch_weights.get("volatility", 0.0) * branch_probs.get("volatility", 0.0)
                    + branch_weights.get("high_vol", 0.0) * p_high
                )
            else:
            ...
            out[f"prob_down_volatility_h{h}"] = branch_probs.get("volatility", 0.0)  # 방향성 탐지에서는 미사용
        ...
        out.update({
            ...
            "prob_down_volatility": prob_down_branch_ens.get("volatility", 0.0),  # 방향성 탐지에서는 미사용
    ...
    if cfg.use_prob_ewma:
        ...
        if bool(cfg.use_multi_branch_downrisk):
            pred_df["prob_down"] = (
                ...
                + branch_weights.get("volatility", 0.0) * pred_df["prob_down_volatility"]
                + branch_weights.get("high_vol", 0.0) * pred_df["prob_high_vol"]
            ).clip(0.0, 1.0)
        else:
    ...
    pred_df.attrs["downrisk_volatility_feature_importance_mean"] = {}  # 방향성 탐지에서는 volatility branch 미사용
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_multi_branch_directional_risk_v8_6_2_direction_no_vol_diagnostics",
        ...
        "feature_set": "directional_up_down_price_trend_price_volume_no_volatility_features",
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.6.2 Direction-No-Vol Diagnostics 결과 요약")
    print("H10/H20 Stage1 + Up/Down Direction without Volatility Features + Overall Risk Allocation")
    print("==============================")
    ...
    for key in [
        ...
        "down_price_trend_vs_primary", "down_price_volume_vs_primary",
    ]:
        if key in cls:
            m = cls[key]
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_v8_6_2_direction_no_vol_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_6_2_direction_no_vol_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_6_2_direction_no_vol_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.2 Direction-No-Vol Up/Down + Objective Condition Search")
...
def main() -> None:
    ...
    pred_path = result_dir / "qqq_xgb_v8_6_2_direction_no_vol_predictions.csv"
    summary_path = result_dir / "qqq_xgb_v8_6_2_direction_no_vol_summary.json"
    latest_path = result_dir / "qqq_xgb_v8_6_2_direction_no_vol_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_v8_6_2_direction_no_vol_stage1_feature_importance.csv"
    importance_up_path = result_dir / "qqq_xgb_v8_6_2_direction_no_vol_up_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_v8_6_2_direction_no_vol_downrisk_feature_importance.csv"
    importance_down_price_trend_path = result_dir / "qqq_xgb_v8_6_2_direction_no_vol_downrisk_price_trend_feature_importance.csv"
    importance_down_price_volume_path = result_dir / "qqq_xgb_v8_6_2_direction_no_vol_downrisk_price_volume_feature_importance.csv"
    importance_down_volatility_path = result_dir / "qqq_xgb_v8_6_2_direction_no_vol_downrisk_volatility_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_v8_6_2_direction_no_vol_condition_search.csv"
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_v8_6_2_direction_no_vol_{name}.csv"
