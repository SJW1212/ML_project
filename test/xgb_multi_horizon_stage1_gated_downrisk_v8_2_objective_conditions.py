"""
XGBoost v8.2 - Objective Condition Search for Stage1-Gated Down-risk Allocation
====================================================================
...
- v8.2 개선: H10 Down-risk 단독 사용을 기본값으로 채택
- v8.2 개선: no-trade band, RISK_OFF threshold, RISK_OFF 비중, 연속 조정 사용 여부를 validation 기반으로 비교 가능
- v8.2 개선: 조건 선택 근거를 condition_search CSV로 저장
- 비용/turnover를 고려해 10거래일 단위 리밸런싱 + 긴급 리밸런싱 제한
- 고변동 라벨 quantile 정책은 고정 또는 adaptive nested validation으로 선택 가능

실행 예시
    py xgb_multi_horizon_stage1_gated_downrisk_v8_2_objective_conditions.py --speed-profile balanced --h10-down-only --condition-search
    py xgb_multi_horizon_stage1_gated_downrisk_v8_2_objective_conditions.py --speed-profile fast --h10-down-only
    py xgb_multi_horizon_stage1_gated_downrisk_v8_2_objective_conditions.py --speed-profile full --adaptive-label --h10-down-only
...
class Config:
    ...
    result_dir: str = "results_xgb_v8_2_objective_conditions"
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_multi_horizon_stage1_gated_downrisk_v8_2_objective_conditions",
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.2 Objective Condition Search 결과 요약")
...
def allocated_subset_stats(df: pd.DataFrame, cfg: Config) -> Dict[str, float]:
    ...
    perf = perf_stats(df["strategy_return_net"], cfg.initial_capital)
    perf["annual_turnover"] = float(df["turnover"].mean() * 252.0)
    perf["avg_stock_weight"] = float(df["stock_weight"].mean())
    perf["rebalance_ratio"] = float(df["rebalanced"].mean())
    perf["emergency_rebalance_ratio"] = float(df["emergency_rebalance"].mean())
    return perf
...
def run_condition_search(
    ...
    for name, cand_cfg in candidates:
        ...
        for prefix, stats in [("select", select_stats), ("holdout", holdout_stats), ("full", full_stats)]:
            for key in ["cagr", "mdd", "sharpe", "sortino", "calmar", "annual_turnover", "avg_stock_weight", "rebalance_ratio", "emergency_rebalance_ratio"]:
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_v8_2_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_2_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_2_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.2 Objective Condition Search Allocation")
...
def main() -> None:
    ...
    pred_path = result_dir / "qqq_xgb_v8_2_predictions.csv"
    summary_path = result_dir / "qqq_xgb_v8_2_summary.json"
    latest_path = result_dir / "qqq_xgb_v8_2_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_v8_2_stage1_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_v8_2_downrisk_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_v8_2_condition_search.csv"
