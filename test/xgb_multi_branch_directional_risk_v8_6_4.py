"""
XGBoost v8.6.4 - Volatility-Adjusted Direction + Soft No-Cap Allocation
====================================================================
...
class Config:
    ...
    # v8.6.4 Overall risk evaluation
    # - 전체 리스크는 고변동 위험과 하락위험을 함께 본다.
    # - Down-risk만 보면 방어 실패/과잉 방어를 전체 국면 관점에서 해석하기 어렵다.
    overall_risk_high_vol_weight: float = 0.70
    overall_risk_down_weight: float = 0.20
    overall_risk_down_minus_up_weight: float = 0.10
    pred_overall_risk_threshold: float = 0.50

    # v8.6.4 Direction model labels
    ...
    # v8.6.4 Direction-label redesign
    # simple: 기존 단순 future_return threshold
    # vol_adj: future_return / expected_vol 기준. 단기 노이즈를 줄이기 위한 기본값.
    # barrier: future max/min return을 volatility barrier로 판정하는 근사 triple-barrier 라벨.
    direction_label_mode: str = "vol_adj"
    direction_z_threshold: float = 0.35
    direction_barrier_z: float = 0.90
    direction_ignore_neutral_training: bool = True
    direction_expected_vol_col: str = "realized_vol_20"

    # v8.6.4 Direction 영향 축소 / risk-score allocation
    use_risk_score_allocation: bool = True
    risk_score_high_vol_weight: float = 0.70
    risk_score_down_weight: float = 0.20
    risk_score_down_minus_up_weight: float = 0.10
    risk_allocation_strength: float = 0.70
    min_stock_weight: float = 0.35
    use_min_stock_floor: bool = True
    direction_stock_adjustment: float = 0.03
    trend_stock_adjustment: float = 0.03
    direction_use_max_high_vol: float = 0.55
    trend_bull_min_score: int = 4
    trend_bear_max_score: int = 2
    ...
    result_dir: str = "results_xgb_v8_6_4_soft_nocap_direction"
...
def compute_overall_risk_prob(
    ...
    핵심 변경:
    - 방향성 up/down 모델의 실측 ROC-AUC가 0.5 미만이었으므로 방향성 비중을 축소한다.
    - 기본값은 high-vol 70%, down 20%, down-up gap 10%다.
    - scalar와 pandas Series 모두 처리한다.
    """
    hv_w = max(0.0, float(getattr(cfg, "risk_score_high_vol_weight", cfg.overall_risk_high_vol_weight)))
    dn_w = max(0.0, float(getattr(cfg, "risk_score_down_weight", cfg.overall_risk_down_weight)))
    gap_w = max(0.0, float(getattr(cfg, "risk_score_down_minus_up_weight", cfg.overall_risk_down_minus_up_weight)))

    if prob_up is None:
        total = hv_w + dn_w
        if total <= 0:
            hv_w, dn_w, total = 0.70, 0.20, 0.90
        ...
        if total <= 0:
            hv_w, dn_w, gap_w, total = 0.70, 0.20, 0.10, 1.0
...
def expected_horizon_vol(df: pd.DataFrame, horizon: int, cfg: Config) -> pd.Series:
    """방향성 라벨용 예상 변동성. feature 시점의 과거 변동성만 사용한다."""
    vol_col = getattr(cfg, "direction_expected_vol_col", "realized_vol_20")
    if vol_col in df.columns:
        vol = df[vol_col].astype(float)
    elif "daily_return" in df.columns:
        vol = df["daily_return"].astype(float).rolling(20, min_periods=10).std()
    else:
        vol = pd.Series(0.01, index=df.index, dtype=float)
    # horizon 수익률 기준으로 스케일 조정. 너무 작은 값은 label 폭주 방지용 하한 적용.
    return (vol * math.sqrt(float(horizon))).replace([np.inf, -np.inf], np.nan).clip(lower=1e-4)
...
def make_direction_labels(df: pd.DataFrame, horizon: int, cfg: Config, direction: str) -> pd.Series:
    ...
    v8.6.4 개선:
    - simple: 기존 단순 수익률 threshold
    - vol_adj: future_return / expected_horizon_vol 기준
    - barrier: future max/min return의 volatility barrier 기반 근사 triple-barrier
    - direction_ignore_neutral_training=True면 애매한 중립 샘플을 NaN으로 두고 학습에서 제외한다.
    """
    if direction not in {"up", "down"}:
        raise ValueError(f"unknown direction: {direction}")

    mode = str(getattr(cfg, "direction_label_mode", "vol_adj")).lower()
    ignore_neutral = bool(getattr(cfg, "direction_ignore_neutral_training", True))
    idx = df.index
    ret = df[f"future_return_{horizon}d"].astype(float)
    y = pd.Series(np.nan if ignore_neutral else 0, index=idx, dtype=float)

    if mode == "simple":
        thr = float(getattr(cfg, "direction_return_threshold", 0.005))
        up_mask = ret > thr
        down_mask = ret < -thr
    elif mode in {"vol_adj", "z", "volatility_adjusted"}:
        exp_vol = expected_horizon_vol(df, horizon, cfg)
        z = ret / exp_vol
        z_thr = float(getattr(cfg, "direction_z_threshold", 0.35))
        up_mask = z > z_thr
        down_mask = z < -z_thr
    elif mode in {"barrier", "triple_barrier", "triple"}:
        exp_vol = expected_horizon_vol(df, horizon, cfg)
        max_z = df[f"future_max_return_{horizon}d"].astype(float) / exp_vol
        min_z = df[f"future_min_return_{horizon}d"].astype(float) / exp_vol
        b = float(getattr(cfg, "direction_barrier_z", 0.90))
        hit_up = max_z >= b
        hit_down = min_z <= -b
        # max/min만으로 선후관계를 알 수 없으므로 양쪽 장벽을 모두 친 경우는 중립 처리한다.
        up_mask = hit_up & ~hit_down
        down_mask = hit_down & ~hit_up
    else:
...
def assign_direction_label(row: pd.Series, horizon: int, cfg: Config) -> str:
    ...
    # row 단위 expected vol 계산. realized_vol_20 등 과거 피처만 사용한다.
    vol_col = getattr(cfg, "direction_expected_vol_col", "realized_vol_20")
    base_vol = float(row.get(vol_col, np.nan))
    if not np.isfinite(base_vol) or base_vol <= 0:
        base_vol = float(row.get("daily_return", 0.01))
        base_vol = abs(base_vol) if np.isfinite(base_vol) and base_vol != 0 else 0.01
    exp_vol = max(base_vol * math.sqrt(float(horizon)), 1e-4)

    if mode in {"vol_adj", "z", "volatility_adjusted"}:
        z = ret / exp_vol
        z_thr = float(getattr(cfg, "direction_z_threshold", 0.35))
        if z > z_thr:
            return "상승"
    ...
    if mode in {"barrier", "triple_barrier", "triple"}:
        max_z = float(row[f"future_max_return_{horizon}d"]) / exp_vol
        min_z = float(row[f"future_min_return_{horizon}d"]) / exp_vol
        b = float(getattr(cfg, "direction_barrier_z", 0.90))
        hit_up = max_z >= b
        hit_down = min_z <= -b
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    """Walk-forward prediction with v8.6.4 directional Up/Down + overall risk.
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            for h in cfg.horizons:
                ...
                y_up_s = make_direction_labels(train_df, h, cfg, "up")
                y_down_s = make_direction_labels(train_df, h, cfg, "down")
                ...
                up_available = False
                up_mask = y_up_s.notna().values
                y_up = y_up_s.dropna().astype(int).values
                if len(y_up) > 0 and len(np.unique(y_up)) == 2 and int(y_up.sum()) >= int(getattr(cfg, "direction_min_positive", 20)):
                    up_model = make_xgb_downrisk(cfg, calc_scale_pos_weight(y_up))
                    up_model.fit(train_df.loc[up_mask, direction_cols], y_up)
                    up_available = True
                ...
                down_mask = y_down_s.notna().values
                y_down = y_down_s.dropna().astype(int).values
                if len(y_down) > 0 and len(np.unique(y_down)) == 2 and int(y_down.sum()) >= int(getattr(cfg, "direction_min_positive", 20)):
                    for branch, cols in down_feature_sets.items():
                        if not cols:
                            continue
                        m_down = make_xgb_downrisk(cfg, calc_scale_pos_weight(y_down))
                        m_down.fit(train_df.loc[down_mask, cols], y_down)
        ...
        for h in cfg.horizons:
            ...
            out[f"prob_down_risk_h{h}"] = p_down  # 호환 컬럼: v8.6.4에서는 방향성 하락 확률로 해석
...
def compute_mid_trend_state(row: pd.Series, cfg: Config) -> Tuple[int, str]:
    """H60/H120 방향성 확률 대신 쓰는 중기 추세 필터."""
    checks = [
        float(row.get("return_60d", 0.0)) > 0.0,
        float(row.get("return_120d", 0.0)) > 0.0,
        float(row.get("price_ma_60_gap", 0.0)) > 0.0,
        float(row.get("price_ma_120_gap", 0.0)) > 0.0,
        float(row.get("ma_gap_20_60", 0.0)) > 0.0,
        float(row.get("trend_slope_60", 0.0)) > 0.0,
    ]
    score = int(sum(bool(x) for x in checks))
    if score >= int(getattr(cfg, "trend_bull_min_score", 4)):
        return score, "BULL"
    if score <= int(getattr(cfg, "trend_bear_max_score", 2)):
        return score, "BEAR"
    return score, "NEUTRAL"


def split_defensive_weight(defensive_weight: float, risk_score: float, prob_high_vol: float) -> Tuple[float, float]:
    """남은 방어자산을 IEF/BIL로 나눈다. 위험이 높을수록 현금 비중을 늘린다."""
    defensive_weight = float(np.clip(defensive_weight, 0.0, 1.0))
    cash_ratio = float(np.clip(0.25 + 0.45 * risk_score + 0.15 * prob_high_vol, 0.25, 0.75))
    cash = defensive_weight * cash_ratio
    bond = defensive_weight - cash
    return float(bond), float(cash)


def continuous_risk_score_allocation(
    row: pd.Series,
    cfg: Config,
    g: Dict[str, float],
) -> Tuple[Tuple[float, float, float], str, float, int, str]:
    """v8.6.4 주식 비중 hard cap 제거형 연속 배분.

    - NORMAL=92% 같은 고정 상한을 쓰지 않는다.
    - risk_score가 충분히 낮으면 주식 비중은 95~100%까지 올라갈 수 있다.
    - 다만 emergency/tail-risk 조건에서는 주식 비중을 강제로 줄인다.
    """
    ph = float(np.clip(row.get("prob_high_vol", 0.0), 0.0, 1.0))
    p_up = float(np.clip(row.get("prob_up", 0.0), 0.0, 1.0))
    p_down = float(np.clip(row.get("prob_down", row.get("prob_down_risk", 0.0)), 0.0, 1.0))
    risk_score = float(np.clip(row.get("prob_overall_risk", compute_overall_risk_prob(ph, p_down, cfg, p_up)), 0.0, 1.0))

    trend_score, trend_state = compute_mid_trend_state(row, cfg)
    direction_gap = p_up - p_down

    # 방향성 신호는 직접 신뢰하지 않고, 고변동이 낮은 구간에서만 ±3% 정도 보정한다.
    adj = 0.0
    if ph <= float(getattr(cfg, "direction_use_max_high_vol", 0.55)):
        if direction_gap >= float(getattr(cfg, "direction_decision_margin", 0.05)):
            adj += float(getattr(cfg, "direction_stock_adjustment", 0.03))
        elif direction_gap <= -float(getattr(cfg, "direction_decision_margin", 0.05)):
            adj -= float(getattr(cfg, "direction_stock_adjustment", 0.03))
    if trend_state == "BULL" and ph < 0.50:
        adj += float(getattr(cfg, "trend_stock_adjustment", 0.03))
    elif trend_state == "BEAR" and ph > 0.45:
        adj -= float(getattr(cfg, "trend_stock_adjustment", 0.03))

    raw_stock = 1.0 - float(getattr(cfg, "risk_allocation_strength", 0.70)) * risk_score + adj

    # emergency trigger: 이 부분은 cap이라기보다 급락 방어 회로다.
    emergency_extreme = (
        ph >= float(g.get("extreme_high_vol_threshold", cfg.extreme_high_vol_threshold))
        and p_down >= float(g.get("extreme_downrisk_threshold", cfg.extreme_downrisk_threshold))
    )
    emergency_risk_off = (
        ph >= float(cfg.emergency_combined_high_vol_threshold)
        and p_down >= float(cfg.emergency_combined_down_threshold)
    ) or ph >= float(cfg.emergency_high_vol_threshold)

    if emergency_extreme:
        raw_stock = min(raw_stock, float(g.get("extreme_stock_weight", cfg.extreme_stock_weight)))
    elif emergency_risk_off:
        raw_stock = min(raw_stock, float(g.get("risk_off_stock_weight", cfg.risk_off_stock_weight)))

    min_stock = float(getattr(cfg, "min_stock_weight", 0.35)) if bool(getattr(cfg, "use_min_stock_floor", True)) else 0.0
    stock = float(np.clip(raw_stock, min_stock, 1.0))
    defensive = 1.0 - stock
    bond, cash = split_defensive_weight(defensive, risk_score, ph)

    # regime은 설명용이다. 실제 비중은 risk_score로 계산한다.
    if emergency_extreme:
        regime = "EXTREME_RISK"
    elif emergency_risk_off or risk_score >= 0.58:
        regime = "RISK_OFF"
    elif risk_score >= 0.38 or ph >= g.get("gate_normal_high_vol_threshold", 0.35):
        regime = "WATCH"
    else:
        regime = "NORMAL"
    return _normalize_weight_tuple(stock, bond, cash), regime, risk_score, trend_score, trend_state
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        if bool(getattr(cfg, "use_risk_score_allocation", True)):
            signal_w, signal_regime, allocation_risk_score, trend_score, trend_state = continuous_risk_score_allocation(row, cfg, current_g)
        else:
            ...
            allocation_risk_score = float(row.get("prob_overall_risk", pdn))
            trend_score, trend_state = compute_mid_trend_state(row, cfg)
        ...
        out.update({
            ...
            "allocation_risk_score": float(allocation_risk_score),
...
def binary_cls_metrics(y_true: np.ndarray, prob: np.ndarray, threshold: float, pos_name: str) -> Dict[str, object]:
    ...
    roc = safe_auc(y_true, prob, "roc")
    pr = safe_auc(y_true, prob, "pr")
    inv_roc = safe_auc(y_true, 1.0 - prob, "roc")
    inv_pr = safe_auc(y_true, 1.0 - prob, "pr")
    out["roc_auc"] = roc
    out["pr_auc"] = pr
    out["inverse_roc_auc"] = inv_roc
    out["inverse_pr_auc"] = inv_pr
    out["best_roc_auc_after_optional_inversion"] = max([x for x in [roc, inv_roc] if x is not None], default=None)
    out["probability_polarity"] = "inverse_better" if (roc is not None and inv_roc is not None and inv_roc > roc) else "normal"
    return out
...
def classification_metrics(pred_df: pd.DataFrame, cfg: Config) -> Dict[str, object]:
    ...
    if "allocation_risk_score" in pred_df.columns:
        p_alloc = pred_df["allocation_risk_score"].astype(float).clip(0.0, 1.0).values
        metrics["allocation_risk_vs_highvol_primary"] = binary_cls_metrics(
            y_primary, p_alloc, cfg.pred_overall_risk_threshold, "배분위험_by_고변동"
        )
        metrics["allocation_risk_vs_down_primary"] = binary_cls_metrics(
            y_down_primary, p_alloc, cfg.pred_overall_risk_threshold, "배분위험_by_하락"
        )
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_v8_6_4_voladj_direction_soft_nocap_allocation_diagnostics",
        ...
        "feature_set": "volatility_adjusted_direction_soft_nocap_allocation_features",
        ...
        "latest_prediction": {
            ...
            "allocation_risk_score": round(float(latest.get("allocation_risk_score", latest.get("prob_overall_risk", 0.0))) * 100, 2),
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.6.4 Diagnostics 결과 요약")
...
def objective_condition_score(stats: Dict[str, float], score_profile: str = "balanced") -> float:
    ...
    # v8.6.4: 평균 주식비중 자체는 제약하지 않는다. 대신 해석 불가능한 regime 쏠림만 약하게 페널티화한다.
    normal = float(stats.get("normal_pct", 0.0))
    watch = float(stats.get("watch_pct", 0.0))
    defensive = float(stats.get("risk_off_pct", 0.0)) + float(stats.get("extreme_risk_pct", 0.0))
    regime_penalty = (
        max(0.0, 0.20 - normal) * 0.08
        + max(0.0, watch - 0.60) * 0.08
        + max(0.0, defensive - 0.35) * 0.10
    )
    return 1.05 * cagr + 0.08 * sharpe + 0.16 * calmar - 0.80 * mdd - 0.080 * annual_turnover - regime_penalty
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_v8_6_4_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_6_4_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_6_4_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.4 Directional Up/Down + Objective Condition Search")
    ...
    parser.add_argument("--direction-label-mode", choices=["simple", "vol_adj", "barrier"], default=None, help="방향성 라벨 방식")
    parser.add_argument("--direction-z-threshold", type=float, default=None, help="vol_adj 방향성 z threshold")
    parser.add_argument("--direction-barrier-z", type=float, default=None, help="barrier 방향성 z threshold")
    parser.add_argument("--keep-neutral-training", action="store_true", help="방향성 중립 샘플을 0으로 두고 학습. 기본은 중립 제외")
    parser.add_argument("--fixed-bucket-allocation", action="store_true", help="v8.6.2식 고정 regime bucket allocation 사용")
    parser.add_argument("--no-min-stock-floor", action="store_true", help="연속 배분에서 주식 하한도 제거")
    parser.add_argument("--risk-allocation-strength", type=float, default=None, help="risk score가 주식 비중을 낮추는 강도")
...
def main() -> None:
    ...
    if getattr(args, "direction_z_threshold", None) is not None:
        cfg.direction_z_threshold = float(args.direction_z_threshold)
    if getattr(args, "direction_barrier_z", None) is not None:
        cfg.direction_barrier_z = float(args.direction_barrier_z)
    if getattr(args, "keep_neutral_training", False):
        cfg.direction_ignore_neutral_training = False
    if getattr(args, "fixed_bucket_allocation", False):
        cfg.use_risk_score_allocation = False
    if getattr(args, "no_min_stock_floor", False):
        cfg.use_min_stock_floor = False
    if getattr(args, "risk_allocation_strength", None) is not None:
        cfg.risk_allocation_strength = float(args.risk_allocation_strength)
    ...
    print(f"    direction_label_mode: {cfg.direction_label_mode}")
    print(f"    direction_ignore_neutral_training: {cfg.direction_ignore_neutral_training}")
    print(f"    use_risk_score_allocation: {cfg.use_risk_score_allocation}")

    print("[3/5] Walk-forward Stage1 + volatility-adjusted Up/Down 방향성 예측")
    ...
    pred_path = result_dir / "qqq_xgb_v8_6_4_predictions.csv"
    summary_path = result_dir / "qqq_xgb_v8_6_4_summary.json"
    latest_path = result_dir / "qqq_xgb_v8_6_4_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_v8_6_4_stage1_feature_importance.csv"
    importance_up_path = result_dir / "qqq_xgb_v8_6_4_up_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_v8_6_4_downrisk_feature_importance.csv"
    importance_down_price_trend_path = result_dir / "qqq_xgb_v8_6_4_downrisk_price_trend_feature_importance.csv"
    importance_down_price_volume_path = result_dir / "qqq_xgb_v8_6_4_downrisk_price_volume_feature_importance.csv"
    importance_down_volatility_path = result_dir / "qqq_xgb_v8_6_4_downrisk_volatility_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_v8_6_4_condition_search.csv"
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_v8_6_4_{name}.csv"
