"""
XGBoost Two-stage ATR-aware Split-vol Risk-first Model
======================================================
...
- prob_up_high_vol / prob_down_high_vol을 이용해 QQQ/IEF/BIL 비중 조절
...
class Config:
    ...
    prob_ewma_span: int = 3

    # allocation
    base_stock_weight: float = 0.95
    down_high_vol_penalty: float = 0.78
    up_high_vol_penalty: float = 0.12
    min_stock_weight: float = 0.20
    max_stock_weight: float = 0.95
    base_cash_ratio_in_defensive: float = 0.15
    ...
    opt_turnover_weight: float = 0.10
    opt_target_avg_stock_weight: float = 0.65
    opt_low_stock_penalty_weight: float = 0.75

    result_dir: str = "results_xgb_two_stage_atr_split_vol"
...
def build_allocation_grid(cfg: Config) -> List[Dict[str, float]]:
    grid = []
    # 기본 48개 grid: 기존 V1의 65개 수준과 비슷한 계산량으로 제한
    base_list = [0.90, 0.95]
    down_penalty_list = [0.70, 0.85, 0.95]
    up_penalty_list = [0.05, 0.12]
    min_stock_list = [0.20, 0.30]
    cash_base_list = [0.15]
    cash_sens_list = [0.45, 0.60]

    idx = 0
    for base in base_list:
        for down_p in down_penalty_list:
            for up_p in up_penalty_list:
                for min_s in min_stock_list:
                    for cash_base in cash_base_list:
                        for cash_sens in cash_sens_list:
                            grid.append({
                                "name": f"xgb_split_cfg_{idx:04d}",
                                "base_stock_weight": base,
                                "down_high_vol_penalty": down_p,
                                "up_high_vol_penalty": up_p,
                                "min_stock_weight": min_s,
                                "max_stock_weight": 0.95,
                                "base_cash_ratio_in_defensive": cash_base,
                                "down_high_vol_cash_sensitivity": cash_sens,
                                "up_high_vol_cash_sensitivity": 0.05,
                            })
                            idx += 1
    return grid
...
def default_alloc_config(cfg: Config) -> Dict[str, float]:
    return {
        "name": "default_xgb_atr_split_vol",
        "base_stock_weight": cfg.base_stock_weight,
        "down_high_vol_penalty": cfg.down_high_vol_penalty,
        "up_high_vol_penalty": cfg.up_high_vol_penalty,
...
def allocate_from_probs(
    ...
    base_stock = cfg_dict["base_stock_weight"]
    stock = (
        base_stock
        - cfg_dict["down_high_vol_penalty"] * prob_down_high_vol
        - cfg_dict["up_high_vol_penalty"] * prob_up_high_vol
    )
    ...
    cash_ratio = (
        ...
        + cfg_dict["down_high_vol_cash_sensitivity"] * prob_down_high_vol
        + cfg_dict["up_high_vol_cash_sensitivity"] * prob_up_high_vol
    )
    ...
    if prev_weights is not None and smoothing > 0:
        ps, pb, pc = prev_weights
        stock = smoothing * ps + (1.0 - smoothing) * stock
        bond = smoothing * pb + (1.0 - smoothing) * bond
        cash = smoothing * pc + (1.0 - smoothing) * cash
...
def simulate_with_config(pred_df: pd.DataFrame, cfg_dict: Dict[str, float], cfg: Config) -> Dict[str, float]:
    ...
    stats["avg_stock_weight"] = np.nan  # scoring용 placeholder
    return stats
...
def score_config(stats: Dict[str, float], cfg: Config, avg_stock_weight: Optional[float] = None) -> float:
    ...
    score = cfg.opt_cagr_weight * cagr - cfg.opt_mdd_weight * mdd - cfg.opt_turnover_weight * turnover
    if avg_stock_weight is not None and avg_stock_weight < cfg.opt_target_avg_stock_weight:
        score -= cfg.opt_low_stock_penalty_weight * (cfg.opt_target_avg_stock_weight - avg_stock_weight)
    return float(score)
...
def apply_rolling_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        if cfg.use_rolling_allocation_optimization and i >= cfg.allocation_min_window and i % cfg.allocation_optimize_every_n_days == 0:
            ...
            # 속도 보호: grid가 너무 크면 일부만 사용하고 싶을 때 여기서 슬라이싱 가능
            for cand in grid:
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        pred_risk = "고변동" if prob_high >= 0.5 else "정상"
        if pred_risk == "정상":
            pred_split = "정상"
        else:
            pred_split = "상승고변동" if prob_up_given_high >= 0.5 else "하락고변동"
    ...
    if cfg.use_prob_ewma:
        ...
        # smoothing 후에도 합계가 prob_high와 맞도록 재정규화
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], cfg_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    summary = {
        ...
        "model_type": "xgb_two_stage_atr_aware_split_vol_risk_first",
        "config": asdict(cfg),
        "period": {
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost Two-stage ATR-aware Split-vol 결과 요약")
    print("==============================")
...
def main() -> None:
    ...
    pred_path = result_dir / "qqq_xgb_two_stage_atr_split_vol_predictions.csv"
    summary_path = result_dir / "qqq_xgb_two_stage_atr_split_vol_summary.json"
    latest_path = result_dir / "qqq_xgb_two_stage_atr_split_vol_latest.json"
