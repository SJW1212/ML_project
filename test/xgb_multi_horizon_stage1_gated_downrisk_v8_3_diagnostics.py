"""
XGBoost v8.3 - Objective Condition Search + Optimization Diagnostics for Stage1-Gated Down-risk Allocation
====================================================================
...
- v8.3 개선: H10 Down-risk 단독 사용을 기본값으로 채택
- v8.3 개선: no-trade band, RISK_OFF threshold, RISK_OFF 비중, 연속 조정 사용 여부를 validation 기반으로 비교 가능
- v8.3 개선: 조건 선택 근거를 condition_search CSV로 저장
- v8.3 개선: regime/probability bin/threshold/turnover/drawdown/feature optimization diagnostics CSV 추가 저장
- 비용/turnover를 고려해 10거래일 단위 리밸런싱 + 긴급 리밸런싱 제한
- 고변동 라벨 quantile 정책은 고정 또는 adaptive nested validation으로 선택 가능

실행 예시
    py xgb_multi_horizon_stage1_gated_downrisk_v8_3_diagnostics.py --speed-profile balanced --h10-down-only --condition-search
    py xgb_multi_horizon_stage1_gated_downrisk_v8_3_diagnostics.py --speed-profile fast --h10-down-only
    py xgb_multi_horizon_stage1_gated_downrisk_v8_3_diagnostics.py --speed-profile full --adaptive-label --h10-down-only
...
class Config:
    ...
    gate_high_vol_threshold: float = 0.60
    gate_riskoff_downrisk_threshold: float = 0.50
    gate_watch_downrisk_threshold: float = 0.65
    ...
    risk_off_bond_weight: float = 0.28
    risk_off_cash_weight: float = 0.14
    ...
    result_dir: str = "results_xgb_v8_3_diagnostics"
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_multi_horizon_stage1_gated_downrisk_v8_3_diagnostics",
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.3 Diagnostics 결과 요약")
    print("H10/H20 Stage1 + H10 Down-risk OVR + Objective Condition Allocation")
    print("==============================")
...
def objective_condition_score(stats: Dict[str, float], score_profile: str = "balanced") -> float:
    ...
    if score_profile == "cagr":
        return 1.35 * cagr + 0.04 * sharpe - 0.50 * mdd - 0.025 * annual_turnover
    if score_profile == "calmar":
        return 0.70 * cagr + 0.10 * sharpe + 0.35 * calmar - 0.90 * mdd - 0.035 * annual_turnover
    if score_profile == "turnover":
        return 0.85 * cagr + 0.08 * sharpe + 0.10 * calmar - 0.75 * mdd - 0.080 * annual_turnover

    # balanced: 수익률, 낙폭, 거래비용을 동시에 고려
    return 1.00 * cagr + 0.08 * sharpe + 0.12 * calmar - 0.75 * mdd - 0.050 * annual_turnover
...
def make_condition_candidate_configs(base_cfg: Config, grid_size: str = "standard") -> List[Tuple[str, Config]]:
    ...
    if grid_size == "compact":
        no_trade_list = [0.07, 0.09]
        cont_list = [False, True]
        riskoff_stock_list = [0.52, 0.58]
        threshold_pairs = [(0.60, 0.50), (0.58, 0.48)]
    elif grid_size == "wide":
        no_trade_list = [0.05, 0.07, 0.09, 0.11]
        cont_list = [False, True]
        riskoff_stock_list = [0.46, 0.48, 0.52, 0.58]
        threshold_pairs = [(0.65, 0.55), (0.62, 0.52), (0.60, 0.50), (0.58, 0.48), (0.55, 0.45)]
    else:
        no_trade_list = [0.07, 0.09, 0.11]
        cont_list = [False, True]
        riskoff_stock_list = [0.48, 0.52, 0.58]
        threshold_pairs = [(0.62, 0.52), (0.60, 0.50), (0.58, 0.48), (0.55, 0.45)]
...
def run_condition_search(
    ...
    for name, cand_cfg in candidates:
        ...
        for prefix, stats in [("select", select_stats), ("holdout", holdout_stats), ("full", full_stats)]:
            for key in [
                ...
                "regime_switch_ratio", "normal_pct", "watch_pct", "high_vol_pct", "risk_off_pct",
    ...
    return best_cfg, report_df, meta
...
def build_threshold_diagnostics(pred_df: pd.DataFrame) -> pd.DataFrame:
    ...
    diagnostics = [
        ...
        ("downrisk_ensemble", "actual_split_vol", "하락고변동", "prob_down_risk"),
    ]
    for h in [10, 20]:
        ...
        if f"prob_down_risk_h{h}" in pred_df.columns:
            diagnostics.append((f"downrisk_h{h}", f"actual_split_vol_h{h}", "하락고변동", f"prob_down_risk_h{h}"))
...
def build_optimization_diagnostics(pred_df: pd.DataFrame, summary: Dict[str, object]) -> Dict[str, pd.DataFrame]:
    ...
    dn_bins = build_probability_bin_analysis(pred_df, "prob_down_risk", "actual_split_vol", "하락고변동")
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_v8_3_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_3_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_3_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.3 Objective Condition Search + Optimization Diagnostics")
...
def main() -> None:
    ...
    pred_path = result_dir / "qqq_xgb_v8_3_predictions.csv"
    summary_path = result_dir / "qqq_xgb_v8_3_summary.json"
    latest_path = result_dir / "qqq_xgb_v8_3_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_v8_3_stage1_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_v8_3_downrisk_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_v8_3_condition_search.csv"
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_v8_3_{name}.csv"
