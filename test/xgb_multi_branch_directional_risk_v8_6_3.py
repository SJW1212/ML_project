"""
XGBoost v8.6.3 - Defensive Hybrid Risk + Auxiliary Direction Filter
====================================================================
...
class Config:
    ...
    # v8.6.3 Overall risk evaluation
    # - 전체 리스크는 고변동 위험과 하락위험을 함께 본다.
    # - Down-risk만 보면 방어 실패/과잉 방어를 전체 국면 관점에서 해석하기 어렵다.
    overall_risk_high_vol_weight: float = 0.55
    overall_risk_down_weight: float = 0.35
    overall_risk_down_minus_up_weight: float = 0.10
    pred_overall_risk_threshold: float = 0.50

    # v8.6.3 Direction model labels
    # future_return_h가 +threshold보다 크면 상승, -threshold보다 작으면 하락, 그 사이를 중립으로 둔다.
    direction_return_threshold: float = 0.005
    direction_decision_margin: float = 0.10
    direction_min_positive: int = 20

    # v8.6.3: 방향성 확률은 배분의 핵심 입력이 아니라 보조 필터로만 사용한다.
    # 이유: v8.6.2에서 up/down 방향성 ROC-AUC가 0.5 미만으로 약했기 때문이다.
    use_direction_aux_filter: bool = True
    direction_aux_margin: float = 0.12
    direction_aux_risk_add: float = 0.03
    direction_aux_risk_cut: float = 0.02
    allocation_tail_risk_weight: float = 0.75
    allocation_overall_risk_weight: float = 0.25

    # v8.6.3: 방향성 하락 모델과 별도로, 방어용 tail-risk 모델을 유지한다.
    # prob_down      = 단순 방향성 하락 확률
    # prob_down_risk = 큰 낙폭/하락고변동 확률
    use_tail_risk_model: bool = True
    ...
    pred_down_risk_threshold: float = 0.52
    ...
    emergency_combined_down_threshold: float = 0.60
    ...
    result_dir: str = "results_xgb_v8_6_3_defensive_hybrid"
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    """Walk-forward prediction with v8.6.3 defensive tail-risk + auxiliary direction filter.
    ...
    tail_imp_hist_by_branch: Dict[str, List[Dict[str, float]]] = {
        "price_trend": [],
        "price_volume": [],
        "volatility": [],
    }
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            for h in cfg.horizons:
                ...
                y_tail = (labels == "하락고변동").astype(int).values
                ...
                # v8.6.3: 방향성 하락 모델과 별도로 방어용 tail-risk 모델을 학습한다.
                # 이 모델은 단순 하락이 아니라 기존 Stage1 라벨의 '하락고변동'을 타깃으로 한다.
                tail_models: Dict[str, Optional[Pipeline]] = {"price_trend": None, "price_volume": None, "volatility": None}
                tail_available: Dict[str, bool] = {"price_trend": False, "price_volume": False, "volatility": False}
                if bool(getattr(cfg, "use_tail_risk_model", True)) and len(np.unique(y_tail)) == 2 and int(y_tail.sum()) >= int(getattr(cfg, "direction_min_positive", 20)):
                    for branch, cols in down_feature_sets.items():
                        if not cols:
                            continue
                        m_tail = make_xgb_downrisk(cfg, calc_scale_pos_weight(y_tail))
                        m_tail.fit(train_df[cols], y_tail)
                        tail_models[branch] = m_tail
                        tail_available[branch] = True
                        impt = extract_model_importance(m_tail, cols)
                        if impt:
                            tail_imp_hist_by_branch[branch].append(impt)
        ...
        prob_tail_branch_ens: Dict[str, float] = {"price_trend": 0.0, "price_volume": 0.0, "volatility": 0.0}
        ...
        for h in cfg.horizons:
            ...
            tail_branch_probs: Dict[str, float] = {}
            tail_models = m.get("tail_models", {})
            tail_available = m.get("tail_available", {})
            for branch, cols in down_feature_sets.items():
                tail_model = tail_models.get(branch) if isinstance(tail_models, dict) else None
                tail_ok = bool(tail_available.get(branch, False)) if isinstance(tail_available, dict) else False
                if tail_model is not None and tail_ok and cols:
                    tail_branch_probs[branch] = float(tail_model.predict_proba(row_df[cols])[0, 1])  # type: ignore[union-attr]
                else:
                    tail_branch_probs[branch] = branch_probs.get(branch, 0.0)
            if bool(cfg.use_multi_branch_downrisk):
                p_tail = (
                    branch_weights["price_trend"] * tail_branch_probs["price_trend"]
                    + branch_weights["price_volume"] * tail_branch_probs["price_volume"]
                    + branch_weights["volatility"] * tail_branch_probs["volatility"]
                    + branch_weights["high_vol"] * p_high
                )
            else:
                p_tail = tail_branch_probs["price_volume"]
            p_tail = float(np.clip(p_tail, 0.0, 1.0))
            ...
            out[f"prob_tail_price_trend_h{h}"] = tail_branch_probs["price_trend"]
            out[f"prob_tail_price_volume_h{h}"] = tail_branch_probs["price_volume"]
            out[f"prob_tail_volatility_h{h}"] = tail_branch_probs["volatility"]
            out[f"prob_down_h{h}"] = p_down
            out[f"prob_down_risk_h{h}"] = p_tail
            ...
            prob_tail_ens += dn_w.get(h, 0.0) * p_tail
            for branch in prob_down_branch_ens:
                prob_down_branch_ens[branch] += dn_w.get(h, 0.0) * branch_probs[branch]
                prob_tail_branch_ens[branch] += dn_w.get(h, 0.0) * tail_branch_probs[branch]
        ...
        prob_tail_ens = float(np.clip(prob_tail_ens, 0.0, 1.0))
        for branch in prob_down_branch_ens:
            prob_down_branch_ens[branch] = float(np.clip(prob_down_branch_ens[branch], 0.0, 1.0))
            prob_tail_branch_ens[branch] = float(np.clip(prob_tail_branch_ens[branch], 0.0, 1.0))
        prob_down_hv = float(np.clip(min(prob_high_ens, prob_tail_ens), 0.0, 1.0))
        prob_up_proxy = prob_up_ens
        prob_overall_risk = compute_overall_risk_prob(prob_high_ens, prob_tail_ens, cfg, prob_up=prob_up_ens)
        ...
        out.update({
            ...
            "prob_tail_price_trend": prob_tail_branch_ens["price_trend"],
            "prob_tail_price_volume": prob_tail_branch_ens["price_volume"],
            "prob_tail_volatility": prob_tail_branch_ens["volatility"],
            "prob_down": prob_down_ens,
            "prob_down_risk": prob_tail_ens,
            ...
            "pred_split_vol": "하락고변동" if (prob_high_ens >= cfg.pred_high_vol_threshold and prob_tail_ens >= cfg.pred_down_risk_threshold) else ("상승고변동" if prob_high_ens >= cfg.pred_high_vol_threshold else "정상"),
    ...
    if cfg.use_prob_ewma:
        ...
        for _tail_col in ["prob_tail_price_trend", "prob_tail_price_volume", "prob_tail_volatility"]:
            if _tail_col in pred_df.columns:
                pred_df[_tail_col] = pred_df[_tail_col].clip(0.0, 1.0)
        if bool(cfg.use_multi_branch_downrisk):
            pred_df["prob_down"] = (
        ...
        if bool(getattr(cfg, "use_tail_risk_model", True)) and all(c in pred_df.columns for c in ["prob_tail_price_trend", "prob_tail_price_volume", "prob_tail_volatility"]):
            pred_df["prob_down_risk"] = (
                branch_weights["price_trend"] * pred_df["prob_tail_price_trend"]
                + branch_weights["price_volume"] * pred_df["prob_tail_price_volume"]
                + branch_weights["volatility"] * pred_df["prob_tail_volatility"]
                + branch_weights["high_vol"] * pred_df["prob_high_vol"]
            ).clip(0.0, 1.0)
        else:
        ...
        pred_df["prob_overall_risk"] = compute_overall_risk_prob(
            pred_df["prob_high_vol"], pred_df["prob_down_risk"], cfg, prob_up=pred_df["prob_up"]
        )
        tail_w = float(getattr(cfg, "allocation_tail_risk_weight", 0.75))
        overall_w = float(getattr(cfg, "allocation_overall_risk_weight", 0.25))
        denom = max(1e-12, tail_w + overall_w)
        direction_gap = (pred_df["prob_down"] - pred_df["prob_up"]).clip(-1.0, 1.0)
        direction_adj = np.where(
            bool(getattr(cfg, "use_direction_aux_filter", True)) & (direction_gap >= float(getattr(cfg, "direction_aux_margin", 0.12))),
            float(getattr(cfg, "direction_aux_risk_add", 0.03)),
            np.where(
                bool(getattr(cfg, "use_direction_aux_filter", True)) & (direction_gap <= -float(getattr(cfg, "direction_aux_margin", 0.12))),
                -float(getattr(cfg, "direction_aux_risk_cut", 0.02)),
                0.0,
            ),
        )
        pred_df["allocation_risk_score"] = ((tail_w / denom) * pred_df["prob_down_risk"] + (overall_w / denom) * pred_df["prob_overall_risk"] + direction_adj).clip(0.0, 1.0)
    ...
    if "allocation_risk_score" not in pred_df.columns:
        if "prob_down_risk" not in pred_df.columns:
            pred_df["prob_down_risk"] = pred_df.get("prob_down", 0.0)
        pred_df["prob_overall_risk"] = compute_overall_risk_prob(
            pred_df["prob_high_vol"], pred_df["prob_down_risk"], cfg, prob_up=pred_df.get("prob_up", 0.0)
        )
        tail_w = float(getattr(cfg, "allocation_tail_risk_weight", 0.75))
        overall_w = float(getattr(cfg, "allocation_overall_risk_weight", 0.25))
        denom = max(1e-12, tail_w + overall_w)
        if "prob_up" in pred_df.columns and "prob_down" in pred_df.columns:
            direction_gap = (pred_df["prob_down"] - pred_df["prob_up"]).clip(-1.0, 1.0)
            direction_adj = np.where(
                bool(getattr(cfg, "use_direction_aux_filter", True)) & (direction_gap >= float(getattr(cfg, "direction_aux_margin", 0.12))),
                float(getattr(cfg, "direction_aux_risk_add", 0.03)),
    ...
    pred_df.attrs["tailrisk_feature_importance_mean"] = combine_weighted_importance(tail_imp_hist_by_branch, branch_weights)
    pred_df.attrs["tailrisk_price_trend_feature_importance_mean"] = mean_importance(tail_imp_hist_by_branch["price_trend"])
    pred_df.attrs["tailrisk_price_volume_feature_importance_mean"] = mean_importance(tail_imp_hist_by_branch["price_volume"])
    pred_df.attrs["tailrisk_volatility_feature_importance_mean"] = mean_importance(tail_imp_hist_by_branch["volatility"])
...
def simulate_gate_config(pred_df: pd.DataFrame, g: Dict[str, float], cfg: Config) -> Dict[str, float]:
    ...
    for _, row in pred_df.iterrows():
        w, _ = allocate_from_probs(float(row["prob_high_vol"]), float(row.get("allocation_risk_score", row.get("prob_down_risk", 0.0))), g, cfg, prev_w)
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        pdn = float(row.get("allocation_risk_score", row.get("prob_down_risk", row.get("prob_down", 0.0))))
        raw_emergency = (
...
def classification_metrics(pred_df: pd.DataFrame, cfg: Config) -> Dict[str, object]:
    ...
    for h in cfg.horizons:
        ...
        if f"actual_split_vol_h{h}" in pred_df.columns and f"prob_down_risk_h{h}" in pred_df.columns:
            y_tail = (pred_df[f"actual_split_vol_h{h}"] == "하락고변동").astype(int).values
            p_tail = pred_df[f"prob_down_risk_h{h}"].astype(float).clip(0.0, 1.0).values
            metrics[f"tailrisk_h{h}"] = binary_cls_metrics(y_tail, p_tail, cfg.pred_down_risk_threshold, "하락고변동")
    ...
    y_tail_primary = (pred_df["actual_split_vol"] == "하락고변동").astype(int).values if "actual_split_vol" in pred_df.columns else y_down_primary
    ...
    if "prob_down_risk" in pred_df.columns:
        p_tail_ens = pred_df["prob_down_risk"].astype(float).clip(0.0, 1.0).values
        metrics["tailrisk_ensemble_vs_primary"] = binary_cls_metrics(y_tail_primary, p_tail_ens, cfg.pred_down_risk_threshold, "하락고변동")
    if "allocation_risk_score" in pred_df.columns:
        p_alloc = pred_df["allocation_risk_score"].astype(float).clip(0.0, 1.0).values
        metrics["allocation_risk_vs_highvol_primary"] = binary_cls_metrics(y_primary, p_alloc, cfg.pred_overall_risk_threshold, "배분위험_by_고변동")
        metrics["allocation_risk_vs_tailrisk_primary"] = binary_cls_metrics(y_tail_primary, p_alloc, cfg.pred_down_risk_threshold, "배분위험_by_하락고변동")
    ...
    if "prob_overall_risk" in pred_df.columns:
        ...
        metrics["overall_risk_vs_tailrisk_primary"] = binary_cls_metrics(
            y_tail_primary,
            p_overall,
            cfg.pred_overall_risk_threshold,
            "전체위험_by_하락고변동",
    ...
    for branch, col in [
        ("price_trend", "prob_tail_price_trend"),
        ("price_volume", "prob_tail_price_volume"),
        ("volatility", "prob_tail_volatility"),
    ]:
        if col in pred_df.columns:
            p_branch = pred_df[col].astype(float).clip(0.0, 1.0).values
            metrics[f"tailrisk_{branch}_vs_primary"] = binary_cls_metrics(
                y_tail_primary,
                p_branch,
                cfg.pred_down_risk_threshold,
                f"하락고변동_{branch}",
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_multi_branch_directional_risk_v8_6_3_diagnostics",
        ...
        "feature_set": "defensive_hybrid_risk_aux_direction_features",
        ...
        "tailrisk_feature_importance_mean": pred_df.attrs.get("tailrisk_feature_importance_mean", {}),
        "tailrisk_price_trend_feature_importance_mean": pred_df.attrs.get("tailrisk_price_trend_feature_importance_mean", {}),
        "tailrisk_price_volume_feature_importance_mean": pred_df.attrs.get("tailrisk_price_volume_feature_importance_mean", {}),
        "tailrisk_volatility_feature_importance_mean": pred_df.attrs.get("tailrisk_volatility_feature_importance_mean", {}),
        ...
        "average_probabilities": {
            ...
            "avg_prob_down_risk": float(pred_df["prob_down_risk"].mean()) if "prob_down_risk" in pred_df.columns else 0.0,
            "avg_allocation_risk_score": float(pred_df["allocation_risk_score"].mean()) if "allocation_risk_score" in pred_df.columns else 0.0,
        ...
        "latest_prediction": {
            ...
            "prob_down_risk": round(float(latest.get("prob_down_risk", 0.0)) * 100, 2),
            "allocation_risk_score": round(float(latest.get("allocation_risk_score", latest.get("prob_down_risk", 0.0))) * 100, 2),
            ...
            "prob_tail_price_trend": round(float(latest.get("prob_tail_price_trend", 0.0)) * 100, 2),
            "prob_tail_price_volume": round(float(latest.get("prob_tail_price_volume", 0.0)) * 100, 2),
            "prob_tail_volatility": round(float(latest.get("prob_tail_volatility", 0.0)) * 100, 2),
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.6.3 Diagnostics 결과 요약")
    ...
    for key in [
        ...
        "overall_risk_vs_highvol_primary", "overall_risk_vs_down_primary", "overall_risk_vs_tailrisk_primary",
        "tailrisk_h10", "tailrisk_h20", "tailrisk_ensemble_vs_primary",
        "allocation_risk_vs_highvol_primary", "allocation_risk_vs_tailrisk_primary",
...
def objective_condition_score(stats: Dict[str, float], score_profile: str = "balanced") -> float:
    ...
    # v8.6.3 balanced: turnover를 유지하면서 방어 성능을 회복하기 위해 페널티를 유지한다.
...
def build_threshold_diagnostics(pred_df: pd.DataFrame) -> pd.DataFrame:
    ...
    diagnostics = [
        ...
        ("overall_risk_tail", "actual_split_vol", "하락고변동", "prob_overall_risk"),
        ("allocation_risk_highvol", "actual_risk", "고변동", "allocation_risk_score"),
        ("allocation_risk_tail", "actual_split_vol", "하락고변동", "allocation_risk_score"),
        ("tailrisk_ensemble", "actual_split_vol", "하락고변동", "prob_down_risk"),
    ...
    for h in [10, 20]:
        ...
        if f"prob_down_risk_h{h}" in pred_df.columns:
            diagnostics.append((f"tailrisk_h{h}", f"actual_split_vol_h{h}", "하락고변동", f"prob_down_risk_h{h}"))
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_v8_6_3_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_6_3_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_6_3_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.3 Defensive Hybrid Risk + Auxiliary Direction Filter")
...
def main() -> None:
    ...
    pred_path = result_dir / "qqq_xgb_v8_6_3_predictions.csv"
    summary_path = result_dir / "qqq_xgb_v8_6_3_summary.json"
    latest_path = result_dir / "qqq_xgb_v8_6_3_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_v8_6_3_stage1_feature_importance.csv"
    importance_up_path = result_dir / "qqq_xgb_v8_6_3_up_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_v8_6_3_downrisk_feature_importance.csv"
    importance_down_price_trend_path = result_dir / "qqq_xgb_v8_6_3_downrisk_price_trend_feature_importance.csv"
    importance_down_price_volume_path = result_dir / "qqq_xgb_v8_6_3_downrisk_price_volume_feature_importance.csv"
    importance_down_volatility_path = result_dir / "qqq_xgb_v8_6_3_downrisk_volatility_feature_importance.csv"
    importance_tail_path = result_dir / "qqq_xgb_v8_6_3_tailrisk_feature_importance.csv"
    importance_tail_price_trend_path = result_dir / "qqq_xgb_v8_6_3_tailrisk_price_trend_feature_importance.csv"
    importance_tail_price_volume_path = result_dir / "qqq_xgb_v8_6_3_tailrisk_price_volume_feature_importance.csv"
    importance_tail_volatility_path = result_dir / "qqq_xgb_v8_6_3_tailrisk_volatility_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_v8_6_3_condition_search.csv"
    ...
    pd.Series(summary.get("tailrisk_feature_importance_mean", {}), name="importance").to_csv(importance_tail_path, encoding="utf-8-sig")
    pd.Series(summary.get("tailrisk_price_trend_feature_importance_mean", {}), name="importance").to_csv(importance_tail_price_trend_path, encoding="utf-8-sig")
    pd.Series(summary.get("tailrisk_price_volume_feature_importance_mean", {}), name="importance").to_csv(importance_tail_price_volume_path, encoding="utf-8-sig")
    pd.Series(summary.get("tailrisk_volatility_feature_importance_mean", {}), name="importance").to_csv(importance_tail_volatility_path, encoding="utf-8-sig")
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_v8_6_3_{name}.csv"
    ...
    print(f"- {importance_tail_path}")
    print(f"- {importance_tail_price_trend_path}")
    print(f"- {importance_tail_price_volume_path}")
    print(f"- {importance_tail_volatility_path}")
    if condition_report_df is not None:
