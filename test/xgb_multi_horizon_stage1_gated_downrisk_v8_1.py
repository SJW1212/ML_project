"""
XGBoost v8.1 - Multi-Horizon Stage1-Gated Down-risk Volatility Timing
====================================================================
...
- v8.1 개선: H20 Down-risk 비중 축소, RISK_OFF 조건 강화, 정상/주의 비중 상향
- v8.1 개선: no-trade band 확대, emergency cooldown 추가, 연속 조정폭 축소
- 비용/turnover를 고려해 10거래일 단위 리밸런싱 + 긴급 리밸런싱 제한
- 고변동 라벨 quantile 정책은 고정 또는 adaptive nested validation으로 선택 가능

실행 예시
    py xgb_multi_horizon_stage1_gated_downrisk_v8_1.py --speed-profile balanced
    py xgb_multi_horizon_stage1_gated_downrisk_v8_1.py --speed-profile fast
    py xgb_multi_horizon_stage1_gated_downrisk_v8_1.py --speed-profile full --adaptive-label
...
class Config:
    ...
    down_risk_weight_h10: float = 0.90
    down_risk_weight_h20: float = 0.10
    ...
    result_dir: str = "results_xgb_multi_horizon_stage1_gated_downrisk_v8_1"
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.1 Multi-Horizon Stage1-Gated Down-risk 결과 요약")
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_v8_1_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_1_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_1_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.1 Multi-Horizon Stage1-Gated Down-risk Allocation")
...
def main() -> None:
    ...
    pred_path = result_dir / "qqq_xgb_v8_1_predictions.csv"
    summary_path = result_dir / "qqq_xgb_v8_1_summary.json"
    latest_path = result_dir / "qqq_xgb_v8_1_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_v8_1_stage1_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_v8_1_downrisk_feature_importance.csv"
