...
class Config:
    ...
    result_dir: str = "results_xgb_direction_strength_specialist_v8_6_9"
    ...
    # v8.6.9 Direction-Strength Specialist
    ...
    up_strength_bonus_threshold_1: float = 0.55
    up_strength_bonus_threshold_2: float = 0.62
    bear_down_strength_cut_threshold_1: float = 0.45
    bear_down_strength_cut_threshold_2: float = 0.55
    direction_strength_max_stock_bonus: float = 0.05
    direction_strength_max_stock_cut: float = 0.08
...
def apply_direction_strength_specialist_policy(
    ...
    """
    v8.6.9 Direction-Strength Specialist.
    - 5-class argmax는 사용하지 않는다.
    - low-risk 구간에서 UP_STRENGTHENING 확률만 작은 bonus로 사용한다.
    - high-vol/bear-stress 구간에서 bear specialist의 DOWN_STRENGTHENING 확률만 방어 penalty로 사용한다.
    """
    ph = _row_float(row, "prob_high_vol")
    pdn = _row_float(row, "prob_down", _row_float(row, "prob_down_risk"))
    ...
    # Upside head: 상승 강화 확률은 저위험/비약세 구간에서만 작은 보너스로 사용.
    if regime == "NORMAL" and ph < 0.45 and trend_state != "BEAR":
        if pus >= float(getattr(cfg, "up_strength_bonus_threshold_1", 0.55)):
            bonus += 0.02
        if ph < 0.35 and pus >= float(getattr(cfg, "up_strength_bonus_threshold_2", 0.62)) and puw < 0.55:
            bonus += 0.02
        if ph < 0.28 and pus >= 0.70 and trend_score >= 4:
            bonus += 0.01

    # Bear specialist: Stage1이 위험을 감지한 뒤에만 하락 강화 확률을 penalty로 사용.
    if ph >= 0.55:
        if pbd >= float(getattr(cfg, "bear_down_strength_cut_threshold_1", 0.45)):
            cut += 0.03
        if ph >= 0.65 and pbd >= float(getattr(cfg, "bear_down_strength_cut_threshold_2", 0.55)):
            cut += 0.04
    if regime == "RISK_OFF" and pbd >= 0.40:
        cut += 0.01

    # DOWN_WEAKENING은 precision이 낮았으므로, 방어 완화 신호로 크게 쓰지 않는다.
    if ph >= 0.55 and pbdw >= 0.60 and pbd < 0.35:
        cut = max(0.0, cut - 0.01)

    bonus = float(np.clip(bonus, 0.0, getattr(cfg, "direction_strength_max_stock_bonus", 0.05)))
    cut = float(np.clip(cut, 0.0, getattr(cfg, "direction_strength_max_stock_cut", 0.08)))
    stock = float(np.clip(stock + bonus - cut, 0.35, 1.00))
    w = _redistribute_after_stock_change(stock, base_w)
    meta = {
        "policy_overlay": float(w[0] - base_w[0]),
        "mid_trend_score": trend_score,
        "mid_trend_state": trend_state,
        "defensive_risk_score": float(np.clip(0.70 * ph + 0.30 * max(pdn, pbd), 0.0, 1.0)),
        "direction_strength_score": float(pus - pbd),
        "up_strength_bonus": float(bonus),
        "bear_strength_cut": float(cut),
        "policy_note": "direction_strength_specialist_probability_overlay",
    }
    return w, meta
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_direction_strength_specialist_v8_6_9_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_direction_strength_specialist_v8_6_9_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_direction_strength_specialist_v8_6_9_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_direction_strength_specialist_v8_6_9_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.9 Direction-Strength Specialist + Objective Condition Search")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.9 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.9 direction-strength specialist 모델 학습/사용 비활성화")
...
def main() -> None:
    ...
    cfg.result_dir = f"results_xgb_direction_strength_specialist_v8_6_9_{cfg.policy_mode}"
    ...
    pred_path = result_dir / "qqq_xgb_direction_strength_specialist_v8_6_9_predictions.csv"
    summary_path = result_dir / "qqq_xgb_direction_strength_specialist_v8_6_9_summary.json"
    latest_path = result_dir / "qqq_xgb_direction_strength_specialist_v8_6_9_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_direction_strength_specialist_v8_6_9_stage1_feature_importance.csv"
    importance_up_path = result_dir / "qqq_xgb_direction_strength_specialist_v8_6_9_up_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_direction_strength_specialist_v8_6_9_downrisk_feature_importance.csv"
    importance_down_price_trend_path = result_dir / "qqq_xgb_direction_strength_specialist_v8_6_9_downrisk_price_trend_feature_importance.csv"
    importance_down_price_volume_path = result_dir / "qqq_xgb_direction_strength_specialist_v8_6_9_downrisk_price_volume_feature_importance.csv"
    importance_down_volatility_path = result_dir / "qqq_xgb_direction_strength_specialist_v8_6_9_downrisk_volatility_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_direction_strength_specialist_v8_6_9_condition_search.csv"
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_direction_strength_specialist_v8_6_9_{name}.csv"
