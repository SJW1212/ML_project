"""
XGBoost v8.6.40b - Recency-Weighted Normal-Unrestricted
====================================================================

목적
- QQQ/IEF/BIL 동적 자산배분 전략
- v8.6.40b: 5D+10D+high-vol ShortMidConfirm 보조 신호 추가
...
실행 예시
    py xgb_recency_weighted_v8_6_40b.py --speed-profile balanced --h10-down-only
    py xgb_recency_weighted_v8_6_40b.py --speed-profile fast --h10-down-only
    py xgb_recency_weighted_v8_6_40b.py --speed-profile full --adaptive-label --h10-down-only
...
class Config:
    ...
    result_dir: str = "results_xgb_recency_weighted_v8_6_40b"
    ...
    # v8.6.40b base allocation: 기본 상태는 high-vol 확률 기반 연속/계단형 비중으로 산출한다.
    ...
    # v8.6.40b Normal-Unrestricted Base
    ...
    # v8.6.40b execution: Tier3/Full은 schedule/no-trade-band에 묶이지 않고 즉시 목표 비중으로 점프한다.
    ...
    # v8.6.40b Volatility-Base Strong-Override Multi-Horizon Upside Strength Trigger
    ...
    # v8.6.40b: prob_bear_down_strengthening 제거. 하락 강화도 상승 강화와 동일하게 5D/10D/20D term structure로 계산한다.
    ...
    # v8.6.40b Recency Sample Weight
    ...
    up_strength_bonus_threshold_2: float = 0.38  # v8.6.40b: Tier2 활성화 기본값
    ...
    up_strength_pred_threshold_5d: float = 0.99  # v8.6.40b: 5D 단독 trigger 비활성화
    ...
    # v8.6.40b: 5D는 allocation trigger에서 제외하고, 10D는 보조 확인, 20D는 핵심 조건으로 사용한다.
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            if bool(getattr(cfg, "use_direction_strength_specialist", True)):
                try:
                    ...
                    # v8.6.40b: 별도 bear-filter specialist 제거.
        ...
        if bool(getattr(cfg, "use_direction_strength_specialist", True)):
            ...
            bear_strength_probs = {name: 0.0 for name in DIRECTION_STRENGTH_LABELS}  # removed in v8.6.40b
        else:
        ...
        out.update({
            ...
            **{f"prob_up_weakening_{int(sh)}d": 0.0 for sh in multi_strength_horizons},
            # v8.6.40b: 하락 강화도 상승 강화와 동일한 horizon별 term structure로 산출한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_recency_weighted_v8_6_40b_diagnostics",
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_recency_weighted_v8_6_40b_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_recency_weighted_v8_6_40b_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_recency_weighted_v8_6_40b_full_adaptive_label"
    else:
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_recency_weighted_v8_6_40b_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_recency_weighted_v8_6_40b_summary.json"
    ...
    cfg.result_dir = f"results_xgb_recency_weighted_v8_6_40b_{cfg.policy_mode}"
    ...
    file_prefix = f"{safe_ticker}_xgb_recency_weighted_v8_6_40b"
