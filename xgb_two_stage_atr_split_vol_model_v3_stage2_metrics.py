"""
XGBoost Two-stage ATR-aware Split-vol Risk-first Model v3 - Stage2 Metrics Fixed
======================================================
...
- 평가 지표는 Stage1 / Stage2 / Final Cascade로 분리
...
class Config:
    ...
    result_dir: str = "results_xgb_two_stage_atr_split_vol_v3_stage2_metrics"
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    # 핵심 수정:
    # - 1단계에서 정상으로 판단되면 최종 3상태 출력은 반드시 "정상"이다.
    # - 2단계 상승/하락 판단은 1단계에서 고변동으로 판단된 경우에만 hard label로 사용한다.
    # - 2단계 자체 성능 평가는 별도 함수에서 "실제 고변동 구간"만 대상으로 계산한다.
    ...
    # smoothing 이후 joint probability로부터 conditional probability를 다시 계산한다.
    # P(up | high-vol) = P(up high-vol) / P(high-vol)
    # P(down | high-vol) = P(down high-vol) / P(high-vol)
    hv_denom = pred_df["prob_high_vol"].replace(0, np.nan)
    pred_df["prob_up_given_high_vol"] = (pred_df["prob_up_high_vol"] / hv_denom).fillna(0.5).clip(0.0, 1.0)
    pred_df["prob_down_given_high_vol"] = (1.0 - pred_df["prob_up_given_high_vol"]).clip(0.0, 1.0)

    pred_df["pred_stage2_direction"] = np.where(
        pred_df["prob_down_given_high_vol"] >= pred_df["prob_up_given_high_vol"],
        "하락고변동",
        "상승고변동",
    )

    def _final_split_label(row: pd.Series) -> str:
        # Cascade hard label: 정상은 1단계에서 이미 판정한다.
        if row["pred_risk"] != "고변동":
            return "정상"

        # 1단계가 고변동일 때만 2단계 방향을 사용한다.
        return row["pred_stage2_direction"]
...
def classification_metrics(pred_df: pd.DataFrame) -> Dict[str, object]:
    ...
    2. stage2_direction_classification_on_true_high_vol
       - 실제 고변동 구간만 대상
       - 상승고변동 / 하락고변동 이진 분류
       - 정상 클래스는 포함하지 않음
    ...
    p_risk = pred_df["prob_high_vol"].astype(float).values
    ...
    # --------------------------------------------------------
    # Stage 2: 실제 고변동 구간만 대상으로 상승/하락 방향 평가
    # --------------------------------------------------------
    direction_labels = ["상승고변동", "하락고변동"]
    true_hv_mask = pred_df["actual_split_vol"].isin(direction_labels)
    hv_df = pred_df.loc[true_hv_mask].copy()

    if len(hv_df) > 0:
        y_stage2_true = hv_df["actual_split_vol"].values

        # 조건부 확률을 사용한다. 즉, P(하락고변동 | 고변동), P(상승고변동 | 고변동)
        if "prob_down_given_high_vol" in hv_df.columns and "prob_up_given_high_vol" in hv_df.columns:
            p_down_cond = hv_df["prob_down_given_high_vol"].astype(float).clip(0.0, 1.0).values
            p_up_cond = hv_df["prob_up_given_high_vol"].astype(float).clip(0.0, 1.0).values
        else:
            denom = hv_df["prob_high_vol"].replace(0, np.nan)
            p_up_cond = (hv_df["prob_up_high_vol"] / denom).fillna(0.5).clip(0.0, 1.0).values
            p_down_cond = (1.0 - p_up_cond).clip(0.0, 1.0)

        y_stage2_pred = np.where(
            p_down_cond >= p_up_cond,
            "하락고변동",
            "상승고변동",
        )
        ...
        stage2 = {
            "rows": int(len(hv_df)),
            "evaluation_scope": "actual high-vol rows only",
            "accuracy": float(accuracy_score(y_stage2_true, y_stage2_pred)),
            ...
            "label_support": {lab: int((hv_df["actual_split_vol"] == lab).sum()) for lab in direction_labels},
            "report": stage2_report,
            "per_class": {},
        }
        for lab in direction_labels:
        ...
        y_down_stage2 = (y_stage2_true == "하락고변동").astype(int)
        y_up_stage2 = (y_stage2_true == "상승고변동").astype(int)

        if len(np.unique(y_down_stage2)) == 2:
            stage2["down_high_vol_roc_auc"] = float(roc_auc_score(y_down_stage2, p_down_cond))
            stage2["down_high_vol_pr_auc"] = float(average_precision_score(y_down_stage2, p_down_cond))
        else:
            stage2["down_high_vol_roc_auc"] = None
            stage2["down_high_vol_pr_auc"] = None

        if len(np.unique(y_up_stage2)) == 2:
            stage2["up_high_vol_roc_auc"] = float(roc_auc_score(y_up_stage2, p_up_cond))
            stage2["up_high_vol_pr_auc"] = float(average_precision_score(y_up_stage2, p_up_cond))
        else:
            stage2["up_high_vol_roc_auc"] = None
            stage2["up_high_vol_pr_auc"] = None

        # Stage2 threshold diagnostics: 조건부 하락 확률 기준
        stage2_diags = []
        for t in [0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70]:
            pred_down = (p_down_cond >= t).astype(int)
            stage2_diags.append({
                "threshold": float(t),
                "basis": "prob_down_given_high_vol",
                "down_high_vol_precision": float(precision_score(y_down_stage2, pred_down, zero_division=0)),
                "down_high_vol_recall": float(recall_score(y_down_stage2, pred_down, zero_division=0)),
                "down_high_vol_f1": float(f1_score(y_down_stage2, pred_down, zero_division=0)),
        ...
        stage2 = {
            ...
            "accuracy": None,
            "macro_f1": None,
            "label_support": {lab: 0 for lab in direction_labels},
            "report": {},
            "per_class": {},
            "down_high_vol_roc_auc": None,
            "down_high_vol_pr_auc": None,
            "up_high_vol_roc_auc": None,
            "up_high_vol_pr_auc": None,
        }
        stage2_diags = []

    # --------------------------------------------------------
    # Final Cascade 3-state: 전체 구간에서 최종 출력 평가
    # --------------------------------------------------------
    ...
    # Final cascade threshold diagnostics: 전체 구간에서 joint P(하락고변동) 기준
    final_diags = []
    for t in [0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50]:
        pred_down = (pred_df["prob_down_high_vol"].values >= t).astype(int)
        final_diags.append({
            "threshold": float(t),
            "basis": "prob_down_high_vol",
            "down_high_vol_precision": float(precision_score(y_down_final, pred_down, zero_division=0)),
            "down_high_vol_recall": float(recall_score(y_down_final, pred_down, zero_division=0)),
            "down_high_vol_f1": float(f1_score(y_down_final, pred_down, zero_division=0)),
            "pred_down_high_vol_ratio": float(pred_down.mean()),
        })
    ...
    return {
        ...
        # 기존 시각화 코드와의 호환용 alias.
        # 의미는 더 이상 "2단계 성능"이 아니라 "최종 cascade 3상태 출력 성능"이다.
        "split_vol_classification": final_cascade,

        # 새 기준: 실제 고변동 subset에서 조건부 확률 기준 threshold 진단.
        "stage2_down_high_vol_threshold_diagnostics_on_true_high_vol": stage2_diags,

        # 기존 시각화 코드와의 호환용 alias. 이제 stage2 subset 기준이다.
        "down_high_vol_threshold_diagnostics": stage2_diags,

        # 전체 구간 joint probability 기준 보조 진단.
        "final_down_high_vol_threshold_diagnostics": final_diags,
    }
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], cfg_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    summary = {
        ...
        "model_type": "xgb_two_stage_atr_aware_split_vol_risk_first_v3_stage2_metrics",
        "config": asdict(cfg),
        "period": {
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    st2 = summary["stage2_direction_classification_on_true_high_vol"]
    ...
    print("\n==============================")
    print("XGBoost Two-stage ATR-aware Split-vol v3 결과 요약")
    print("Stage1 / Stage2 / Final Cascade 지표 분리")
    print("==============================")
    ...
    print("\n[2단계 Direction 분류 성능 - 실제 고변동 구간만 평가]")
    print(f"Rows: {st2['rows']}")
    print(f"Accuracy: {st2['accuracy']}")
    print(f"Macro F1: {st2['macro_f1']}")
    ...
    if st2.get("per_class"):
        for lab, vals in st2["per_class"].items():
            print(
                f"  - {lab}: "
                f"P={vals['precision']:.6f}, "
                f"R={vals['recall']:.6f}, "
                f"F1={vals['f1']:.6f}, "
