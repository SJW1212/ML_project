...
class Config:
    ...
    up_strength_allocation_score_minimum: float = 0.40
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    ...
    """
    ph = float(np.clip(prob_high_vol, 0.0, 1.0))
    # [v8.6.40 P0] Remove raw down-risk influence from allocation.
...
def apply_direction_strength_specialist_policy(
    ...
    """
    ph = _row_float(row, "prob_high_vol")
    # v8.6.40b: weak 확률(prob_up/prob_down/prob_bear_down_strengthening)은 allocation에서 제거한다.
    ...
    # v8.6.40b: 강한 상승 override는 regime bucket보다 실제 high-vol cap을 우선한다.
    ...
    if True:
        ...
        # [v8.6.40b P1] Ignore weak UpStrength allocation signals below validated score floor.
        ...
        # v8.6.40b Normal-Unrestricted policy.
        ...
        # v8.6.40b ShortMidConfirm optional allocation actions.
    ...
    # v8.6.40b: bear/down 약한 확률 기반 cut은 제거한다.
...
# ============================================================
# v8.6.40b Separate PortfolioPolicyModel
# ============================================================
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        # [v8.6.40 P0/P2] Conflict diagnostics. v8.6.40a/b record only; v8.6.40c may hard-cap extreme high-vol.
...
def classification_metrics(pred_df: pd.DataFrame, cfg: Config) -> Dict[str, object]:
    ...
    # v8.6.40b: prob_up/prob_down은 약한 출력으로 판정되어 summary 핵심 성능에서 제외한다.
    ...
    if "actual_direction_strength" in pred_df.columns:
        ...
        # v8.6.40b: prob_bear_down_strengthening은 allocation/summary 성능에서 제외한다.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.40b Normal-Unrestricted Volatility-Base Multi-Horizon Upside Strength Trigger")
    ...
    parser.add_argument("--transaction-cost-rate", type=float, default=None, help="거래비용률. 예: 0.001 = 10bps")
    ...
    parser.add_argument("--rebalance-every", type=int, default=None, help="리밸런싱 주기 직접 지정. v8.6.40b 기본 5거래일")
    parser.add_argument("--emergency-cooldown", type=int, default=None, help="긴급 리밸런싱 cooldown 직접 지정. v8.6.40b 기본 5거래일")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist", "portfolio_model"], default=None, help="v8.6.40b policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.40b up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.40b 기본 ON인 stale offensive decay 비활성화")
    ...
    parser.add_argument("--no-horizon-train-window", action="store_true", help="v8.6.40b horizon별 train_rows = horizon*multiplier 모드 비활성화")
    ...
    parser.add_argument("--disable-recency-weight", action="store_true", help="v8.6.40b 최근 데이터 sample_weight 비활성화")
    ...
    parser.add_argument("--disable-normal-unrestricted", action="store_true", help="v8.6.40b 정상 상태 비중 제한 완화 정책 비활성화")
    ...
    parser.add_argument("--disable-vol-base-allocation", action="store_true", help="v8.6.40b high-vol 확률 기반 기본 비중을 끄고 regime bucket 기본 비중 사용")
    ...
    parser.add_argument("--disable-tier2", action="store_true", help="Tier2 비활성화. v8.6.40b 기본값은 활성화")
...
def drop_weak_probability_output_columns(pred_df: pd.DataFrame) -> pd.DataFrame:
    """
    v8.6.40b external-output filter.
...
def main() -> None:
    ...
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.40b: Volatility-Base Strong-Override Upside Strength Trigger
        ...
        # v8.6.40b: 기본 상태는 prob_high_vol 기반 포트폴리오, 강한 신호만 즉시 override한다.
        cfg.use_vol_probability_base_allocation = True
        # v8.6.40b: 정상/저변동 상태 기본 비중 상향.
        ...
        # v8.6.40b: Tier2를 기본 활성화한다. 기존 실패 구간 재검증을 위해 CLI로 조건/비중 조정 가능.
        ...
        # v8.6.40b: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
    ...
    if args.transaction_cost_rate is not None:
        cfg.transaction_cost_rate = float(args.transaction_cost_rate)
    ...
    if bool(getattr(cfg, "enable_v8640_diagnostics", True)):
        try:
            auto_review_text = v8640_build_auto_review(pred_df, summary, ticker=str(cfg.target_ticker), version="v8.6.40b")
