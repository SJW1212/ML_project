"""
XGBoost v8.7.0 - Tier1-Boost CAGR-Tuned Recency Weighted Strength Policy
====================================================================

목적
- QQQ/IEF/BIL 동적 자산배분 전략
- v8.7.0: Tier3/Full stock 완화 실험, CUSTOM_LITE, CUSTOM 내부 성과 분해 리포트 추가
- v8.7.0: 5D+10D+high-vol ShortMidConfirm 보조 신호 추가
...
실행 예시
    py xgb_trend_participation_v8_7_0.py --speed-profile balanced --h10-down-only
    py xgb_trend_participation_v8_7_0.py --speed-profile fast --h10-down-only
    py xgb_trend_participation_v8_7_0.py --speed-profile full --adaptive-label --h10-down-only
...
class Config:
    ...
    result_dir: str = "results_xgb_trend_participation_v8_7_0"
    ...
    # v8.7.0 base allocation: 기본 상태는 high-vol 확률 기반 연속/계단형 비중으로 산출한다.
    ...
    # v8.7.0 Normal-Unrestricted Base
    ...
    normal_base_boost_enabled: bool = True
    normal_base_stock_lt_25: float = 0.86
    normal_base_stock_lt_35: float = 0.82
    normal_base_stock_lt_50: float = 0.74
    normal_full_enabled: bool = True
    normal_full_stock_weight: float = 1.00
    normal_full_high_vol_threshold: float = 0.25
    normal_full_down_strength_threshold: float = 0.35
    normal_full_up_score_threshold: float = 0.38
    normal_full_p20_threshold: float = 0.34
    normal_full_allow_short_mid: bool = True
    normal_full_force_rebalance: bool = False

    # v8.7.0 execution: Tier3/Full은 schedule/no-trade-band에 묶이지 않고 즉시 목표 비중으로 점프한다.
    ...
    # v8.7.0 Separate PortfolioPolicyModel
    ...
    # v8.7.0 Volatility-Base Strong-Override Multi-Horizon Upside Strength Trigger
    ...
    # v8.7.0: prob_bear_down_strengthening 제거. 하락 강화도 상승 강화와 동일하게 5D/10D/20D term structure로 계산한다.
    ...
    recency_weight_mode: str = "exponential"  # exponential, linear
    recency_half_life_5d: int = 126
    recency_half_life_10d: int = 252
    recency_half_life_20d: int = 504
    recency_weight_min: float = 0.25
    recency_weight_max: float = 4.00
    recency_weight_normalize: bool = True
    recency_apply_stage1: bool = True
    recency_apply_direction_binary: bool = True
    recency_apply_downrisk: bool = True
    recency_apply_direction_strength: bool = True
    ...
    up_strength_bonus_threshold_2: float = 0.38  # v8.7.0: Tier2 활성화 기본값
    ...
    up_strength_pred_threshold_5d: float = 0.99  # v8.7.0: 5D 단독 trigger 비활성화
    ...
    # v8.7.0: 5D는 allocation trigger에서 제외하고, 10D는 보조 확인, 20D는 핵심 조건으로 사용한다.
    ...
    short_mid_confirm_mode: str = "base_upgrade"  # diagnostic, tier1_upgrade, tier2_add, tier2_replace, base_upgrade
    ...
    # v8.7.0 CAGR tuning / CUSTOM_LITE
    # - NORMAL 전체 비중 상향이 아니라 NORMAL 내부의 약한 strength-confirm 구간만 선별적으로 상향한다.
    # - 기본값은 OFF. --enable-custom-lite로 단독 실험한다.
    enable_custom_lite: bool = False
    custom_lite_high_vol_threshold: float = 0.35
    custom_lite_up_score_threshold: float = 0.18
    custom_lite_p20_threshold: float = 0.00
    custom_lite_down_strength_threshold: float = 0.45
    custom_lite_stock_weight: float = 0.86
    custom_lite_require_p20: bool = False
    custom_lite_exclude_bear_trend: bool = False
    custom_lite_force_rebalance: bool = False
    # v8.7.0 redesigned CUSTOM_LITE: NORMAL/no-offensive 내부에서 약한 상승 강도만 선별
    custom_lite_require_no_offensive: bool = True
    custom_lite_allow_tier1: bool = False
    custom_lite_apply_after_offensive: bool = True

    # v8.7.0 Tier1-only boost
    # - 최근 진단에서 Tier1-only 구간이 Tier3와 유사하거나 더 높은 평균수익률을 보였기 때문에
    #   full stock까지 올리지는 않고 최소 주식비중만 86~88%로 보정한다.
    enable_tier1_boost: bool = False
    tier1_boost_high_vol_threshold: float = 0.70
    tier1_boost_down_strength_threshold: float = 0.32
    tier1_boost_stock_weight: float = 0.86
    tier1_boost_require_no_tier2: bool = True
    tier1_boost_exclude_full_tier3: bool = True
    tier1_boost_force_rebalance: bool = False

    # v8.7.0 Market Trend Participation Overlay
    # - 시장 Sharpe/추세 일관성이 높은 구간에서는 vol-base 방어 비중을 완화해 QQQ B&H drag를 줄인다.
    # - 기본 ON. 기존 구조만 비교하려면 --disable-trend-participation 사용.
    ...
    # v8.7.0 Recovery Re-risk Overlay
    # - 2020년형 V자 반등에서 위험 확률이 늦게 내려가며 재진입이 지연되는 문제를 완화한다.
    ...
    # v8.7.0 Soft Drawdown Guard
    # - 강한 stop-loss가 아니라 CUSTOM/NORMAL의 중간 drawdown을 완화하는 약한 guard.
    # - 기본 OFF. CAGR 훼손 가능성이 있으므로 별도 실험한다.
    ...
    drawdown_guard_relax_in_trend: bool = True
    drawdown_guard_relax_in_recovery: bool = True
    drawdown_guard_force_rebalance: bool = False

    # v8.7.0 TierWeightOptimizer
...
def _get_recency_half_life(cfg: Config, horizon: int) -> int:
    h = int(horizon)
    if h == 5:
        return int(getattr(cfg, "recency_half_life_5d", 126))
    if h == 10:
        return int(getattr(cfg, "recency_half_life_10d", 252))
    if h == 20:
        return int(getattr(cfg, "recency_half_life_20d", 504))
    return int(getattr(cfg, "recency_half_life_20d", 504))


def make_recency_sample_weight(n: int, cfg: Config, horizon: int) -> np.ndarray:
    """Create time-decay sample weights for a train window.

    The latest row receives the largest weight. We normalize the mean to 1.0
    so the effective loss scale stays comparable to the non-weighted baseline.
    """
    n = int(n)
    if n <= 0 or not bool(getattr(cfg, "use_recency_sample_weight", True)):
        return np.ones(max(n, 0), dtype=float)

    mode = str(getattr(cfg, "recency_weight_mode", "exponential")).lower()
    distance = (n - 1) - np.arange(n, dtype=float)
    half_life = max(1.0, float(_get_recency_half_life(cfg, int(horizon))))

    if mode == "linear":
        # Oldest sample weight approaches 0.5 when distance ~= half_life, bounded by clipping below.
        w = 1.0 / (1.0 + distance / half_life)
    else:
        # Exponential half-life. distance == half_life => weight is halved.
        w = np.power(0.5, distance / half_life)

    w = np.asarray(w, dtype=float)
    w = np.clip(w, float(getattr(cfg, "recency_weight_min", 0.25)), float(getattr(cfg, "recency_weight_max", 4.0)))
    if bool(getattr(cfg, "recency_weight_normalize", True)) and np.nanmean(w) > 0:
        w = w / float(np.nanmean(w))
    return w.astype(float)


def combine_sample_weights(*weights: Optional[np.ndarray]) -> Optional[np.ndarray]:
    valid = [np.asarray(w, dtype=float) for w in weights if w is not None]
    if not valid:
        return None
    out = np.ones_like(valid[0], dtype=float)
    for w in valid:
        if len(w) != len(out):
            raise ValueError("sample_weight length mismatch")
        out = out * w
    m = float(np.nanmean(out)) if len(out) else 1.0
    if m > 0:
        out = out / m
...
def horizon_train_window_config(cfg: Config) -> Dict[str, object]:
    ...
    return {
        ...
        "recency_sample_weight": {
            "enabled": bool(getattr(cfg, "use_recency_sample_weight", True)),
            "mode": str(getattr(cfg, "recency_weight_mode", "exponential")),
            "half_life_by_horizon": {
                "5": int(getattr(cfg, "recency_half_life_5d", 126)),
                "10": int(getattr(cfg, "recency_half_life_10d", 252)),
                "20": int(getattr(cfg, "recency_half_life_20d", 504)),
            },
            "clip": {
                "min": float(getattr(cfg, "recency_weight_min", 0.25)),
                "max": float(getattr(cfg, "recency_weight_max", 4.0)),
            },
            "normalize": bool(getattr(cfg, "recency_weight_normalize", True)),
            "apply_stage1": bool(getattr(cfg, "recency_apply_stage1", True)),
            "apply_direction_binary": bool(getattr(cfg, "recency_apply_direction_binary", True)),
            "apply_downrisk": bool(getattr(cfg, "recency_apply_downrisk", True)),
            "apply_direction_strength": bool(getattr(cfg, "recency_apply_direction_strength", True)),
        },
    }
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            for h in cfg.horizons:
                ...
                recency_w_h = make_recency_sample_weight(len(train_df_h), cfg, int(h))

                if len(np.unique(y_high)) < 2:
                    continue

                stage1_model = make_xgb_stage1(cfg, calc_scale_pos_weight(y_high))
                if bool(getattr(cfg, "recency_apply_stage1", True)):
                    stage1_model.fit(X_train_h, y_high, model__sample_weight=recency_w_h)
                else:
                ...
                if len(np.unique(y_up)) == 2 and int(y_up.sum()) >= int(getattr(cfg, "direction_min_positive", 20)):
                    ...
                    if bool(getattr(cfg, "recency_apply_direction_binary", True)):
                        up_model.fit(train_df_h[direction_cols], y_up, model__sample_weight=recency_w_h)
                    else:
                ...
                if len(np.unique(y_down)) == 2 and int(y_down.sum()) >= int(getattr(cfg, "direction_min_positive", 20)):
                    for branch, cols in down_feature_sets.items():
                        ...
                        if bool(getattr(cfg, "recency_apply_downrisk", True)):
                            m_down.fit(train_df_h[cols], y_down, model__sample_weight=recency_w_h)
                        else:
            ...
            if bool(getattr(cfg, "use_direction_strength_specialist", True)):
                try:
                    ...
                    # v8.7.0: 별도 bear-filter specialist 제거.
        ...
        if bool(getattr(cfg, "use_direction_strength_specialist", True)):
            ...
            bear_strength_probs = {name: 0.0 for name in DIRECTION_STRENGTH_LABELS}  # removed in v8.7.0
        else:
        ...
        out.update({
            ...
            **{f"prob_up_weakening_{int(sh)}d": 0.0 for sh in multi_strength_horizons},
            # v8.7.0: 하락 강화도 상승 강화와 동일한 horizon별 term structure로 산출한다.
...
def fit_direction_strength_specialist(
    ...
    class_w = _multiclass_sample_weights(y_local)
    recency_w = make_recency_sample_weight(len(idx), cfg, int(h_for_window))
    if bool(getattr(cfg, "recency_apply_direction_strength", True)):
        sw = combine_sample_weights(class_w, recency_w)
    else:
        sw = combine_sample_weights(class_w)
...
def apply_direction_strength_specialist_policy(
    ...
    """
    v8.7.0 Volatility-Base Strong-Override Upside Strength Trigger.
    ...
    """
    ph = _row_float(row, "prob_high_vol")
    # v8.7.0: weak 확률(prob_up/prob_down/prob_bear_down_strengthening)은 allocation에서 제거한다.
    ...
    normal_unrestricted_enabled = bool(getattr(cfg, "normal_unrestricted_enabled", True))
    normal_base_boost_signal = bool(
        normal_unrestricted_enabled
        and bool(getattr(cfg, "normal_base_boost_enabled", True))
        and ph < 0.50
    )
    normal_full_up_confirm = bool(
        pus_score >= float(getattr(cfg, "normal_full_up_score_threshold", 0.38))
        or p20 >= float(getattr(cfg, "normal_full_p20_threshold", 0.34))
        or (bool(getattr(cfg, "normal_full_allow_short_mid", True)) and short_mid_confirm)
    )
    normal_full_signal = bool(
        normal_unrestricted_enabled
        and bool(getattr(cfg, "normal_full_enabled", True))
        and ph < float(getattr(cfg, "normal_full_high_vol_threshold", 0.25))
        and pds_score < float(getattr(cfg, "normal_full_down_strength_threshold", 0.35))
        and normal_full_up_confirm
    )
    normal_unrestricted_action = "diagnostic_only"

    custom_lite_p20_ok = True
    if bool(getattr(cfg, "custom_lite_require_p20", True)):
        custom_lite_p20_ok = bool(p20 >= float(getattr(cfg, "custom_lite_p20_threshold", 0.25)))
    custom_lite_trend_ok = True
    if bool(getattr(cfg, "custom_lite_exclude_bear_trend", False)):
        custom_lite_trend_ok = bool(trend_state != "BEAR")
    custom_lite_base_candidate = bool(
        normal_unrestricted_enabled
        and bool(getattr(cfg, "enable_custom_lite", False))
        and str(regime) == "NORMAL"
        and ph < float(getattr(cfg, "custom_lite_high_vol_threshold", 0.35))
        and pus_score >= float(getattr(cfg, "custom_lite_up_score_threshold", 0.26))
        and pds_score < float(getattr(cfg, "custom_lite_down_strength_threshold", 0.42))
        and custom_lite_p20_ok
        and custom_lite_trend_ok
    )
    custom_lite_signal = bool(custom_lite_base_candidate)
    custom_lite_no_offensive_ok = True
    custom_lite_action = "off"
    tier1_boost_signal = False
    tier1_boost_action = "off"

    # v8.7.0: 강한 상승 override는 regime bucket보다 실제 high-vol cap을 우선한다.
    ...
    if True:
        ...
        if not bool(getattr(cfg, "disable_tier2_signal", False)):
            tier2_signal = bool(
        ...
        if not bool(getattr(cfg, "disable_tier2_signal", False)):
            if short_mid_mode == "tier2_replace":
                tier2_signal = bool(short_mid_confirm)
                short_mid_policy_action = "tier2_replace" if short_mid_confirm else "tier2_replace_off"
            elif short_mid_mode == "tier2_add":
                if short_mid_confirm and not tier2_signal:
                    short_mid_policy_action = "tier2_add"
                tier2_signal = bool(tier2_signal or short_mid_confirm)
        tier3_signal = bool(
        ...
        # v8.7.0 Tier1-only boost: Tier3/Full보다 약하지만 진단상 수익률이 높은 Tier1-only 구간을 선별 상향한다.
        tier1_boost_signal = bool(
            bool(getattr(cfg, "enable_tier1_boost", False))
            and tier1_signal
            and (not bool(getattr(cfg, "tier1_boost_require_no_tier2", True)) or not tier2_signal)
            and (not bool(getattr(cfg, "tier1_boost_exclude_full_tier3", True)) or not (tier3_signal or full_stock_signal))
            and ph < float(getattr(cfg, "tier1_boost_high_vol_threshold", 0.70))
            and pds_score < float(getattr(cfg, "tier1_boost_down_strength_threshold", 0.32))
        )
        tier1_boost_action = "off"
        if tier1_boost_signal:
            tier1_boost_stock = float(getattr(cfg, "tier1_boost_stock_weight", 0.86))
            if tier1_boost_stock > target_stock + 1e-12:
                target_stock = max(target_stock, tier1_boost_stock)
                offensive_tier = max(offensive_tier, 1)
                tier1_boost_action = "tier1_boost_upgrade"
                if bool(getattr(cfg, "tier1_boost_force_rebalance", False)):
                    force_rebalance = True

        # v8.7.0 Normal-Unrestricted policy.
        # 정상/저변동 상태에서는 기본 비중 cap을 완화하고, 상승 확인까지 있으면 100%를 허용한다.
        if normal_base_boost_signal and not (tier1_signal or tier2_signal or tier3_signal or full_stock_signal):
            if ph < 0.25:
                normal_base_weight = float(getattr(cfg, "normal_base_stock_lt_25", 0.86))
            elif ph < 0.35:
                normal_base_weight = float(getattr(cfg, "normal_base_stock_lt_35", 0.82))
            else:
                normal_base_weight = float(getattr(cfg, "normal_base_stock_lt_50", 0.74))
            if normal_base_weight > target_stock + 1e-12:
                target_stock = max(target_stock, normal_base_weight)
                offensive_tier = max(offensive_tier, 1)
                normal_unrestricted_action = "normal_base_boost"

        if normal_full_signal:
            target_stock = max(target_stock, float(getattr(cfg, "normal_full_stock_weight", 1.00)))
            offensive_tier = max(offensive_tier, 3)
            normal_unrestricted_action = "normal_full"
            if bool(getattr(cfg, "normal_full_force_rebalance", False)):
                force_rebalance = True

        # v8.7.0 CUSTOM_LITE 재설계:
        # - NORMAL 전체 상향이 아니라 NORMAL/no-offensive 내부에서 약한 상승강도만 선별한다.
        # - 기본값은 require_no_offensive=True라서 Tier/Full/ShortMid와 중복되지 않는다.
        custom_lite_blocking_offensive = bool(tier1_signal or tier2_signal or tier3_signal or full_stock_signal or normal_full_signal or short_mid_confirm or tier1_boost_signal)
        if bool(getattr(cfg, "custom_lite_allow_tier1", False)):
            custom_lite_blocking_offensive = bool(tier2_signal or tier3_signal or full_stock_signal or normal_full_signal or short_mid_confirm)
        custom_lite_no_offensive_ok = bool(not custom_lite_blocking_offensive)
        if bool(getattr(cfg, "custom_lite_apply_after_offensive", True)):
            custom_lite_signal = bool(
                custom_lite_base_candidate
                and (not bool(getattr(cfg, "custom_lite_require_no_offensive", True)) or custom_lite_no_offensive_ok)
            )
        if custom_lite_signal and not (tier3_signal or full_stock_signal or normal_full_signal):
            lite_stock = float(getattr(cfg, "custom_lite_stock_weight", 0.86))
            if lite_stock > target_stock + 1e-12:
                target_stock = max(target_stock, lite_stock)
                offensive_tier = max(offensive_tier, 1)
                custom_lite_action = "custom_lite_upgrade"
                normal_unrestricted_action = "custom_lite"
                if bool(getattr(cfg, "custom_lite_force_rebalance", False)):
                    force_rebalance = True

        # v8.7.0 ShortMidConfirm optional allocation actions.
        # 기본 diagnostic 모드에서는 아래 두 동작이 작동하지 않는다.
        if short_mid_mode == "base_upgrade" and short_mid_confirm and not (tier1_signal or tier2_signal or tier3_signal or full_stock_signal):
            target_stock = max(target_stock, float(getattr(cfg, "short_mid_base_upgrade_stock_weight", 0.82)))
            offensive_tier = max(offensive_tier, 1)
            short_mid_policy_action = "base_upgrade"

        if short_mid_mode == "tier1_upgrade" and tier1_signal and short_mid_loose_confirm and not (tier2_signal or tier3_signal or full_stock_signal):
            target_stock = max(target_stock, float(getattr(cfg, "short_mid_tier1_upgrade_stock_weight", 0.84)))
            offensive_tier = max(offensive_tier, 1)
            short_mid_policy_action = "tier1_upgrade"

        # Tier 2: 10D+20D 확인 기반 중간 공격 구간. 88% 제한.
        if tier2_signal:
            tier2_weight = float(getattr(cfg, "up_strength_pair_10d_20d_stock_weight", 0.88))
            if short_mid_mode in {"tier2_add", "tier2_replace"} and short_mid_confirm:
    ...
    # v8.7.0: bear/down 약한 확률 기반 cut은 제거한다.
    ...
    meta = {
        ...
        "normal_unrestricted_enabled": bool(normal_unrestricted_enabled),
        "normal_base_boost_signal": bool(normal_base_boost_signal),
        "normal_full_signal": bool(normal_full_signal),
        "normal_full_up_confirm": bool(normal_full_up_confirm),
        "normal_unrestricted_action": str(normal_unrestricted_action),
        "normal_full_down_strength_score": float(pds_score),
        "custom_lite_signal": bool(custom_lite_signal),
        "custom_lite_action": str(custom_lite_action),
        "custom_lite_target_stock": float(getattr(cfg, "custom_lite_stock_weight", 0.86)),
        "custom_lite_base_candidate": bool(custom_lite_base_candidate),
        "custom_lite_no_offensive_ok": bool(custom_lite_no_offensive_ok),
        "custom_lite_p20_ok": bool(custom_lite_p20_ok),
        "custom_lite_trend_ok": bool(custom_lite_trend_ok),
        "tier1_boost_signal": bool(tier1_boost_signal),
        "tier1_boost_action": str(tier1_boost_action),
        "tier1_boost_target_stock": float(getattr(cfg, "tier1_boost_stock_weight", 0.86)),
...
def _market_sharpe_proxy(row: pd.Series, horizon: int) -> float:
    """과거 수익률/실현변동성 기반 간단한 시장 Sharpe proxy.

    return_h / (daily_vol * sqrt(h)) 형태이며, 모든 입력은 이미 과거 rolling feature다.
    """
    h = int(horizon)
    ret = _row_float(row, f"return_{h}d", np.nan)
    if not np.isfinite(ret):
        return 0.0
    vol_col = f"realized_vol_{h}" if f"realized_vol_{h}" in row.index else "realized_vol_60"
    vol = _row_float(row, vol_col, np.nan)
...
def apply_trend_participation_overlay(
    ...
    """v8.7.0: 강한 단방향 상승 추세장에서 구조적 underweight drag를 줄이는 overlay."""
    ...
    trend_bull = bool(
        bool(getattr(cfg, "enable_trend_participation_overlay", True))
    ...
    if trend_bull:
        ...
        if bool(getattr(cfg, "trend_full_stock_when_low_vol", False)) and ph < float(getattr(cfg, "trend_full_stock_high_vol_threshold", 0.25)):
            floor = max(floor, 1.0)
        # down_strength가 과도하면 floor를 약하게 완화한다. 강한 추세라도 하방 강화가 높으면 100% 추격을 피한다.
        if pds_score >= 0.50:
...
def apply_recovery_rerisk_overlay(
    ...
    """v8.7.0: 폭락 후 V자 반등 초입 재진입 지연을 줄이는 overlay."""
    ph = _row_float(row, "prob_high_vol", 0.0)
    dd60 = _row_float(row, "drawdown_60", 0.0)
    ret10 = _row_float(row, "return_10d", 0.0)
    ret20 = _row_float(row, "return_20d", 0.0)
    ma20_gap = _row_float(row, "price_ma_20_gap", 0.0)
    pds_score = _row_float(row, "prob_down_strengthening_score", 0.0)
    pus_score = _row_float(row, "prob_up_strengthening_score", 0.0)
    recovery = bool(
        bool(getattr(cfg, "enable_recovery_rerisk_overlay", True))
...
def apply_soft_drawdown_guard(
    ...
    *,
    ...
    """v8.7.0: 명시적 stop-loss가 아닌 약한 포트폴리오 drawdown guard."""
    ph = _row_float(row, "prob_high_vol", 0.0)
    active = False
    action = "off"
    cut = 0.0
    target_stock = float(signal_w[0])
    if bool(getattr(cfg, "enable_soft_drawdown_guard", False)):
        relaxed = bool(
            (trend_bull and bool(getattr(cfg, "drawdown_guard_relax_in_trend", True)))
            or (recovery_risk_on and bool(getattr(cfg, "drawdown_guard_relax_in_recovery", True)))
        )
        threshold1 = float(getattr(cfg, "drawdown_guard_threshold_1", -0.08))
        threshold2 = float(getattr(cfg, "drawdown_guard_threshold_2", -0.12))
        # trend/recovery 구간에서는 너무 빠른 재위험화를 막지 않기 위해 trigger를 3%p 완화한다.
        if relaxed:
            threshold1 -= 0.03
            threshold2 -= 0.03
        if ph >= float(getattr(cfg, "drawdown_guard_high_vol_min", 0.40)) and target_stock > float(getattr(cfg, "drawdown_guard_min_stock", 0.30)):
            if portfolio_drawdown <= threshold2:
                active = True
                cut = float(getattr(cfg, "drawdown_guard_cut_2", 0.20))
                cap = float(getattr(cfg, "drawdown_guard_stock_cap_2", 0.70))
                target_stock = min(max(float(getattr(cfg, "drawdown_guard_min_stock", 0.30)), target_stock - cut), cap)
                action = "drawdown_guard_level2"
            elif portfolio_drawdown <= threshold1:
                active = True
                cut = float(getattr(cfg, "drawdown_guard_cut_1", 0.10))
                target_stock = max(float(getattr(cfg, "drawdown_guard_min_stock", 0.30)), target_stock - cut)
                action = "drawdown_guard_level1"
    out_w = _redistribute_after_stock_change(target_stock, signal_w)
    meta = {
        "portfolio_drawdown_guard_input": float(portfolio_drawdown),
        "drawdown_guard_active": bool(active),
        "drawdown_guard_action": str(action),
        ...
        "drawdown_guard_force_rebalance": bool(active and bool(getattr(cfg, "drawdown_guard_force_rebalance", False))),
    }
    return out_w, meta
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    # v8.7.0 soft drawdown guard는 현재까지 실현된 전략 equity만 사용한다.
    running_equity_net = float(getattr(cfg, "initial_capital", 100_000_000))
    ...
    for i, row in pred_df.iterrows():
        ...
        # v8.7.0 structural overlays: trend participation -> recovery re-risk -> optional soft drawdown guard.
        pre_structural_signal_w = signal_w
        trend_w, trend_meta = apply_trend_participation_overlay(signal_w, row, cfg)
        signal_w = trend_w
        policy_meta.update(trend_meta)

        recovery_w, recovery_meta = apply_recovery_rerisk_overlay(signal_w, row, cfg)
        signal_w = recovery_w
        policy_meta.update(recovery_meta)

        dd_window = int(max(2, getattr(cfg, "drawdown_guard_window", 60)))
        recent_equity = equity_history[-dd_window:] if len(equity_history) >= 1 else [running_equity_net]
        peak_equity = max(recent_equity) if recent_equity else running_equity_net
        portfolio_dd_window = float(running_equity_net / peak_equity - 1.0) if peak_equity > 0 else 0.0
        guard_w, guard_meta = apply_soft_drawdown_guard(
            signal_w,
            row,
            cfg,
            portfolio_dd_window,
        ...
        policy_meta["structural_overlay_total"] = float(signal_w[0] - pre_structural_signal_w[0])
        policy_meta["portfolio_drawdown_window_pre_trade"] = float(portfolio_dd_window)
        policy_force_rebalance = bool(
        ...
        if prev_w is None:
            ...
            if policy_force_rebalance:
                ...
                elif bool(policy_meta.get("recovery_rerisk_force_rebalance", False)):
                    hold_reason = "recovery_rerisk_force"
                elif bool(policy_meta.get("trend_participation_force_rebalance", False)):
                    hold_reason = "trend_participation_force"
                elif bool(policy_meta.get("drawdown_guard_force_rebalance", False)):
                    hold_reason = "drawdown_guard_force"
                else:
        ...
        running_equity_net = float(running_equity_net * (1.0 + net))
        ...
        out.update({
            ...
            "structural_overlay_total": float(policy_meta.get("structural_overlay_total", 0.0)),
            "market_sharpe_60": float(policy_meta.get("market_sharpe_60", np.nan)),
            "market_sharpe_120": float(policy_meta.get("market_sharpe_120", np.nan)),
            "trend_positive_ratio_60": float(policy_meta.get("trend_positive_ratio_60", np.nan)),
            "trend_bull_regime": bool(policy_meta.get("trend_bull_regime", False)),
            "trend_participation_action": str(policy_meta.get("trend_participation_action", "")),
            "trend_participation_overlay": float(policy_meta.get("trend_participation_overlay", 0.0)),
            "trend_participation_target_stock": float(policy_meta.get("trend_participation_target_stock", np.nan)),
            "recovery_risk_on": bool(policy_meta.get("recovery_risk_on", False)),
            "recovery_rerisk_action": str(policy_meta.get("recovery_rerisk_action", "")),
            "recovery_rerisk_overlay": float(policy_meta.get("recovery_rerisk_overlay", 0.0)),
            "recovery_rerisk_target_stock": float(policy_meta.get("recovery_rerisk_target_stock", np.nan)),
            "portfolio_drawdown_window_pre_trade": float(policy_meta.get("portfolio_drawdown_window_pre_trade", 0.0)),
            "drawdown_guard_active": bool(policy_meta.get("drawdown_guard_active", False)),
            "drawdown_guard_action": str(policy_meta.get("drawdown_guard_action", "")),
            "drawdown_guard_overlay": float(policy_meta.get("drawdown_guard_overlay", 0.0)),
            "drawdown_guard_target_stock": float(policy_meta.get("drawdown_guard_target_stock", np.nan)),
            "policy_force_rebalance": bool(policy_force_rebalance),
...
def classification_metrics(pred_df: pd.DataFrame, cfg: Config) -> Dict[str, object]:
    ...
    # v8.7.0: prob_up/prob_down은 약한 출력으로 판정되어 summary 핵심 성능에서 제외한다.
    ...
    if "actual_direction_strength" in pred_df.columns:
        ...
        # v8.7.0: prob_bear_down_strengthening은 allocation/summary 성능에서 제외한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_trend_participation_v8_7_0_diagnostics",
        ...
        "direction_strength_specialist": {
            ...
            "recency_sample_weight_enabled": bool(getattr(cfg, "use_recency_sample_weight", True)),
            "recency_weight_mode": str(getattr(cfg, "recency_weight_mode", "exponential")),
            "recency_half_life": {
                "5d": int(getattr(cfg, "recency_half_life_5d", 126)),
                "10d": int(getattr(cfg, "recency_half_life_10d", 252)),
                "20d": int(getattr(cfg, "recency_half_life_20d", 504)),
            },
            "recency_weight_clip": {
                "min": float(getattr(cfg, "recency_weight_min", 0.25)),
                "max": float(getattr(cfg, "recency_weight_max", 4.0)),
            ...
            "recency_apply": {
                "stage1": bool(getattr(cfg, "recency_apply_stage1", True)),
                "direction_binary": bool(getattr(cfg, "recency_apply_direction_binary", True)),
                "downrisk": bool(getattr(cfg, "recency_apply_downrisk", True)),
                "direction_strength": bool(getattr(cfg, "recency_apply_direction_strength", True)),
            },
            ...
            "normal_unrestricted_enabled": bool(getattr(cfg, "normal_unrestricted_enabled", True)),
            "normal_base_boost_rate": float(pred_df.get("normal_base_boost_signal", pd.Series(False, index=pred_df.index)).astype(bool).mean()),
            "normal_full_signal_rate": float(pred_df.get("normal_full_signal", pd.Series(False, index=pred_df.index)).astype(bool).mean()),
            "custom_lite_enabled": bool(getattr(cfg, "enable_custom_lite", False)),
            "custom_lite_signal_rate": float(pred_df.get("custom_lite_signal", pd.Series(False, index=pred_df.index)).astype(bool).mean()),
            "custom_lite_base_candidate_rate": float(pred_df.get("custom_lite_base_candidate", pd.Series(False, index=pred_df.index)).astype(bool).mean()),
            "custom_lite_no_offensive_ok_rate": float(pred_df.get("custom_lite_no_offensive_ok", pd.Series(False, index=pred_df.index)).astype(bool).mean()),
            "custom_lite_action_distribution_pct": pred_df.get("custom_lite_action", pd.Series("", index=pred_df.index)).astype(str).value_counts(normalize=True).mul(100).round(2).to_dict(),
            "tier1_boost_enabled": bool(getattr(cfg, "enable_tier1_boost", False)),
            "tier1_boost_signal_rate": float(pred_df.get("tier1_boost_signal", pd.Series(False, index=pred_df.index)).astype(bool).mean()),
            "tier1_boost_action_distribution_pct": pred_df.get("tier1_boost_action", pd.Series("", index=pred_df.index)).astype(str).value_counts(normalize=True).mul(100).round(2).to_dict(),
            "normal_unrestricted_action_distribution_pct": pred_df.get("normal_unrestricted_action", pd.Series("", index=pred_df.index)).astype(str).value_counts(normalize=True).mul(100).round(2).to_dict(),
            "avg_normal_full_down_strength_score": float(pred_df.get("normal_full_down_strength_score", pd.Series(0.0, index=pred_df.index)).astype(float).mean()),
        ...
        "structural_overlays": {
            "trend_participation_enabled": bool(getattr(cfg, "enable_trend_participation_overlay", True)),
            ...
            "recovery_rerisk_enabled": bool(getattr(cfg, "enable_recovery_rerisk_overlay", True)),
            ...
            "soft_drawdown_guard_enabled": bool(getattr(cfg, "enable_soft_drawdown_guard", False)),
            "drawdown_guard_active_rate": float(pred_df.get("drawdown_guard_active", pd.Series(False, index=pred_df.index)).astype(bool).mean()),
            "drawdown_guard_action_distribution_pct": pred_df.get("drawdown_guard_action", pd.Series("", index=pred_df.index)).astype(str).value_counts(normalize=True).mul(100).round(2).to_dict(),
            "avg_drawdown_guard_overlay": float(pred_df.get("drawdown_guard_overlay", pd.Series(0.0, index=pred_df.index)).astype(float).mean()),
        },
        ...
        "latest_prediction": {
            ...
            "custom_lite_signal": bool(latest.get("custom_lite_signal", False)),
            "custom_lite_action": str(latest.get("custom_lite_action", "")),
            "custom_lite_base_candidate": bool(latest.get("custom_lite_base_candidate", False)),
            "custom_lite_no_offensive_ok": bool(latest.get("custom_lite_no_offensive_ok", False)),
            "custom_lite_target_stock": round(float(latest.get("custom_lite_target_stock", 0.0)) * 100, 2) if pd.notna(latest.get("custom_lite_target_stock", np.nan)) else None,
            "tier1_boost_signal": bool(latest.get("tier1_boost_signal", False)),
            "tier1_boost_action": str(latest.get("tier1_boost_action", "")),
            "tier1_boost_target_stock": round(float(latest.get("tier1_boost_target_stock", 0.0)) * 100, 2) if pd.notna(latest.get("tier1_boost_target_stock", np.nan)) else None,
...
def _segment_return_stats(pred_df: pd.DataFrame, mask: pd.Series, section: str, segment: str) -> Dict[str, object]:
    g = pred_df.loc[mask.fillna(False).astype(bool)].copy()
    rr = g["strategy_return_net"].astype(float) if "strategy_return_net" in g.columns else pd.Series(dtype=float)
    total_n = len(pred_df)
    return {
        "section": section,
        "segment": segment,
        "count": int(len(g)),
        "pct": float(len(g) / total_n) if total_n else 0.0,
        "win_rate": _win_rate(rr) if len(g) else 0.0,
        "mean_daily_return": float(rr.mean()) if len(g) else 0.0,
        "ann_return_est": _annualized_return(rr) if len(g) else 0.0,
        "ann_vol_est": _annualized_vol(rr) if len(g) else 0.0,
        "avg_stock_weight": float(g["stock_weight"].mean()) if len(g) and "stock_weight" in g.columns else 0.0,
        "avg_turnover": float(g["turnover"].mean()) if len(g) and "turnover" in g.columns else 0.0,
        "avg_prob_high_vol": float(g["prob_high_vol"].mean()) if len(g) and "prob_high_vol" in g.columns else 0.0,
        "avg_up_strength_score": float(g["prob_up_strengthening_score"].mean()) if len(g) and "prob_up_strengthening_score" in g.columns else 0.0,
        "avg_down_strength_score": float(g["prob_down_strengthening_score"].mean()) if len(g) and "prob_down_strengthening_score" in g.columns else 0.0,
        "avg_offensive_tier": float(g["offensive_tier"].fillna(0).astype(float).mean()) if len(g) and "offensive_tier" in g.columns else 0.0,
        "avg_full_stock_rate": float(g["full_stock_signal"].astype(bool).mean()) if len(g) and "full_stock_signal" in g.columns else 0.0,
        "avg_tier3_rate": float(g["tier3_signal"].astype(bool).mean()) if len(g) and "tier3_signal" in g.columns else 0.0,
    }


def build_custom_signal_breakdown(pred_df: pd.DataFrame) -> pd.DataFrame:
    """v8.7.0: signal-flag 우선순위 기준까지 포함한 성과 분해 리포트.

    v8.6.40의 custom_breakdown은 allocation_regime=CUSTOM에만 묶여 있어
    full_stock/tier3 신호가 NORMAL/WATCH로 남는 경우를 놓쳤다.
    따라서 v8.7.0은 allocation_regime, executed_regime, signal_binary,
    priority_signal을 모두 저장한다.
    """
    rows: List[Dict[str, object]] = []
    if pred_df.empty or "strategy_return_net" not in pred_df.columns:
        return pd.DataFrame(rows)
    idx = pred_df.index
    alloc = pred_df.get("allocation_regime", pd.Series("", index=idx)).astype(str)
    executed = pred_df.get("executed_regime", pd.Series("", index=idx)).astype(str)
    signal = pred_df.get("signal_regime", pd.Series("", index=idx)).astype(str)
    tier = pred_df.get("offensive_tier", pd.Series(0, index=idx)).fillna(0).astype(int)
    full = pred_df.get("full_stock_signal", pd.Series(False, index=idx)).astype(bool)
    t3 = pred_df.get("tier3_signal", pd.Series(False, index=idx)).astype(bool)
    t2 = pred_df.get("tier2_signal", pd.Series(False, index=idx)).astype(bool)
    t1 = pred_df.get("tier1_signal", pd.Series(False, index=idx)).astype(bool)
    sm = pred_df.get("short_mid_confirm", pd.Series(False, index=idx)).astype(bool)
    lite = pred_df.get("custom_lite_signal", pd.Series(False, index=idx)).astype(bool)
    lite_base = pred_df.get("custom_lite_base_candidate", pd.Series(False, index=idx)).astype(bool)
    t1boost = pred_df.get("tier1_boost_signal", pd.Series(False, index=idx)).astype(bool)
    offensive = pred_df.get("offensive_active", pd.Series(False, index=idx)).astype(bool)
    trend_bull = pred_df.get("trend_bull_regime", pd.Series(False, index=idx)).astype(bool)
    recovery = pred_df.get("recovery_risk_on", pd.Series(False, index=idx)).astype(bool)
    dd_guard = pred_df.get("drawdown_guard_active", pd.Series(False, index=idx)).astype(bool)

    for val in sorted(alloc.dropna().unique()):
        rows.append(_segment_return_stats(pred_df, alloc == val, "allocation_regime", str(val)))
    for val in sorted(executed.dropna().unique()):
        rows.append(_segment_return_stats(pred_df, executed == val, "executed_regime", str(val)))
    for val in sorted(signal.dropna().unique()):
        rows.append(_segment_return_stats(pred_df, signal == val, "signal_regime", str(val)))
    for k in sorted(tier.dropna().unique()):
        rows.append(_segment_return_stats(pred_df, tier == int(k), "offensive_tier", f"tier_{int(k)}"))

    binary_segments = {
        "full_stock_true": full,
        "tier3_true": t3,
        "tier2_true": t2,
        "tier1_true": t1,
        "tier1_boost_true": t1boost,
        "short_mid_true": sm,
        "custom_lite_true": lite,
        "custom_lite_base_candidate_true": lite_base,
        "offensive_active_true": offensive,
        "trend_bull_true": trend_bull,
        "recovery_risk_on_true": recovery,
        "drawdown_guard_active_true": dd_guard,
        "no_offensive_true": ~offensive,
    }
    for name, mask in binary_segments.items():
        rows.append(_segment_return_stats(pred_df, mask, "signal_binary", name))

    # 독립 signal priority. allocation_regime과 무관하게 실제 신호 품질을 본다.
    priority_segments = {
        "priority_trend_bull_no_full_t3": trend_bull & ~full & ~t3,
        "priority_recovery_no_trend_full_t3": recovery & ~trend_bull & ~full & ~t3,
        "priority_drawdown_guard": dd_guard,
        "priority_full_stock": full,
        "priority_tier3_no_full": t3 & ~full,
        "priority_tier1_boost_no_t3_full": t1boost & ~t3 & ~full,
        "priority_custom_lite_no_boost_t3_full": lite & ~t1boost & ~t3 & ~full,
        "priority_short_mid_no_lite_boost_t3_full": sm & ~lite & ~t1boost & ~t3 & ~full,
        "priority_tier2_only": t2 & ~sm & ~lite & ~t1boost & ~t3 & ~full,
        "priority_tier1_only": t1 & ~t2 & ~sm & ~lite & ~t1boost & ~t3 & ~full,
        "priority_no_strength_signal": ~t1 & ~t2 & ~sm & ~lite & ~t1boost & ~t3 & ~full,
    }
    for name, mask in priority_segments.items():
        rows.append(_segment_return_stats(pred_df, mask, "priority_signal", name))

    custom = alloc == "CUSTOM"
    rows.append(_segment_return_stats(pred_df, custom, "custom_breakdown", "custom_all"))
    rows.append(_segment_return_stats(pred_df, custom & full, "custom_breakdown", "custom_full_stock"))
    rows.append(_segment_return_stats(pred_df, custom & t3 & ~full, "custom_breakdown", "custom_tier3_no_full"))
    rows.append(_segment_return_stats(pred_df, custom & t1boost & ~t3 & ~full, "custom_breakdown", "custom_tier1_boost_no_t3_full"))
    rows.append(_segment_return_stats(pred_df, custom & lite & ~t1boost & ~t3 & ~full, "custom_breakdown", "custom_lite_no_boost_tier3_full"))
    rows.append(_segment_return_stats(pred_df, custom & sm & ~lite & ~t1boost & ~t3 & ~full, "custom_breakdown", "custom_short_mid_no_lite_boost_tier3_full"))
    rows.append(_segment_return_stats(pred_df, custom & t2 & ~sm & ~lite & ~t1boost & ~t3 & ~full, "custom_breakdown", "custom_tier2_only"))
    rows.append(_segment_return_stats(pred_df, custom & t1 & ~t2 & ~sm & ~lite & ~t1boost & ~t3 & ~full, "custom_breakdown", "custom_tier1_only"))
    rows.append(_segment_return_stats(pred_df, custom & ~t1 & ~t2 & ~sm & ~lite & ~t1boost & ~t3 & ~full, "custom_breakdown", "custom_no_strength_signal"))

    normal = alloc == "NORMAL"
    rows.append(_segment_return_stats(pred_df, normal, "normal_breakdown", "normal_all"))
    rows.append(_segment_return_stats(pred_df, normal & full, "normal_breakdown", "normal_full_stock"))
    rows.append(_segment_return_stats(pred_df, normal & t3 & ~full, "normal_breakdown", "normal_tier3_no_full"))
    rows.append(_segment_return_stats(pred_df, normal & t1boost & ~t3 & ~full, "normal_breakdown", "normal_tier1_boost"))
    rows.append(_segment_return_stats(pred_df, normal & lite & ~t1boost & ~t3 & ~full, "normal_breakdown", "normal_custom_lite"))
    rows.append(_segment_return_stats(pred_df, normal & ~lite & ~t1boost & offensive, "normal_breakdown", "normal_other_offensive"))
    rows.append(_segment_return_stats(pred_df, normal & ~lite & ~t1boost & ~offensive, "normal_breakdown", "normal_no_offensive"))

    # signal이 어느 regime에서 발생했는지 확인하는 overlap 진단.
    rows.append(_segment_return_stats(pred_df, full & custom, "signal_regime_overlap", "full_stock_in_custom"))
    rows.append(_segment_return_stats(pred_df, full & ~custom, "signal_regime_overlap", "full_stock_not_custom"))
    rows.append(_segment_return_stats(pred_df, t3 & custom, "signal_regime_overlap", "tier3_in_custom"))
    rows.append(_segment_return_stats(pred_df, t3 & ~custom, "signal_regime_overlap", "tier3_not_custom"))
    rows.append(_segment_return_stats(pred_df, t1boost & custom, "signal_regime_overlap", "tier1_boost_in_custom"))
    rows.append(_segment_return_stats(pred_df, t1boost & ~custom, "signal_regime_overlap", "tier1_boost_not_custom"))

    df = pd.DataFrame(rows)
    if df.empty:
        return df
    return df.sort_values(["section", "ann_return_est", "count"], ascending=[True, False, False]).reset_index(drop=True)
...
def diagnostics_summary(diagnostics: Dict[str, pd.DataFrame]) -> Dict[str, object]:
    ...
    csb = diagnostics.get("custom_signal_breakdown", pd.DataFrame())
    if not csb.empty:
        custom_rows = csb[(csb.get("section") == "custom_breakdown") & (csb.get("count", 0) > 0)].copy()
        if not custom_rows.empty:
            out["best_custom_breakdown_by_ann_return"] = custom_rows.sort_values("ann_return_est", ascending=False).iloc[0].to_dict()
    return out
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_trend_participation_v8_7_0_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_trend_participation_v8_7_0_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_trend_participation_v8_7_0_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.7.0 Trend Participation + Recovery Re-risk Policy")
    ...
    parser.add_argument("--rebalance-every", type=int, default=None, help="리밸런싱 주기 직접 지정. v8.7.0 기본 5거래일")
    parser.add_argument("--emergency-cooldown", type=int, default=None, help="긴급 리밸런싱 cooldown 직접 지정. v8.7.0 기본 5거래일")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist", "portfolio_model"], default=None, help="v8.7.0 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.7.0 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.7.0 기본 ON인 stale offensive decay 비활성화")
    ...
    parser.add_argument("--no-horizon-train-window", action="store_true", help="v8.7.0 horizon별 train_rows = horizon*multiplier 모드 비활성화")
    ...
    parser.add_argument("--disable-recency-weight", action="store_true", help="v8.7.0 최근 데이터 sample_weight 비활성화")
    parser.add_argument("--recency-mode", choices=["exponential", "linear"], default=None, help="최근 데이터 가중 방식")
    parser.add_argument("--recency-half-life", type=int, default=None, help="5D/10D/20D에 동일 half-life 적용")
    parser.add_argument("--recency-half-life-5d", type=int, default=None, help="5D recency half-life. 기본 126")
    parser.add_argument("--recency-half-life-10d", type=int, default=None, help="10D recency half-life. 기본 252")
    parser.add_argument("--recency-half-life-20d", type=int, default=None, help="20D recency half-life. 기본 504")
    parser.add_argument("--recency-weight-min", type=float, default=None, help="recency sample_weight 하한. 기본 0.25")
    parser.add_argument("--recency-weight-max", type=float, default=None, help="recency sample_weight 상한. 기본 4.0")
    parser.add_argument("--disable-recency-stage1", action="store_true", help="Stage1 high-vol 모델에는 recency weight를 적용하지 않음")
    parser.add_argument("--disable-recency-direction-binary", action="store_true", help="binary up/down 보조 모델에는 recency weight를 적용하지 않음")
    parser.add_argument("--disable-recency-downrisk", action="store_true", help="down-risk branch 모델에는 recency weight를 적용하지 않음")
    parser.add_argument("--disable-recency-strength", action="store_true", help="UP/DOWN_STRENGTHENING specialist에는 recency weight를 적용하지 않음")
    ...
    parser.add_argument("--disable-normal-unrestricted", action="store_true", help="v8.7.0 정상 상태 비중 제한 완화 정책 비활성화")
    parser.add_argument("--disable-normal-base-boost", action="store_true", help="정상/저변동 기본 비중 상향만 비활성화")
    parser.add_argument("--disable-normal-full", action="store_true", help="정상+상승확인 시 100% 허용 규칙 비활성화")
    parser.add_argument("--normal-base-lt25", type=float, default=None, help="prob_high_vol < 25% 기본 주식 비중. 기본 0.86")
    parser.add_argument("--normal-base-lt35", type=float, default=None, help="25~35% high-vol 기본 주식 비중. 기본 0.82")
    parser.add_argument("--normal-base-lt50", type=float, default=None, help="35~50% high-vol 기본 주식 비중. 기본 0.74")
    parser.add_argument("--normal-full-high-vol", type=float, default=None, help="정상 100% 허용 high-vol 상한. 기본 0.25")
    parser.add_argument("--normal-full-down-strength", type=float, default=None, help="정상 100% 허용 down-strength score 상한. 기본 0.35")
    parser.add_argument("--normal-full-up-score", type=float, default=None, help="정상 100% 허용 up-strength score threshold. 기본 0.38")
    parser.add_argument("--normal-full-p20", type=float, default=None, help="정상 100% 허용 20D up-strength threshold. 기본 0.34")
    parser.add_argument("--normal-full-stock", type=float, default=None, help="정상 full signal 목표 주식 비중. 기본 1.00")
    parser.add_argument("--normal-full-force-rebalance", action="store_true", help="normal_full 발생 시 schedule/no-trade-band 우회")
    parser.add_argument("--disable-vol-base-allocation", action="store_true", help="v8.7.0 high-vol 확률 기반 기본 비중을 끄고 regime bucket 기본 비중 사용")
    ...
    parser.add_argument("--enable-custom-lite", action="store_true", help="NORMAL 내부 약한 strength 확인 구간만 90% 내외로 상향")
    parser.add_argument("--custom-lite-high-vol", type=float, default=None, help="CUSTOM_LITE high-vol 상한. 기본 0.35")
    parser.add_argument("--custom-lite-up-score", type=float, default=None, help="CUSTOM_LITE up_strength_score threshold. 기본 0.26")
    parser.add_argument("--custom-lite-p20", type=float, default=None, help="CUSTOM_LITE 20D up_strength threshold. 기본 0.25")
    parser.add_argument("--custom-lite-down-strength", type=float, default=None, help="CUSTOM_LITE down_strength_score 상한. 기본 0.42")
    parser.add_argument("--custom-lite-stock", type=float, default=None, help="CUSTOM_LITE 목표 주식 비중. 기본 0.90")
    parser.add_argument("--custom-lite-no-p20", action="store_true", help="CUSTOM_LITE에서 20D 확인 조건 제거")
    parser.add_argument("--custom-lite-exclude-bear-trend", action="store_true", help="mid_trend_state=BEAR이면 CUSTOM_LITE 비활성화")
    parser.add_argument("--custom-lite-force-rebalance", action="store_true", help="CUSTOM_LITE 발생 시 schedule/no-trade-band 우회")
    parser.add_argument("--custom-lite-allow-tier1", action="store_true", help="CUSTOM_LITE가 Tier1 신호와 겹쳐도 적용 허용")
    parser.add_argument("--custom-lite-allow-offensive-overlap", action="store_true", help="CUSTOM_LITE에서 no-offensive 요구조건 제거")
    parser.add_argument("--enable-tier1-boost", action="store_true", help="v8.7.0 Tier1-only 고품질 구간 최소 주식비중 보정 활성화")
    parser.add_argument("--tier1-boost-high-vol", type=float, default=None, help="Tier1 boost high-vol 상한. 기본 0.70")
    parser.add_argument("--tier1-boost-down-strength", type=float, default=None, help="Tier1 boost down_strength_score 상한. 기본 0.32")
    parser.add_argument("--tier1-boost-stock", type=float, default=None, help="Tier1 boost 목표 최소 주식비중. 기본 0.86")
    parser.add_argument("--tier1-boost-allow-tier2", action="store_true", help="Tier1 boost가 Tier2와 겹쳐도 적용 허용")
    parser.add_argument("--tier1-boost-allow-full-tier3", action="store_true", help="Tier1 boost의 Tier3/Full 제외 조건 제거")
    parser.add_argument("--tier1-boost-force-rebalance", action="store_true", help="Tier1 boost 발생 시 schedule/no-trade-band 우회")
    parser.add_argument("--disable-trend-participation", action="store_true", help="v8.7 trend participation overlay 비활성화")
    parser.add_argument("--trend-sharpe60", type=float, default=None, help="Trend bull 60D Sharpe proxy threshold. 기본 0.50")
    parser.add_argument("--trend-sharpe120", type=float, default=None, help="Trend bull 120D Sharpe proxy threshold. 기본 0.45")
    parser.add_argument("--trend-positive-ratio60", type=float, default=None, help="Trend bull 60D 상승일 비율 threshold. 기본 0.55")
    parser.add_argument("--trend-max-high-vol", type=float, default=None, help="Trend participation 허용 high-vol 상한. 기본 0.65")
    parser.add_argument("--trend-floor-lt25", type=float, default=None, help="Trend bull & high-vol<25% stock floor. 기본 0.98")
    parser.add_argument("--trend-floor-lt35", type=float, default=None, help="Trend bull & high-vol<35% stock floor. 기본 0.96")
    parser.add_argument("--trend-floor-lt50", type=float, default=None, help="Trend bull & high-vol<50% stock floor. 기본 0.92")
    parser.add_argument("--trend-floor-lt65", type=float, default=None, help="Trend bull & high-vol<65% stock floor. 기본 0.86")
    parser.add_argument("--trend-full-stock-low-vol", action="store_true", help="Trend bull & low-vol 구간에서 100% QQQ 허용")
    parser.add_argument("--trend-force-rebalance", action="store_true", help="Trend overlay 발생 시 schedule/no-trade-band 우회")
    parser.add_argument("--disable-recovery-rerisk", action="store_true", help="v8.7 recovery re-risk overlay 비활성화")
    parser.add_argument("--recovery-dd60", type=float, default=None, help="Recovery trigger 60D drawdown threshold. 기본 -0.12")
    parser.add_argument("--recovery-ret10", type=float, default=None, help="Recovery trigger 10D return threshold. 기본 0.06")
    parser.add_argument("--recovery-ret20", type=float, default=None, help="Recovery trigger 20D return threshold. 기본 0.08")
    parser.add_argument("--recovery-stock-floor", type=float, default=None, help="Recovery re-risk 기본 stock floor. 기본 0.78")
    parser.add_argument("--recovery-strong-stock-floor", type=float, default=None, help="Recovery + strong up score stock floor. 기본 0.90")
    parser.add_argument("--recovery-no-force-rebalance", action="store_true", help="Recovery overlay 발생 시 즉시 리밸런싱 우회 비활성화")
    parser.add_argument("--enable-drawdown-guard", action="store_true", help="Soft portfolio drawdown guard 활성화")
    parser.add_argument("--drawdown-guard-window", type=int, default=None, help="Drawdown guard equity peak window. 기본 60")
    parser.add_argument("--drawdown-guard-threshold-1", type=float, default=None, help="Soft drawdown guard 1단계 threshold. 기본 -0.08")
    parser.add_argument("--drawdown-guard-threshold-2", type=float, default=None, help="Soft drawdown guard 2단계 threshold. 기본 -0.12")
    parser.add_argument("--drawdown-guard-high-vol-min", type=float, default=None, help="Drawdown guard high-vol 최소값. 기본 0.40")
    parser.add_argument("--drawdown-guard-force-rebalance", action="store_true", help="Drawdown guard 발생 시 schedule/no-trade-band 우회")
    parser.add_argument("--disable-strong-override", action="store_true", help="Tier3/Full의 schedule/no-trade-band 우회 즉시 리밸런싱 비활성화")
    parser.add_argument("--disable-tier2", action="store_true", help="Tier2 비활성화. v8.7.0 기본값은 활성화")
    parser.add_argument("--short-mid-mode", choices=["diagnostic", "tier1_upgrade", "tier2_add", "tier2_replace", "base_upgrade"], default=None, help="5D+10D+high-vol ShortMidConfirm 적용 방식. 기본 diagnostic=출력만 추가")
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_trend_participation_v8_7_0_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_trend_participation_v8_7_0_summary.json"
    ...
    cfg.result_dir = f"results_xgb_trend_participation_v8_7_0_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.7.0: 기본 상태는 prob_high_vol 기반 포트폴리오, 강한 신호만 즉시 override한다.
        cfg.use_vol_probability_base_allocation = True
        # v8.7.0: 정상/저변동 상태 기본 비중 상향.
        cfg.vol_base_stock_lt_25 = float(getattr(cfg, "normal_base_stock_lt_25", 0.86)) if bool(getattr(cfg, "normal_base_boost_enabled", True)) else 0.78
        cfg.vol_base_stock_lt_35 = float(getattr(cfg, "normal_base_stock_lt_35", 0.82)) if bool(getattr(cfg, "normal_base_boost_enabled", True)) else 0.74
        cfg.vol_base_stock_lt_50 = float(getattr(cfg, "normal_base_stock_lt_50", 0.74)) if bool(getattr(cfg, "normal_base_boost_enabled", True)) else 0.68
        ...
        # v8.7.0: Tier2를 기본 활성화한다. 기존 실패 구간 재검증을 위해 CLI로 조건/비중 조정 가능.
        ...
        # v8.7.0: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
    ...
    if getattr(args, "disable_recency_weight", False):
        cfg.use_recency_sample_weight = False
    if getattr(args, "recency_mode", None) is not None:
        cfg.recency_weight_mode = str(args.recency_mode)
    if getattr(args, "recency_half_life", None) is not None:
        v = int(args.recency_half_life)
        cfg.recency_half_life_5d = v
        cfg.recency_half_life_10d = v
        cfg.recency_half_life_20d = v
    if getattr(args, "recency_half_life_5d", None) is not None:
        cfg.recency_half_life_5d = int(args.recency_half_life_5d)
    if getattr(args, "recency_half_life_10d", None) is not None:
        cfg.recency_half_life_10d = int(args.recency_half_life_10d)
    if getattr(args, "recency_half_life_20d", None) is not None:
        cfg.recency_half_life_20d = int(args.recency_half_life_20d)
    if getattr(args, "recency_weight_min", None) is not None:
        cfg.recency_weight_min = float(args.recency_weight_min)
    if getattr(args, "recency_weight_max", None) is not None:
        cfg.recency_weight_max = float(args.recency_weight_max)
    if getattr(args, "disable_recency_stage1", False):
        cfg.recency_apply_stage1 = False
    if getattr(args, "disable_recency_direction_binary", False):
        cfg.recency_apply_direction_binary = False
    if getattr(args, "disable_recency_downrisk", False):
        cfg.recency_apply_downrisk = False
    if getattr(args, "disable_recency_strength", False):
        cfg.recency_apply_direction_strength = False
    ...
    if getattr(args, "disable_normal_unrestricted", False):
        cfg.normal_unrestricted_enabled = False
    if getattr(args, "disable_normal_base_boost", False):
        cfg.normal_base_boost_enabled = False
        # v8.6.32 base로 복귀
        cfg.vol_base_stock_lt_25 = 0.78
        cfg.vol_base_stock_lt_35 = 0.74
        cfg.vol_base_stock_lt_50 = 0.68
    if getattr(args, "disable_normal_full", False):
        cfg.normal_full_enabled = False
    if getattr(args, "normal_base_lt25", None) is not None:
        cfg.normal_base_stock_lt_25 = float(args.normal_base_lt25)
        cfg.vol_base_stock_lt_25 = float(args.normal_base_lt25)
    if getattr(args, "normal_base_lt35", None) is not None:
        cfg.normal_base_stock_lt_35 = float(args.normal_base_lt35)
        cfg.vol_base_stock_lt_35 = float(args.normal_base_lt35)
    if getattr(args, "normal_base_lt50", None) is not None:
        cfg.normal_base_stock_lt_50 = float(args.normal_base_lt50)
        cfg.vol_base_stock_lt_50 = float(args.normal_base_lt50)
    if getattr(args, "normal_full_high_vol", None) is not None:
        cfg.normal_full_high_vol_threshold = float(args.normal_full_high_vol)
    if getattr(args, "normal_full_down_strength", None) is not None:
        cfg.normal_full_down_strength_threshold = float(args.normal_full_down_strength)
    if getattr(args, "normal_full_up_score", None) is not None:
        cfg.normal_full_up_score_threshold = float(args.normal_full_up_score)
    if getattr(args, "normal_full_p20", None) is not None:
        cfg.normal_full_p20_threshold = float(args.normal_full_p20)
    if getattr(args, "normal_full_stock", None) is not None:
        cfg.normal_full_stock_weight = float(args.normal_full_stock)
    if getattr(args, "normal_full_force_rebalance", False):
        cfg.normal_full_force_rebalance = True
    ...
    if getattr(args, "enable_custom_lite", False):
        cfg.enable_custom_lite = True
    if getattr(args, "custom_lite_high_vol", None) is not None:
        cfg.custom_lite_high_vol_threshold = float(args.custom_lite_high_vol)
    if getattr(args, "custom_lite_up_score", None) is not None:
        cfg.custom_lite_up_score_threshold = float(args.custom_lite_up_score)
    if getattr(args, "custom_lite_p20", None) is not None:
        cfg.custom_lite_p20_threshold = float(args.custom_lite_p20)
    if getattr(args, "custom_lite_down_strength", None) is not None:
        cfg.custom_lite_down_strength_threshold = float(args.custom_lite_down_strength)
    if getattr(args, "custom_lite_stock", None) is not None:
        cfg.custom_lite_stock_weight = float(args.custom_lite_stock)
    if getattr(args, "custom_lite_no_p20", False):
        cfg.custom_lite_require_p20 = False
    if getattr(args, "custom_lite_exclude_bear_trend", False):
        cfg.custom_lite_exclude_bear_trend = True
    if getattr(args, "custom_lite_force_rebalance", False):
        cfg.custom_lite_force_rebalance = True
    if getattr(args, "custom_lite_allow_tier1", False):
        cfg.custom_lite_allow_tier1 = True
    if getattr(args, "custom_lite_allow_offensive_overlap", False):
        cfg.custom_lite_require_no_offensive = False
    if getattr(args, "enable_tier1_boost", False):
        cfg.enable_tier1_boost = True
    if getattr(args, "tier1_boost_high_vol", None) is not None:
        cfg.tier1_boost_high_vol_threshold = float(args.tier1_boost_high_vol)
    if getattr(args, "tier1_boost_down_strength", None) is not None:
        cfg.tier1_boost_down_strength_threshold = float(args.tier1_boost_down_strength)
    if getattr(args, "tier1_boost_stock", None) is not None:
        cfg.tier1_boost_stock_weight = float(args.tier1_boost_stock)
    if getattr(args, "tier1_boost_allow_tier2", False):
        cfg.tier1_boost_require_no_tier2 = False
    if getattr(args, "tier1_boost_allow_full_tier3", False):
        cfg.tier1_boost_exclude_full_tier3 = False
    if getattr(args, "tier1_boost_force_rebalance", False):
        cfg.tier1_boost_force_rebalance = True

    if getattr(args, "disable_trend_participation", False):
        cfg.enable_trend_participation_overlay = False
    if getattr(args, "trend_sharpe60", None) is not None:
        cfg.trend_sharpe60_threshold = float(args.trend_sharpe60)
    if getattr(args, "trend_sharpe120", None) is not None:
        cfg.trend_sharpe120_threshold = float(args.trend_sharpe120)
    if getattr(args, "trend_positive_ratio60", None) is not None:
        cfg.trend_positive_ratio_60_threshold = float(args.trend_positive_ratio60)
    if getattr(args, "trend_max_high_vol", None) is not None:
        cfg.trend_max_high_vol_for_overlay = float(args.trend_max_high_vol)
    if getattr(args, "trend_floor_lt25", None) is not None:
        cfg.trend_floor_lt25 = float(args.trend_floor_lt25)
    if getattr(args, "trend_floor_lt35", None) is not None:
        cfg.trend_floor_lt35 = float(args.trend_floor_lt35)
    if getattr(args, "trend_floor_lt50", None) is not None:
        cfg.trend_floor_lt50 = float(args.trend_floor_lt50)
    if getattr(args, "trend_floor_lt65", None) is not None:
        cfg.trend_floor_lt65 = float(args.trend_floor_lt65)
    if getattr(args, "trend_full_stock_low_vol", False):
        cfg.trend_full_stock_when_low_vol = True
    if getattr(args, "trend_force_rebalance", False):
        cfg.trend_force_rebalance = True

    if getattr(args, "disable_recovery_rerisk", False):
        cfg.enable_recovery_rerisk_overlay = False
    if getattr(args, "recovery_dd60", None) is not None:
        cfg.recovery_dd60_threshold = float(args.recovery_dd60)
    if getattr(args, "recovery_ret10", None) is not None:
        cfg.recovery_return10_threshold = float(args.recovery_ret10)
    if getattr(args, "recovery_ret20", None) is not None:
        cfg.recovery_return20_threshold = float(args.recovery_ret20)
    if getattr(args, "recovery_stock_floor", None) is not None:
        cfg.recovery_stock_floor = float(args.recovery_stock_floor)
    if getattr(args, "recovery_strong_stock_floor", None) is not None:
        cfg.recovery_strong_stock_floor = float(args.recovery_strong_stock_floor)
    if getattr(args, "recovery_no_force_rebalance", False):
        cfg.recovery_force_rebalance = False

    if getattr(args, "enable_drawdown_guard", False):
        cfg.enable_soft_drawdown_guard = True
    if getattr(args, "drawdown_guard_window", None) is not None:
        cfg.drawdown_guard_window = int(args.drawdown_guard_window)
    if getattr(args, "drawdown_guard_threshold_1", None) is not None:
        cfg.drawdown_guard_threshold_1 = float(args.drawdown_guard_threshold_1)
    if getattr(args, "drawdown_guard_threshold_2", None) is not None:
        cfg.drawdown_guard_threshold_2 = float(args.drawdown_guard_threshold_2)
    if getattr(args, "drawdown_guard_high_vol_min", None) is not None:
        cfg.drawdown_guard_high_vol_min = float(args.drawdown_guard_high_vol_min)
    if getattr(args, "drawdown_guard_force_rebalance", False):
        cfg.drawdown_guard_force_rebalance = True
    ...
    file_prefix = f"{safe_ticker}_xgb_trend_participation_v8_7_0"
