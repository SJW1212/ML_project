"""
XGBoost v8.6.28 - Volatility-Base Strong-Override No-Sideway Strength Allocation
====================================================================
...
실행 예시
    py xgb_vol_base_down_strength_term_v8_6_28.py --speed-profile balanced --h10-down-only --condition-search
    py xgb_vol_base_down_strength_term_v8_6_28.py --speed-profile fast --h10-down-only
    py xgb_vol_base_down_strength_term_v8_6_28.py --speed-profile full --adaptive-label --h10-down-only
...
class Config:
    ...
    result_dir: str = "results_xgb_vol_base_down_strength_term_v8_6_28"
    ...
    # v8.6.28 base allocation: 기본 상태는 high-vol 확률 기반 연속/계단형 비중으로 산출한다.
    ...
    # v8.6.28 execution: Tier3/Full은 schedule/no-trade-band에 묶이지 않고 즉시 목표 비중으로 점프한다.
    ...
    # v8.6.28 Volatility-Base Strong-Override Multi-Horizon Upside Strength Trigger
    ...
    # v8.6.28: prob_bear_down_strengthening 제거. 하락 강화도 상승 강화와 동일하게 5D/10D/20D term structure로 계산한다.
    ...
    up_strength_bonus_threshold_2: float = 0.38  # v8.6.28: Tier2 재활성화
    ...
    up_strength_pred_threshold_5d: float = 0.99  # v8.6.28: 5D 단독 trigger 비활성화
    ...
    # v8.6.28: 5D는 allocation trigger에서 제외하고, 10D는 보조 확인, 20D는 핵심 조건으로 사용한다.
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            if bool(getattr(cfg, "use_direction_strength_specialist", True)):
                try:
                    ...
                    # v8.6.28: 별도 bear-filter specialist 제거.
        ...
        if bool(getattr(cfg, "use_direction_strength_specialist", True)):
            ...
            bear_strength_probs = {name: 0.0 for name in DIRECTION_STRENGTH_LABELS}  # removed in v8.6.28
        else:
        ...
        out.update({
            ...
            **{f"prob_up_weakening_{int(sh)}d": 0.0 for sh in multi_strength_horizons},
            # v8.6.28: 하락 강화도 상승 강화와 동일한 horizon별 term structure로 산출한다.
...
def apply_direction_strength_specialist_policy(
    ...
    """
    v8.6.28 Volatility-Base Strong-Override Upside Strength Trigger.
    ...
    """
    ph = _row_float(row, "prob_high_vol")
    # v8.6.28: weak 확률(prob_up/prob_down/prob_bear_down_strengthening)은 allocation에서 제거한다.
    ...
    # v8.6.28: 강한 상승 override는 regime bucket보다 실제 high-vol cap을 우선한다.
    ...
    # v8.6.28: bear/down 약한 확률 기반 cut은 제거한다.
...
def classification_metrics(pred_df: pd.DataFrame, cfg: Config) -> Dict[str, object]:
    ...
    # v8.6.28: prob_up/prob_down은 약한 출력으로 판정되어 summary 핵심 성능에서 제외한다.
    ...
    if "actual_direction_strength" in pred_df.columns:
        ...
        # v8.6.28: prob_bear_down_strengthening은 allocation/summary 성능에서 제외한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_vol_base_down_strength_term_v8_6_28_diagnostics",
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_vol_base_down_strength_term_v8_6_28_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_vol_base_down_strength_term_v8_6_28_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_vol_base_down_strength_term_v8_6_28_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.28 Volatility-Base Strong-Override Multi-Horizon Upside Strength Trigger + Objective Condition Search")
    ...
    parser.add_argument("--rebalance-every", type=int, default=None, help="리밸런싱 주기 직접 지정. v8.6.28 기본 5거래일")
    parser.add_argument("--emergency-cooldown", type=int, default=None, help="긴급 리밸런싱 cooldown 직접 지정. v8.6.28 기본 5거래일")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.28 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.28 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.28 기본 ON인 stale offensive decay 비활성화")
    ...
    parser.add_argument("--no-horizon-train-window", action="store_true", help="v8.6.28 horizon별 train_rows = horizon*multiplier 모드 비활성화")
    ...
    parser.add_argument("--disable-vol-base-allocation", action="store_true", help="v8.6.28 high-vol 확률 기반 기본 비중을 끄고 regime bucket 기본 비중 사용")
    ...
    parser.add_argument("--disable-tier2", action="store_true", help="v8.6.28 기본 ON인 Tier2를 비활성화")
...
def drop_weak_probability_output_columns(pred_df: pd.DataFrame) -> pd.DataFrame:
    """
    v8.6.28 external-output filter.
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_vol_base_down_strength_term_v8_6_28_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_vol_base_down_strength_term_v8_6_28_summary.json"
    ...
    cfg.result_dir = f"results_xgb_vol_base_down_strength_term_v8_6_28_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.28: 기본 상태는 prob_high_vol 기반 포트폴리오, 강한 신호만 즉시 override한다.
        ...
        # v8.6.28: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
    ...
    file_prefix = f"{safe_ticker}_xgb_vol_base_down_strength_term_v8_6_28"
