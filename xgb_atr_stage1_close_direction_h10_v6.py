"""
XGBoost ATR-aware Risk-first Model v6-H10 - Stage1 + Close-direction Stage2
===========================================================================

목적
- QQQ 가격/거래량/변동성 피처 기반으로 향후 10거래일 고변동 위험을 예측
- 1단계: 정상 vs 고변동
- 2단계: 실제 고변동 구간만 대상으로 10거래일 뒤 종가 기준 상승/하락 방향 예측
- 상승고변동/하락고변동 라벨은 최소 수익률 threshold 없이 future_return_N >= 0 기준으로 단순화
- 최종 확률은 P(고변동) × P(상승/하락 | 고변동) 형태로 계산
- 포트폴리오 배분은 prob_high_vol, prob_up_high_vol, prob_down_high_vol 기반으로 계산

실행
    py xgb_atr_stage1_close_direction_h10_v6.py

필요 패키지
    pip install pandas numpy yfinance scikit-learn xgboost

주의
- 라벨 생성에는 미래 10거래일 수익률/변동성/최대상승/최대하락을 사용합니다.
...
class Config:
    ...
    down_hard_threshold: float = 0.25
    down_extreme_threshold: float = 0.35
    ...
    no_trade_band: float = 0.07
    max_daily_weight_change: float = 0.08
    pred_high_vol_threshold: float = 0.50
    pred_down_high_vol_threshold: float = 0.50  # conditional P(하락 | 고변동) 기준. threshold 제거 방침에 따라 0.5 사용
    pred_up_high_vol_threshold: float = 0.20
    rebalance_every_n_days: int = 10

    # emergency rebalance thresholds for horizon-10 version
    emergency_down_threshold: float = 0.30
    emergency_combined_down_threshold: float = 0.20
    emergency_combined_high_vol_threshold: float = 0.50
    emergency_high_vol_threshold: float = 0.70
    ...
    opt_turnover_weight: float = 0.50
    opt_target_avg_stock_weight: float = 0.76
    opt_low_stock_penalty_weight: float = 0.40

    result_dir: str = "results_xgb_atr_stage1_close_direction_h10_v6"
...
def assign_atr_aware_label(row: pd.Series, th: Dict[str, float]) -> str:
    ...
    1. 먼저 기존 ATR-aware 규칙으로 고변동 여부만 판정한다.
    2. 고변동이 아니면 "정상".
    3. 고변동이면 N일 뒤 종가 수익률 future_return_20d 기준으로만 방향을 정한다.
       - future_return_20d >= 0: 상승고변동
       - future_return_20d < 0 : 하락고변동

    주의:
    - 컬럼명은 기존 코드 호환을 위해 future_return_20d를 유지하지만,
      실제 horizon은 CFG.risk_horizon_days입니다. v6 기본값은 10거래일입니다.
    - 상승/하락 방향에는 최소 수익률 threshold를 두지 않습니다.
    """
    ...
    # 고변동 판정 threshold만 ATR regime으로 조정한다.
    ...
    # v6 핵심: 고변동 내부 방향은 N일 뒤 종가 수익률 부호만 사용한다.
    if future_ret >= 0:
        return "상승고변동"
    return "하락고변동"
...
def build_allocation_grid(cfg: Config) -> List[Dict[str, float]]:
    """
    v4 배분 grid.
    Stage2 방향 모델은 쓰지 않고, Stage1 고변동 확률과 Down-risk OVR 확률만 사용한다.
    turnover를 낮추기 위해 후보 수를 제한하고, 리밸런싱은 별도 로직에서 5거래일 단위로 처리한다.
    """
    grid: List[Dict[str, float]] = []
    base_list = [0.90, 0.91, 0.92]
    max_stock_list = [0.90, 0.92]
    high_weight_list = [0.60, 0.70, 0.80]
    down_weight_list = [0.40, 0.50, 0.60]
    no_trade_list = [0.06, 0.07, 0.08]
            ...
            for high_w in high_weight_list:
                for down_w in down_weight_list:
                    for ntb in no_trade_list:
                        grid.append({
                            "name": f"xgb_h10_v6_close_dir_cfg_{idx:04d}",
                            ...
                            "down_extreme_threshold": cfg.down_extreme_threshold,
                            "soft_stock_cap": cfg.soft_stock_cap,
                            "hard_stock_cap": cfg.hard_stock_cap,
                            "extreme_stock_cap": cfg.extreme_stock_cap,
...
def default_alloc_config(cfg: Config) -> Dict[str, float]:
    return {
        "name": "default_xgb_atr_stage1_close_direction_h10_v6",
...
def nonlinear_risk_penalty(risk_score: float) -> float:
    """v4: 과도한 방어를 막기 위해 v2보다 완만한 penalty를 사용한다."""
    rs = float(np.clip(risk_score, 0.0, 1.0))
    if rs < 0.25:
        return 0.18 * rs
    if rs < 0.50:
        return 0.045 + 0.55 * (rs - 0.25)
    return 0.1825 + 0.90 * (rs - 0.50)
...
def allocate_from_probs(
    ...
    핵심:
    - Stage2 상승/하락 방향 분류를 제거한다.
    - prob_high_vol: 전체 고변동 위험
    - prob_down_risk: 하락고변동 one-vs-rest 확률
    - prob_up_proxy: max(prob_high_vol - prob_down_risk, 0) 형태의 약한 기회 보정값
    - 하락위험은 multiplier가 아니라 cap으로 제한한다.
    """
    ph = float(np.clip(prob_high_vol, 0.0, 1.0))
    pu = float(np.clip(prob_up_proxy, 0.0, 1.0))
    ...
    stock = cfg_dict["base_stock_weight"] - risk_penalty + cfg_dict["opportunity_weight"] * opportunity_score
    stock = float(np.clip(stock, cfg_dict["min_stock_weight"], cfg_dict["max_stock_weight"]))

    # 하락위험이 큰 구간만 주식 비중 상한을 제한한다.
    if pdn >= cfg_dict["down_extreme_threshold"]:
        stock = min(stock, cfg_dict["extreme_stock_cap"])
    elif pdn >= cfg_dict["down_hard_threshold"]:
        stock = min(stock, cfg_dict["hard_stock_cap"])
    elif pdn >= cfg_dict["down_soft_threshold"]:
        stock = min(stock, cfg_dict["soft_stock_cap"])

    # turnover 제어: 작은 차이는 거래하지 않고, 큰 차이만 완만하게 이동한다.
    if prev_weights is not None:
        prev_stock = float(prev_weights[0])
        if abs(stock - prev_stock) < cfg_dict["no_trade_band"]:
            stock = prev_stock
        else:
            max_chg = cfg_dict.get("max_daily_weight_change", None)
            if max_chg is not None and max_chg > 0:
                stock = float(np.clip(stock, prev_stock - max_chg, prev_stock + max_chg))
...
def apply_rolling_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        emergency_rebalance = (
            prob_down_risk_now >= cfg.emergency_down_threshold
            or (
                prob_down_risk_now >= cfg.emergency_combined_down_threshold
                and prob_high_vol_now >= cfg.emergency_combined_high_vol_threshold
            )
            or prob_high_vol_now >= cfg.emergency_high_vol_threshold
        )
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            # Stage2: 실제 고변동 구간만 대상으로 N일 뒤 종가 방향 학습
            # 상승고변동=1, 하락고변동=0
            stage2_mask = labels != "정상"
            stage2_labels = labels.loc[stage2_mask]
            X_stage2 = train_df.loc[stage2_mask, feature_cols]
            y_stage2 = (stage2_labels == "상승고변동").astype(int).values
            up_count = int(np.sum(y_stage2 == 1))
            down_count = int(np.sum(y_stage2 == 0))
            if len(np.unique(y_stage2)) == 2 and up_count >= 20 and down_count >= 20:
                spw_stage2 = calc_scale_pos_weight(y_stage2)
                downrisk_model = make_xgb_stage2(cfg, spw_stage2)
                downrisk_model.fit(X_stage2, y_stage2)
        ...
        if downrisk_model is not None and downrisk_available:
            # Stage2 positive class = 상승고변동
            prob_up_given_hv = float(downrisk_model.predict_proba(X_now)[0, 1])
        else:
            prob_up_given_hv = 0.5

        prob_up_given_hv = float(np.clip(prob_up_given_hv, 0.0, 1.0))
        prob_down_given_hv = float(1.0 - prob_up_given_hv)
        prob_up_hv = float(np.clip(prob_high * prob_up_given_hv, 0.0, 1.0))
        prob_down_hv = float(np.clip(prob_high * prob_down_given_hv, 0.0, 1.0))
        prob_down_risk = prob_down_hv  # allocation 호환용 alias: joint P(하락고변동)
        prob_up_proxy = prob_up_hv
        ...
        if pred_risk != "고변동":
            ...
            pred_split = "하락고변동" if prob_down_given_hv >= cfg.pred_down_high_vol_threshold else "상승고변동"
        ...
        prediction_rows.append({
            ...
            "prob_up_given_high_vol": prob_up_given_hv,
            "prob_down_given_high_vol": prob_down_given_hv,
    ...
    if cfg.use_prob_ewma:
        for col in ["prob_high_vol", "prob_up_given_high_vol"]:
        ...
        pred_df["prob_up_given_high_vol"] = pred_df["prob_up_given_high_vol"].clip(0.0, 1.0)
        pred_df["prob_down_given_high_vol"] = (1.0 - pred_df["prob_up_given_high_vol"]).clip(0.0, 1.0)
        pred_df["prob_normal"] = 1.0 - pred_df["prob_high_vol"]
        pred_df["prob_up_high_vol"] = (pred_df["prob_high_vol"] * pred_df["prob_up_given_high_vol"]).clip(0.0, 1.0)
        pred_df["prob_down_high_vol"] = (pred_df["prob_high_vol"] * pred_df["prob_down_given_high_vol"]).clip(0.0, 1.0)
        pred_df["prob_down_risk"] = pred_df["prob_down_high_vol"]
        pred_df["prob_up_proxy"] = pred_df["prob_up_high_vol"]
    ...
    pred_df["pred_split_vol"] = np.where(
        ...
        "정상",
        np.where(pred_df["prob_down_given_high_vol"] >= cfg.pred_down_high_vol_threshold, "하락고변동", "상승고변동"),
    )

    pred_df.attrs["stage1_feature_importance_mean"] = mean_importance(stage1_importance_history)
    pred_df.attrs["stage2_feature_importance_mean"] = mean_importance(downrisk_importance_history)
    pred_df.attrs["downrisk_feature_importance_mean"] = pred_df.attrs["stage2_feature_importance_mean"]
...
def classification_metrics(pred_df: pd.DataFrame) -> Dict[str, object]:
    ...
    1. Stage1 Risk Classification
       - 전체 구간 대상
       - 정상 / 고변동 이진 분류

    2. Stage2 Close-direction Classification
       - 실제 고변동 구간만 대상
       - 상승고변동 / 하락고변동 이진 분류
       - 방향 기준은 N일 뒤 종가 수익률 부호

    3. Final Cascade 3-state Classification
       - 전체 구간 대상
    ...
    # Stage2: 실제 고변동 구간만 평가
    hv_mask = pred_df["actual_risk"].eq("고변동")
    stage2_df = pred_df.loc[hv_mask].copy()
    dir_labels = ["상승고변동", "하락고변동"]

    y_true_dir = stage2_df["actual_split_vol"].values
    y_pred_dir = np.where(
        stage2_df["prob_down_given_high_vol"].astype(float).values >= CFG.pred_down_high_vol_threshold,
        "하락고변동",
        "상승고변동",
    )
    p_up_given = stage2_df["prob_up_given_high_vol"].astype(float).clip(0.0, 1.0).values
    p_down_given = stage2_df["prob_down_given_high_vol"].astype(float).clip(0.0, 1.0).values
    ...
    stage2 = {
        ...
        "evaluation_scope": "true high-vol rows only",
        "direction_rule": "future_return_N >= 0 => 상승고변동, future_return_N < 0 => 하락고변동",
        "decision_threshold": float(CFG.pred_down_high_vol_threshold),
        "accuracy": float(accuracy_score(y_true_dir, y_pred_dir)) if len(stage2_df) else 0.0,
        "macro_f1": float(f1_score(y_true_dir, y_pred_dir, labels=dir_labels, average="macro", zero_division=0)) if len(stage2_df) else 0.0,
        "label_support": {lab: int((stage2_df["actual_split_vol"] == lab).sum()) for lab in dir_labels},
        "per_class": {},
        "report": stage2_report,
    }
    for lab in dir_labels:
        stage2["per_class"][lab] = {
            "precision": float(stage2_report.get(lab, {}).get("precision", 0.0)),
            "recall": float(stage2_report.get(lab, {}).get("recall", 0.0)),
            "f1": float(stage2_report.get(lab, {}).get("f1-score", 0.0)),
            "support": int(stage2_report.get(lab, {}).get("support", 0)),
        }

    y_up_true = (stage2_df["actual_split_vol"] == "상승고변동").astype(int).values
    y_down_true = (stage2_df["actual_split_vol"] == "하락고변동").astype(int).values
    if len(stage2_df) and len(np.unique(y_up_true)) == 2:
        stage2["up_high_vol_roc_auc"] = float(roc_auc_score(y_up_true, p_up_given))
        stage2["up_high_vol_pr_auc"] = float(average_precision_score(y_up_true, p_up_given))
    else:
        stage2["up_high_vol_roc_auc"] = None
        stage2["up_high_vol_pr_auc"] = None
    if len(stage2_df) and len(np.unique(y_down_true)) == 2:
        stage2["down_high_vol_roc_auc"] = float(roc_auc_score(y_down_true, p_down_given))
        stage2["down_high_vol_pr_auc"] = float(average_precision_score(y_down_true, p_down_given))
    else:
        stage2["down_high_vol_roc_auc"] = None
        stage2["down_high_vol_pr_auc"] = None

    # 조건부 하락 방향 threshold 진단: 실제 고변동 구간만
    down_diags = []
    for t in [0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70]:
        pred = np.where(p_down_given >= t, "하락고변동", "상승고변동")
        down_diags.append({
            "threshold": float(t),
            "basis": "prob_down_given_high_vol on true high-vol rows",
            "down_high_vol_precision": float(precision_score(y_down_true, (pred == "하락고변동").astype(int), zero_division=0)) if len(stage2_df) else 0.0,
            "down_high_vol_recall": float(recall_score(y_down_true, (pred == "하락고변동").astype(int), zero_division=0)) if len(stage2_df) else 0.0,
            "down_high_vol_f1": float(f1_score(y_down_true, (pred == "하락고변동").astype(int), zero_division=0)) if len(stage2_df) else 0.0,
            "pred_down_high_vol_ratio": float((pred == "하락고변동").mean()) if len(stage2_df) else 0.0,
        })
    ...
    final_cascade = {
        ...
        "macro_f1": float(f1_score(y_true_final, y_pred_final, labels=final_labels, average="macro", zero_division=0)),
        "report": final_report,
    ...
    if len(np.unique(y_down_final)) == 2:
        final_cascade["down_high_vol_roc_auc"] = float(roc_auc_score(y_down_final, pred_df["prob_down_high_vol"].values))
        final_cascade["down_high_vol_pr_auc"] = float(average_precision_score(y_down_final, pred_df["prob_down_high_vol"].values))
    else:
    ...
    return {
        ...
        "stage2_close_direction_classification_on_true_high_vol": stage2,
        "final_cascade_3state_classification": final_cascade,

        # 호환용 alias
        "stage2_direction_classification_on_true_high_vol": stage2,
        "split_vol_classification": final_cascade,
        "down_high_vol_threshold_diagnostics": down_diags,
        "stage2_down_high_vol_threshold_diagnostics_on_true_high_vol": down_diags,
    }
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], cfg_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    summary = {
        ...
        "model_type": "xgb_atr_stage1_close_direction_risk_first_h10_v6",
        "config": asdict(cfg),
        "period": {
        ...
        "downrisk_feature_importance_mean": pred_df.attrs.get("stage2_feature_importance_mean", {}),
        ...
        "label_design": {
            ...
            "stage1": "정상 / 고변동",
            "stage2": "실제 고변동 구간에서 N일 뒤 종가 기준 상승고변동 / 하락고변동",
            "direction_rule": "future_return_N >= 0이면 상승고변동, future_return_N < 0이면 하락고변동. 최소 수익률 threshold 없음",
            ...
            "stage2_training_rule": "actual high-vol rows only; 상승고변동=1, 하락고변동=0",
            "atr_aware": True,
            "down_priority": False,
        },
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    st2 = summary["stage2_close_direction_classification_on_true_high_vol"]
    ...
    print("\n==============================")
    print("XGBoost ATR-aware H10 v6 결과 요약")
    print("Stage1 + Close-direction Stage2 + Final Cascade")
    print("==============================")
    ...
    print("\n[2단계 Close-direction 분류 성능 - 실제 고변동 구간만 평가]")
    print(f"Rows: {st2['rows']}")
    print(f"Decision threshold P(하락|고변동): {st2['decision_threshold']}")
    print(f"Accuracy: {st2['accuracy']:.6f}")
    print(f"Macro F1: {st2['macro_f1']:.6f}")
    print(f"Up-high-vol ROC-AUC: {st2['up_high_vol_roc_auc']}")
    print(f"Up-high-vol PR-AUC: {st2['up_high_vol_pr_auc']}")
    print(f"Down-high-vol ROC-AUC: {st2['down_high_vol_roc_auc']}")
    print(f"Down-high-vol PR-AUC: {st2['down_high_vol_pr_auc']}")
    print(f"Label support: {st2['label_support']}")
    for lab, m in st2["per_class"].items():
        print(f"  - {lab}: P={m['precision']:.6f}, R={m['recall']:.6f}, F1={m['f1']:.6f}, support={m['support']}")
...
def main() -> None:
    ...
    print("[3] Walk-forward XGBoost Stage1 + Close-direction Stage2 예측")
    ...
    pred_path = result_dir / "qqq_xgb_atr_stage1_close_direction_h10_v6_predictions.csv"
    summary_path = result_dir / "qqq_xgb_atr_stage1_close_direction_h10_v6_summary.json"
    latest_path = result_dir / "qqq_xgb_atr_stage1_close_direction_h10_v6_latest.json"
