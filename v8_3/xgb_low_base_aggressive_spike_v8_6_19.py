"""
XGBoost v8.6.19 - Low-Base Aggressive Spike Multi-Horizon Upside Strength Allocation
====================================================================
...
class Config:
    ...
    extreme_stock_weight: float = 0.25
    extreme_bond_weight: float = 0.45
    extreme_cash_weight: float = 0.30

    # Base bucket allocations: low-base regime
    normal_stock_weight: float = 0.60
    normal_bond_weight: float = 0.25
    normal_cash_weight: float = 0.15

    watch_stock_weight: float = 0.50
    watch_bond_weight: float = 0.30
    watch_cash_weight: float = 0.20

    high_vol_stock_weight: float = 0.45
    high_vol_bond_weight: float = 0.35
    high_vol_cash_weight: float = 0.20

    risk_off_stock_weight: float = 0.38
    risk_off_bond_weight: float = 0.37
    risk_off_cash_weight: float = 0.25
    ...
    result_dir: str = "results_xgb_low_base_aggressive_spike_v8_6_19"
    ...
    # v8.6.19 Low-Base Aggressive Spike Multi-Horizon Upside Strength Trigger
    ...
    # v8.6.19 controlled consensus thresholds
    ...
    up_strength_single_20d_stock_weight: float = 0.85
    up_strength_pair_5d_10d_stock_weight: float = 0.90
    up_strength_pair_5d_20d_stock_weight: float = 0.92
    up_strength_pair_10d_20d_stock_weight: float = 0.95
    ...
    # v8.6.19: 100% 진입은 매우 강한 5D/10D/20D 합의에만 허용한다.
    up_strength_offensive_stock_weight_1: float = 0.80
    up_strength_offensive_stock_weight_2: float = 0.90
    ...
    # v8.6.19 diagnostics / stale offensive de-risking
...
def ensemble_weights(cfg: Config) -> Tuple[Dict[int, float], Dict[int, float]]:
    # v8.6.19에서 cfg.horizons에는 UP_STRENGTHENING 5D 라벨 생성을 위해 5가 포함될 수 있다.
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    """
    v8.6.19: allocation gate에서 사용할 하락 위험 점수.
...
def build_direction_strength_labels(df: pd.DataFrame, cfg: Config, horizon: Optional[int] = None) -> Tuple[pd.Series, pd.Series, pd.DataFrame]:
    ...
    v8.6.19에서는 5D/10D/20D specialist를 각각 학습하므로 horizon 인자를 선택적으로 받는다.
...
def apply_direction_strength_specialist_policy(
    ...
    - 평상시 base stock은 낮게 유지한다.
    - 5D/10D/20D 상승 조짐이 보이면 80% / 90% / 100%로 빠르게 점프한다.
    - 회전율 증가는 허용하되, 위험 regime에서는 공격 전환을 차단한다.
    ...
    # v8.6.19: 5D는 더 엄격하게, 20D는 유지해 중기 신호를 우선한다.
    ...
    if regime in {"NORMAL", "WATCH"} and pbd < block_bear:
        ...
        if consensus_count == 1 and ph < low_vol_1:
            if pred20:
                ...
                # 5D 단독은 약한 상승 조짐으로 보고 80%까지만 빠르게 올린다.
                offensive_tier = max(offensive_tier, 1)
        ...
        # v8.6.19에서는 합의 조건이 있는 경우에만 score를 보조로 사용한다.
        ...
        if prob_spread >= 0.40:
            # Low-base spike에서는 신호가 맞으면 빠르게 진입해야 하므로 spread penalty는 약하게 둔다.
        ...
        # 5D 단독은 80%까지만 허용한다. 낮은 기본 비중에서는 이 자체가 약한 spike 신호다.
        if five_day_only:
            target_stock_from_consensus = min(target_stock_from_consensus, max(base_stock, float(getattr(cfg, "up_strength_single_5d_stock_weight", 0.80))))
    ...
    meta = {
        ...
        "policy_note": "low_base_aggressive_spike_multi_horizon_upside_strength_stage1_risk_gate",
    }
    return w, meta
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        if prev_w is None:
            ...
            # v8.6.19 optional: offensive target이 사라졌는데 이전 98~100% 비중이 과도하게 남는 문제를 실험적으로 제어한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_low_base_aggressive_spike_v8_6_19_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.6.19 Low-Base Aggressive Spike Diagnostics 결과 요약")
...
def make_condition_candidate_configs(base_cfg: Config, grid_size: str = "standard") -> List[Tuple[str, Config]]:
    ...
    if grid_size == "compact":
        ...
        # v8.6.19 standard: v8.6.10의 과도한 방어/turnover를 줄이는 공격형 후보군
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_low_base_aggressive_spike_v8_6_19_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_low_base_aggressive_spike_v8_6_19_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_low_base_aggressive_spike_v8_6_19_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.19 Low-Base Aggressive Spike Multi-Horizon Upside Strength Trigger + Objective Condition Search")
    ...
    parser.add_argument("--rebalance-every", type=int, default=None, help="리밸런싱 주기 직접 지정. v8.6.19 기본 5거래일")
    parser.add_argument("--emergency-cooldown", type=int, default=None, help="긴급 리밸런싱 cooldown 직접 지정. v8.6.19 기본 5거래일")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.19 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.19 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.19 기본 ON인 stale offensive decay 비활성화")
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_low_base_aggressive_spike_v8_6_19_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_low_base_aggressive_spike_v8_6_19_summary.json"
    ...
    cfg.result_dir = f"results_xgb_low_base_aggressive_spike_v8_6_19_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        cfg.no_trade_band = min(float(getattr(cfg, "no_trade_band", 0.10)), 0.10)
        cfg.emergency_cooldown_days = min(int(getattr(cfg, "emergency_cooldown_days", 5)), 5)
        # v8.6.19: 기본 주식 비중을 낮게 유지하고, 상승 강화 신호에서만 80/90/100%로 급격히 올린다.
        cfg.normal_stock_weight = 0.60
        cfg.normal_bond_weight = 0.25
        cfg.normal_cash_weight = 0.15
        cfg.watch_stock_weight = 0.50
        cfg.watch_bond_weight = 0.30
        cfg.watch_cash_weight = 0.20
        cfg.high_vol_stock_weight = 0.45
        cfg.high_vol_bond_weight = 0.35
        cfg.high_vol_cash_weight = 0.20
        cfg.risk_off_stock_weight = 0.38
        cfg.risk_off_bond_weight = 0.37
        cfg.risk_off_cash_weight = 0.25
        cfg.extreme_high_vol_threshold = 0.86
        cfg.extreme_downrisk_threshold = 0.86
        cfg.extreme_stock_weight = 0.25
        cfg.extreme_bond_weight = 0.45
        cfg.extreme_cash_weight = 0.30
        ...
        cfg.direction_strength_max_stock_bonus = max(float(getattr(cfg, "direction_strength_max_stock_bonus", 0.75)), 0.75)
        cfg.enable_stale_offensive_decay = bool(getattr(cfg, "enable_stale_offensive_decay", True))
        # v8.6.19: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
    ...
    # v8.6.19 direct low-base overrides after policy defaults.
    if getattr(args, "base_normal_stock", None) is not None:
        cfg.normal_stock_weight = float(args.base_normal_stock)
        rem = max(0.0, 1.0 - cfg.normal_stock_weight); cfg.normal_bond_weight = rem * 2/3; cfg.normal_cash_weight = rem * 1/3
    if getattr(args, "base_watch_stock", None) is not None:
    ...
    file_prefix = f"{safe_ticker}_xgb_low_base_aggressive_spike_v8_6_19"
