"""
XGBoost v8.6.38 - Volatility-Base + ShortMidConfirm
====================================================================

목적
- QQQ/IEF/BIL 동적 자산배분 전략
- v8.6.38: 5D+10D+high-vol ShortMidConfirm 보조 신호 추가
...
실행 예시
    py xgb_normal_unrestricted_v8_6_38.py --speed-profile balanced --h10-down-only
    py xgb_normal_unrestricted_v8_6_38.py --speed-profile fast --h10-down-only
    py xgb_normal_unrestricted_v8_6_38.py --speed-profile full --adaptive-label --h10-down-only
...
class Config:
    ...
    result_dir: str = "results_xgb_normal_unrestricted_v8_6_38"
    ...
    # v8.6.38 base allocation: 기본 상태는 high-vol 확률 기반 연속/계단형 비중으로 산출한다.
    ...
    # v8.6.38 Normal-Unrestricted Base
    ...
    # v8.6.38 execution: Tier3/Full은 schedule/no-trade-band에 묶이지 않고 즉시 목표 비중으로 점프한다.
    ...
    # v8.6.38 Separate PortfolioPolicyModel
    ...
    # v8.6.38 Volatility-Base Strong-Override Multi-Horizon Upside Strength Trigger
    ...
    # v8.6.38: prob_bear_down_strengthening 제거. 하락 강화도 상승 강화와 동일하게 5D/10D/20D term structure로 계산한다.
    ...
    horizon_train_multiplier_5d: float = 50.4   # v8.6.38 default: H5=252 rows
    horizon_train_multiplier_10d: float = 50.4  # v8.6.38 default: H10=504 rows
    horizon_train_multiplier_20d: float = 37.8  # v8.6.38 default: H20=756 rows
    horizon_train_preset: str = "moderate_1y_2y_3y"
    ...
    up_strength_bonus_threshold_2: float = 0.38  # v8.6.38: Tier2 활성화 기본값
    ...
    up_strength_pred_threshold_5d: float = 0.99  # v8.6.38: 5D 단독 trigger 비활성화
    ...
    # v8.6.38: 5D는 allocation trigger에서 제외하고, 10D는 보조 확인, 20D는 핵심 조건으로 사용한다.
...
def horizon_train_window_config(cfg: Config) -> Dict[str, object]:
    ...
    return {
        ...
        "min_rows": int(getattr(cfg, "horizon_train_min_rows", 504)),
        "preset": str(getattr(cfg, "horizon_train_preset", "")),
...
def set_horizon_train_rows_exact(cfg: Config, h5_rows: int, h10_rows: int, h20_rows: int, *, preset_name: str = "custom") -> Config:
    """Set exact horizon-specific training windows.

    Existing formula: rows = max(horizon_train_min_rows, horizon * multiplier).
    This helper sets the shared minimum to the smallest requested window and then
    sets each multiplier so H5/H10/H20 resolve to the requested rows.
    """
    h5_rows = int(h5_rows)
    h10_rows = int(h10_rows)
    h20_rows = int(h20_rows)
    if h5_rows <= 0 or h10_rows <= 0 or h20_rows <= 0:
        raise ValueError("horizon train rows must be positive integers")
    cfg.use_horizon_train_window = True
    cfg.direction_strength_use_horizon_train_window = True
    cfg.horizon_train_min_rows = int(min(h5_rows, h10_rows, h20_rows))
    cfg.horizon_train_multiplier_5d = float(h5_rows) / 5.0
    cfg.horizon_train_multiplier_10d = float(h10_rows) / 10.0
    cfg.horizon_train_multiplier_20d = float(h20_rows) / 20.0
    cfg.horizon_train_preset = str(preset_name)
    return cfg


def apply_horizon_train_preset(cfg: Config, preset: str) -> Config:
    """Apply v8.6.38 short train-window presets.

    - current: legacy long windows, H5=750/H10=1000/H20=1260
    - moderate_1y_2y_3y: H5=252/H10=504/H20=756
    - aggressive_6m_1y_2y: H5=126/H10=252/H20=504
    - ultra_3m_6m_1y: H5=63/H10=126/H20=252
    - extreme_4_8_12: H5=20/H10=80/H20=240, i.e. horizon*4/8/12
    """
    name = str(preset or "moderate_1y_2y_3y").strip().lower()
    aliases = {
        "legacy": "current",
        "default": "moderate_1y_2y_3y",
        "moderate": "moderate_1y_2y_3y",
        "aggressive": "aggressive_6m_1y_2y",
        "ultra": "ultra_3m_6m_1y",
        "extreme": "extreme_4_8_12",
        "4_8_12": "extreme_4_8_12",
    }
    name = aliases.get(name, name)
    if name == "current":
        cfg.use_horizon_train_window = True
        cfg.direction_strength_use_horizon_train_window = True
        cfg.horizon_train_min_rows = 504
        cfg.horizon_train_multiplier_5d = 150.0
        cfg.horizon_train_multiplier_10d = 100.0
        cfg.horizon_train_multiplier_20d = 63.0
        cfg.horizon_train_preset = name
        return cfg
    if name == "moderate_1y_2y_3y":
        return set_horizon_train_rows_exact(cfg, 252, 504, 756, preset_name=name)
    if name == "aggressive_6m_1y_2y":
        return set_horizon_train_rows_exact(cfg, 126, 252, 504, preset_name=name)
    if name == "ultra_3m_6m_1y":
        return set_horizon_train_rows_exact(cfg, 63, 126, 252, preset_name=name)
    if name == "extreme_4_8_12":
        return set_horizon_train_rows_exact(cfg, 20, 80, 240, preset_name=name)
    raise ValueError(f"unknown horizon train preset: {preset}")


def parse_horizon_train_rows(rows_text: str) -> Tuple[int, int, int]:
    parts = [x.strip() for x in str(rows_text).split(",") if x.strip()]
    if len(parts) != 3:
        raise ValueError("--horizon-train-rows must be formatted as H5,H10,H20, e.g. 252,504,756")
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            if bool(getattr(cfg, "use_direction_strength_specialist", True)):
                try:
                    ...
                    # v8.6.38: 별도 bear-filter specialist 제거.
        ...
        if bool(getattr(cfg, "use_direction_strength_specialist", True)):
            ...
            bear_strength_probs = {name: 0.0 for name in DIRECTION_STRENGTH_LABELS}  # removed in v8.6.38
        else:
        ...
        out.update({
            ...
            **{f"prob_up_weakening_{int(sh)}d": 0.0 for sh in multi_strength_horizons},
            # v8.6.38: 하락 강화도 상승 강화와 동일한 horizon별 term structure로 산출한다.
...
def apply_direction_strength_specialist_policy(
    ...
    """
    v8.6.38 Volatility-Base Strong-Override Upside Strength Trigger.
    ...
    """
    ph = _row_float(row, "prob_high_vol")
    # v8.6.38: weak 확률(prob_up/prob_down/prob_bear_down_strengthening)은 allocation에서 제거한다.
    ...
    # v8.6.38: 강한 상승 override는 regime bucket보다 실제 high-vol cap을 우선한다.
    ...
    if True:
        ...
        # v8.6.38 Normal-Unrestricted policy.
        ...
        # v8.6.38 ShortMidConfirm optional allocation actions.
    ...
    # v8.6.38: bear/down 약한 확률 기반 cut은 제거한다.
...
def classification_metrics(pred_df: pd.DataFrame, cfg: Config) -> Dict[str, object]:
    ...
    # v8.6.38: prob_up/prob_down은 약한 출력으로 판정되어 summary 핵심 성능에서 제외한다.
    ...
    if "actual_direction_strength" in pred_df.columns:
        ...
        # v8.6.38: prob_bear_down_strengthening은 allocation/summary 성능에서 제외한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_normal_unrestricted_v8_6_38_diagnostics",
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_normal_unrestricted_v8_6_38_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_normal_unrestricted_v8_6_38_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_normal_unrestricted_v8_6_38_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.38 Short-Horizon-Train Normal-Unrestricted Volatility-Base Multi-Horizon Upside Strength Trigger")
    ...
    parser.add_argument("--rebalance-every", type=int, default=None, help="리밸런싱 주기 직접 지정. v8.6.38 기본 5거래일")
    parser.add_argument("--emergency-cooldown", type=int, default=None, help="긴급 리밸런싱 cooldown 직접 지정. v8.6.38 기본 5거래일")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist", "portfolio_model"], default=None, help="v8.6.38 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.38 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.38 기본 ON인 stale offensive decay 비활성화")
    ...
    parser.add_argument("--no-horizon-train-window", action="store_true", help="v8.6.38 horizon별 train_rows = horizon*multiplier 모드 비활성화")
    parser.add_argument("--horizon-train-preset", choices=["current", "moderate_1y_2y_3y", "aggressive_6m_1y_2y", "ultra_3m_6m_1y", "extreme_4_8_12"], default="moderate_1y_2y_3y", help="v8.6.38 horizon별 단축 학습창 preset. 기본: 252/504/756")
    parser.add_argument("--horizon-train-rows", type=str, default=None, help="H5,H10,H20 exact rows. 예: 252,504,756 또는 20,80,240")
    parser.add_argument("--horizon-train-min-rows", type=int, default=None, help="horizon별 학습창 최소 row 수. preset/custom rows 적용 후 직접 override 가능")
    ...
    parser.add_argument("--disable-normal-unrestricted", action="store_true", help="v8.6.38 정상 상태 비중 제한 완화 정책 비활성화")
    ...
    parser.add_argument("--disable-vol-base-allocation", action="store_true", help="v8.6.38 high-vol 확률 기반 기본 비중을 끄고 regime bucket 기본 비중 사용")
    ...
    parser.add_argument("--disable-tier2", action="store_true", help="Tier2 비활성화. v8.6.38 기본값은 활성화")
...
def drop_weak_probability_output_columns(pred_df: pd.DataFrame) -> pd.DataFrame:
    """
    v8.6.38 external-output filter.
...
def main() -> None:
    ...
    if getattr(args, "asset_list", None) or getattr(args, "asset_preset", None):
        ...
        batch_root = Path(args.result_dir or "results_xgb_normal_unrestricted_v8_6_38_multi_asset")
        ...
        for ticker in tickers:
            ...
            summary_path = out_dir / f"{safe}_xgb_normal_unrestricted_v8_6_38_summary.json"
    ...
    cfg.result_dir = f"results_xgb_normal_unrestricted_v8_6_38_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.38: 기본 상태는 prob_high_vol 기반 포트폴리오, 강한 신호만 즉시 override한다.
        cfg.use_vol_probability_base_allocation = True
        # v8.6.38: 정상/저변동 상태 기본 비중 상향.
        ...
        # v8.6.38: Tier2를 기본 활성화한다. 기존 실패 구간 재검증을 위해 CLI로 조건/비중 조정 가능.
        ...
        # v8.6.38: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
    ...
    if getattr(args, "no_horizon_train_window", False):
        ...
        # v8.6.38: apply short horizon-train preset before low-level manual overrides.
        if getattr(args, "horizon_train_rows", None) is not None:
            _h5, _h10, _h20 = parse_horizon_train_rows(args.horizon_train_rows)
            set_horizon_train_rows_exact(cfg, _h5, _h10, _h20, preset_name="custom_rows")
        else:
            apply_horizon_train_preset(cfg, getattr(args, "horizon_train_preset", "moderate_1y_2y_3y"))
    ...
    file_prefix = f"{safe_ticker}_xgb_normal_unrestricted_v8_6_38"
