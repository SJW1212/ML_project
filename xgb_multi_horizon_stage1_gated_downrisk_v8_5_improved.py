"""
XGBoost v8.4 - Stable Condition Search + Optimization Diagnostics for Stage1-Gated Down-risk Allocation
====================================================================
...
class Config:
    ...
    result_dir: str = "results_xgb_v8_4_stable_conditions"
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    if cfg.use_prob_ewma:
        prob_cols = [
            ...
            if c.startswith("prob_high_vol") or c.startswith("prob_down_risk")
        ]
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_multi_horizon_stage1_gated_downrisk_v8_5_diagnostics",
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.5 Diagnostics 결과 요약")
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_v8_4_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_4_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_v8_4_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.4 Objective Condition Search + Optimization Diagnostics")
...
def main() -> None:
    ...
    df["stock_next_return"] = returns_df["stock_next_return"]
    df["bond_next_return"] = returns_df["bond_next_return"]
    df["cash_next_return"] = returns_df["cash_next_return"]
    ...
    pred_path = result_dir / "qqq_xgb_v8_5_predictions.csv"
    summary_path = result_dir / "qqq_xgb_v8_5_summary.json"
    latest_path = result_dir / "qqq_xgb_v8_5_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_v8_5_stage1_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_v8_5_downrisk_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_v8_5_condition_search.csv"
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_v8_5_{name}.csv"
