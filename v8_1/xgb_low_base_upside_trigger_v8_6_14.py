"""
XGBoost v8.6.14 - Mid-Base Loose Upside Trigger Allocation
====================================================================
...
class Config:
    ...
    extreme_bond_weight: float = 0.38
    extreme_cash_weight: float = 0.17

    # Base bucket allocations: low-base regime
    normal_stock_weight: float = 0.82
    normal_bond_weight: float = 0.12
    normal_cash_weight: float = 0.06

    watch_stock_weight: float = 0.75
    watch_bond_weight: float = 0.17
    watch_cash_weight: float = 0.08

    high_vol_stock_weight: float = 0.68
    high_vol_bond_weight: float = 0.22
    high_vol_cash_weight: float = 0.10

    risk_off_stock_weight: float = 0.60
    risk_off_bond_weight: float = 0.27
    risk_off_cash_weight: float = 0.13
    ...
    result_dir: str = "results_xgb_low_base_upside_trigger_v8_6_14"
    ...
    # v8.6.14 Mid-Base Loose Upside Trigger
    # - 5-class argmax를 allocation에 직접 쓰지 않는다.
    # - UP_STRENGTHENING은 low-risk 구간의 stock bonus로만 사용한다.
    # - bear_stress DOWN_STRENGTHENING은 high-vol 구간의 risk penalty로만 사용한다.
    use_direction_strength_specialist: bool = True
    direction_strength_horizon: int = 20
    ...
    # v8.6.14 low-base upside trigger thresholds
    # 기본 저비중 상태에서 UP_STRENGTHENING 확률이 높을 때만 단계적으로 공격 전환한다.
    up_strength_bonus_threshold_1: float = 0.30
    up_strength_bonus_threshold_2: float = 0.40
    up_strength_bonus_threshold_3: float = 0.50
    bear_down_strength_cut_threshold_1: float = 0.55
    bear_down_strength_cut_threshold_2: float = 0.65
    direction_strength_max_stock_bonus: float = 0.25
    ...
    # Low-base upside trigger target weights
    # 약한 상승 강화: 85%, 중간 상승 강화: 92%, 강한 상승 강화: 100%
    up_strength_offensive_stock_weight_1: float = 0.90
    up_strength_offensive_stock_weight_2: float = 0.96
    up_strength_offensive_stock_weight_3: float = 1.00
    up_strength_low_vol_threshold_1: float = 0.70
    up_strength_low_vol_threshold_2: float = 0.62
    up_strength_low_vol_threshold_3: float = 0.55
    up_strength_bear_block_threshold: float = 0.75

    # v8.6.14 diagnostics / stale offensive de-risking
    # 기본값 True: 저비중 기본형에서는 이전 공격 비중이 오래 남으면 전략 의도가 깨진다.
    enable_stale_offensive_decay: bool = True
    stale_offensive_stock_gap_threshold: float = 0.07
    stale_offensive_up_strength_reset_threshold: float = 0.22
    stale_offensive_high_vol_threshold: float = 0.62
...
def run_walk_forward(df: pd.DataFrame, feature_cols: List[str], cfg: Config) -> pd.DataFrame:
    ...
    strength_y_full, strength_valid_full, strength_aux_full = build_direction_strength_labels(all_df, cfg)
    ...
    strength_models: Dict[str, Dict[str, object]] = {}
    ...
    for k, pos in enumerate(candidate_positions):
        ...
        if need_retrain:
            ...
            if bool(getattr(cfg, "use_direction_strength_specialist", True)):
                try:
                    ...
                    y_strength_train, valid_strength_train, _strength_aux_train = build_direction_strength_labels(strength_train_df, cfg)
                    strength_models = {
                        "upside": fit_direction_strength_specialist(
                            strength_train_df,
                            feature_cols,
                            y_strength_train,
                            valid_strength_train,
                            str(getattr(cfg, "upside_strength_train_filter", "major_only")),
                            cfg,
                        ),
                        "bear": fit_direction_strength_specialist(
                            strength_train_df,
                            feature_cols,
                            y_strength_train,
                            valid_strength_train,
                    ...
                    warnings.warn(f"direction-strength specialist training skipped: {exc}", RuntimeWarning)
                    strength_models = {}
        ...
        up_strength_probs = predict_direction_strength_one(strength_models.get("upside", {}), row_df) if bool(getattr(cfg, "use_direction_strength_specialist", True)) else {name: 0.0 for name in DIRECTION_STRENGTH_LABELS}
        bear_strength_probs = predict_direction_strength_one(strength_models.get("bear", {}), row_df) if bool(getattr(cfg, "use_direction_strength_specialist", True)) else {name: 0.0 for name in DIRECTION_STRENGTH_LABELS}
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    """
    v8.6.14: allocation gate에서 사용할 하락 위험 점수.
...
def build_direction_strength_labels(df: pd.DataFrame, cfg: Config) -> Tuple[pd.Series, pd.Series, pd.DataFrame]:
    """5-class 방향·추세강도 라벨. trend_delta는 label 생성에만 사용하고 feature에는 넣지 않는다."""
    h = int(getattr(cfg, "direction_strength_horizon", 20))
...
def apply_direction_strength_specialist_policy(
    ...
    설계 의도:
    - 기본 주식 비중은 75~82% 수준의 중간 방어형으로 둔다.
    - 하락 탐지는 allocation에서 기본 제외하고 Stage1 high-vol만 위험 차단기로 사용한다.
    - UP_STRENGTHENING 확률 조건을 완화해 90% -> 96% -> 100%로 더 자주 공격 전환한다.
    - 공격 신호가 사라졌는데 이전 공격 비중이 남는 stale hold는 기본적으로 차단한다.
    """
    ph = _row_float(row, "prob_high_vol")
    pdn_raw = _row_float(row, "prob_down", _row_float(row, "prob_down_risk"))
    pdn_alloc = allocation_downrisk_score(ph, pdn_raw, cfg)
    pus = _row_float(row, "prob_up_strengthening")
    puw = _row_float(row, "prob_up_weakening")
    ...
    # Low-base upside trigger:
    # 기본 비중이 낮기 때문에, 상승 강화 신호가 일정 수준 이상일 때만 단계적으로 주식 비중을 올린다.
    low_vol_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.70))
    low_vol_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.62))
    low_vol_3 = float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.55))
    t1 = float(getattr(cfg, "up_strength_bonus_threshold_1", 0.30))
    t2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.40))
    t3 = float(getattr(cfg, "up_strength_bonus_threshold_3", 0.50))
    ...
    if regime in {"NORMAL", "WATCH"} and pbd < block_bear:
        if ph < low_vol_1 and pus >= t1:
            stock = max(stock, float(getattr(cfg, "up_strength_offensive_stock_weight_1", 0.90)))
            bonus = max(bonus, stock - base_stock)
            offensive_active = True
            offensive_tier = max(offensive_tier, 1)
        if ph < low_vol_2 and pus >= t2 and puw < 0.80:
            stock = max(stock, float(getattr(cfg, "up_strength_offensive_stock_weight_2", 0.96)))
            bonus = max(bonus, stock - base_stock)
            offensive_active = True
            offensive_tier = max(offensive_tier, 2)
        if ph < low_vol_3 and pus >= t3 and puw < 0.75:
            stock = max(stock, float(getattr(cfg, "up_strength_offensive_stock_weight_3", 1.00)))
            bonus = max(bonus, stock - base_stock)
            offensive_active = True
            offensive_tier = max(offensive_tier, 3)

    # Optional bear cut:
    # 기본 OFF. 통합 결과에서 bear specialist ROC-AUC가 약했으므로, 실험용으로만 둔다.
    if bool(getattr(cfg, "use_bear_specialist_cut", False)):
        ...
        if ph >= 0.75 and pbd >= float(getattr(cfg, "bear_down_strength_cut_threshold_2", 0.65)):
            cut += 0.02
        # DOWN_WEAKENING은 precision이 낮으므로 큰 완화 신호로 쓰지 않는다.
    ...
    # RISK_OFF/EXTREME_RISK에서는 offensive overlay로 방어를 훼손하지 않는다.
    ...
    meta = {
        ...
        "direction_strength_score": float(pus - pbd),
        ...
        "policy_note": "mid_base_loose_upside_trigger_stage1_risk_gate",
    }
    return w, meta
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        if prev_w is None:
            ...
            # v8.6.14 optional: offensive target이 사라졌는데 이전 98~100% 비중이 과도하게 남는 문제를 실험적으로 제어한다.
            # 기본값은 OFF다. 켜려면 --enable-stale-offensive-decay를 사용한다.
            stale_gap = float(prev_w[0] - signal_w[0])
            stale_up_prob = float(row.get("prob_up_strengthening", 0.0))
            stale_decay = (
        ...
        out.update({
            ...
            "stale_offensive_hold": bool((w[0] - signal_w[0]) >= float(getattr(cfg, "stale_offensive_stock_gap_threshold", 0.055)) and float(row.get("prob_up_strengthening", 0.0)) < float(getattr(cfg, "stale_offensive_up_strength_reset_threshold", 0.20))),
        })
        rows.append(out)
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_low_base_upside_trigger_v8_6_14_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print("\n==============================")
    print("XGBoost v8.6.9 Diagnostics 결과 요약")
...
def make_condition_candidate_configs(base_cfg: Config, grid_size: str = "standard") -> List[Tuple[str, Config]]:
    ...
    if grid_size == "compact":
        ...
        # v8.6.14 standard: v8.6.10의 과도한 방어/turnover를 줄이는 공격형 후보군
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_low_base_upside_trigger_v8_6_14_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_low_base_upside_trigger_v8_6_14_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_low_base_upside_trigger_v8_6_14_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.14 Mid-Base Loose Upside Trigger + Objective Condition Search")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.14 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.14 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.14 기본 ON인 stale offensive decay 비활성화")
    ...
    parser.add_argument("--stale-offensive-gap", type=float, default=None, help="stale offensive decay를 작동시키는 stock gap. 기본 0.055")
    parser.add_argument("--stale-offensive-reset-threshold", type=float, default=None, help="UP_STRENGTHENING 확률이 이 값보다 낮으면 stale offensive로 간주. 기본 0.20")
    parser.add_argument("--stale-offensive-high-vol-threshold", type=float, default=None, help="high-vol 확률이 이 값 이상이면 stale offensive decay 허용. 기본 0.62")
...
def main() -> None:
    ...
    cfg.result_dir = f"results_xgb_low_base_upside_trigger_v8_6_14_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.14: Mid-Base Loose Upside Trigger
        # - 기본 주식 비중을 중간 수준으로 두고, 상승 강화 조건을 널널하게 적용해 더 자주 공격 전환한다.
        ...
        cfg.no_trade_band = max(cfg.no_trade_band, 0.20)
        # v8.6.14: v8.6.13보다 기본 비중을 높인 mid-base 구조.
        # 너무 낮은 기본 비중으로 인한 CAGR 붕괴를 막되, v8.6.12보다 낮은 평균 주식 비중을 목표로 한다.
        cfg.normal_stock_weight = 0.82
        cfg.normal_bond_weight = 0.12
        cfg.normal_cash_weight = 0.06
        cfg.watch_stock_weight = 0.75
        cfg.watch_bond_weight = 0.17
        cfg.watch_cash_weight = 0.08
        cfg.risk_off_stock_weight = 0.60
        cfg.risk_off_bond_weight = 0.27
        cfg.risk_off_cash_weight = 0.13
        cfg.extreme_high_vol_threshold = 0.86
        cfg.extreme_downrisk_threshold = 0.86
        cfg.extreme_stock_weight = 0.45
        cfg.extreme_bond_weight = 0.38
        cfg.extreme_cash_weight = 0.17
        ...
        cfg.up_strength_bonus_threshold_2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.40))
        cfg.up_strength_bonus_threshold_3 = float(getattr(cfg, "up_strength_bonus_threshold_3", 0.50))
        cfg.up_strength_low_vol_threshold_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.70))
        cfg.up_strength_low_vol_threshold_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.62))
        cfg.up_strength_low_vol_threshold_3 = float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.55))
        cfg.up_strength_bear_block_threshold = float(getattr(cfg, "up_strength_bear_block_threshold", 0.75))
        cfg.direction_strength_max_stock_bonus = max(float(getattr(cfg, "direction_strength_max_stock_bonus", 0.25)), 0.25)
        cfg.enable_stale_offensive_decay = bool(getattr(cfg, "enable_stale_offensive_decay", True))
        # v8.6.14: 공격 전환을 더 오래 허용하되, 과도한 stale hold는 차단한다.
        cfg.stale_offensive_stock_gap_threshold = float(getattr(cfg, "stale_offensive_stock_gap_threshold", 0.07))
        cfg.stale_offensive_up_strength_reset_threshold = float(getattr(cfg, "stale_offensive_up_strength_reset_threshold", 0.22))
        cfg.stale_offensive_high_vol_threshold = float(getattr(cfg, "stale_offensive_high_vol_threshold", 0.62))
    ...
    # v8.6.14 direct low-base overrides after policy defaults.
    if getattr(args, "base_normal_stock", None) is not None:
        cfg.normal_stock_weight = float(args.base_normal_stock)
        rem = max(0.0, 1.0 - cfg.normal_stock_weight); cfg.normal_bond_weight = rem * 2/3; cfg.normal_cash_weight = rem * 1/3
    if getattr(args, "base_watch_stock", None) is not None:
    ...
    # direction_strength_horizon이 기존 horizons에 없으면 자동 포함한다.
    if int(getattr(cfg, "direction_strength_horizon", 20)) not in tuple(cfg.horizons):
        cfg.horizons = tuple(sorted(set(tuple(cfg.horizons) + (int(cfg.direction_strength_horizon),))))
    ...
    print("[3/5] Walk-forward Stage1 + 기존 Up/Down + Direction-Strength Specialist 예측")
    ...
    pred_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_14_predictions.csv"
    summary_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_14_summary.json"
    latest_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_14_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_14_stage1_feature_importance.csv"
    importance_up_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_14_up_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_14_downrisk_feature_importance.csv"
    importance_down_price_trend_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_14_downrisk_price_trend_feature_importance.csv"
    importance_down_price_volume_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_14_downrisk_price_volume_feature_importance.csv"
    importance_down_volatility_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_14_downrisk_volatility_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_14_condition_search.csv"
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_low_base_upside_trigger_v8_6_14_{name}.csv"
