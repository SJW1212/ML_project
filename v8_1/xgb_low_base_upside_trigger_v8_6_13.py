"""
XGBoost v8.6.9 - Direction-Strength Specialist Allocation
====================================================================
...
class Config:
    ...
    normal_stock_weight: float = 0.70
    normal_bond_weight: float = 0.20
    normal_cash_weight: float = 0.10

    watch_stock_weight: float = 0.60
    watch_bond_weight: float = 0.25
    watch_cash_weight: float = 0.15

    high_vol_stock_weight: float = 0.50
    high_vol_bond_weight: float = 0.33
    high_vol_cash_weight: float = 0.17

    risk_off_stock_weight: float = 0.45
    risk_off_bond_weight: float = 0.35
    risk_off_cash_weight: float = 0.20
    ...
    result_dir: str = "results_xgb_low_base_upside_trigger_v8_6_13"
    ...
    # v8.6.13 Low-Base Upside Trigger
    ...
    # v8.6.13 low-base upside trigger thresholds
    # 기본 저비중 상태에서 UP_STRENGTHENING 확률이 높을 때만 단계적으로 공격 전환한다.
    up_strength_bonus_threshold_1: float = 0.36
    up_strength_bonus_threshold_2: float = 0.45
    up_strength_bonus_threshold_3: float = 0.55
    ...
    up_strength_offensive_stock_weight_1: float = 0.85
    up_strength_offensive_stock_weight_2: float = 0.92
    up_strength_offensive_stock_weight_3: float = 1.00
    up_strength_low_vol_threshold_1: float = 0.55
    up_strength_low_vol_threshold_2: float = 0.50
    up_strength_low_vol_threshold_3: float = 0.45
    up_strength_bear_block_threshold: float = 0.75

    # v8.6.13 diagnostics / stale offensive de-risking
    # 기본값 True: 저비중 기본형에서는 이전 공격 비중이 오래 남으면 전략 의도가 깨진다.
    enable_stale_offensive_decay: bool = True
    stale_offensive_stock_gap_threshold: float = 0.04
    stale_offensive_up_strength_reset_threshold: float = 0.30
    stale_offensive_high_vol_threshold: float = 0.55
...
def allocation_downrisk_score(prob_high_vol: float, prob_down_risk: float, cfg: Config) -> float:
    """
    v8.6.13: allocation gate에서 사용할 하락 위험 점수.
...
def apply_direction_strength_specialist_policy(
    ...
    설계 의도:
    - 기본 주식 비중은 60~70% 수준으로 낮게 둔다.
    - 하락 탐지는 allocation에서 기본 제외하고 Stage1 high-vol만 위험 차단기로 사용한다.
    - UP_STRENGTHENING 확률이 높을 때만 85% -> 92% -> 100%로 단계적 공격 전환한다.
    ...
    low_vol_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.55))
    low_vol_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.50))
    low_vol_3 = float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.45))
    t1 = float(getattr(cfg, "up_strength_bonus_threshold_1", 0.36))
    t2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.45))
    t3 = float(getattr(cfg, "up_strength_bonus_threshold_3", 0.55))
    ...
    if regime in {"NORMAL", "WATCH"} and pbd < block_bear:
        if ph < low_vol_1 and pus >= t1:
            stock = max(stock, float(getattr(cfg, "up_strength_offensive_stock_weight_1", 0.85)))
    ...
    meta = {
        ...
        "policy_note": "low_base_upside_trigger_stage1_risk_gate",
    }
    return w, meta
...
def apply_allocation(pred_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    for i, row in pred_df.iterrows():
        ...
        if prev_w is None:
            ...
            # v8.6.13 optional: offensive target이 사라졌는데 이전 98~100% 비중이 과도하게 남는 문제를 실험적으로 제어한다.
...
def build_summary(pred_df: pd.DataFrame, feature_cols: List[str], gate_usage: Dict[str, int], cfg: Config) -> Dict[str, object]:
    ...
    return {
        "model_type": "xgb_low_base_upside_trigger_v8_6_13_diagnostics",
        "policy_mode": str(getattr(cfg, "policy_mode", "base")),
...
def make_condition_candidate_configs(base_cfg: Config, grid_size: str = "standard") -> List[Tuple[str, Config]]:
    ...
    if grid_size == "compact":
        ...
        # v8.6.13 standard: v8.6.10의 과도한 방어/turnover를 줄이는 공격형 후보군
...
def apply_speed_profile(cfg: Config, profile: str) -> Config:
    if profile == "fast":
        ...
        cfg.result_dir = "results_xgb_low_base_upside_trigger_v8_6_13_fast"
        ...
        cfg.use_adaptive_label_policy = False
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_low_base_upside_trigger_v8_6_13_balanced"
    elif profile == "full":
        cfg.retrain_every_n_days = 10
        ...
        cfg.use_rolling_gate_optimization = False
        cfg.result_dir = "results_xgb_low_base_upside_trigger_v8_6_13_full_adaptive_label"
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="XGBoost v8.6.13 Low-Base Upside Trigger + Objective Condition Search")
    ...
    parser.add_argument("--policy-mode", choices=["base", "return_seeking", "defensive_risk", "aggressive_dynamic", "direction_strength_specialist", "strength_specialist", "ds_specialist"], default=None, help="v8.6.13 policy branch")
    parser.add_argument("--no-policy-overlay", action="store_true", help="policy overlay를 끄고 v8.6.5 base bucket만 사용")
    parser.add_argument("--no-direction-strength", action="store_true", help="v8.6.13 up-strength offensive overlay 모델 학습/사용 비활성화")
    ...
    parser.add_argument("--disable-stale-offensive-decay", action="store_true", help="v8.6.13 기본 ON인 stale offensive decay 비활성화")
...
def main() -> None:
    ...
    cfg.result_dir = f"results_xgb_low_base_upside_trigger_v8_6_13_{cfg.policy_mode}"
    if cfg.policy_mode == "defensive_risk":
        ...
        # v8.6.13: Low-Base Upside Trigger
        # - 기본 주식 비중을 낮게 두고, 상승 강화 확률이 높을 때만 공격 전환한다.
        ...
        cfg.no_trade_band = max(cfg.no_trade_band, 0.18)
        cfg.normal_stock_weight = 0.70
        cfg.normal_bond_weight = 0.20
        cfg.normal_cash_weight = 0.10
        cfg.watch_stock_weight = 0.60
        cfg.watch_bond_weight = 0.25
        cfg.watch_cash_weight = 0.15
        cfg.risk_off_stock_weight = 0.45
        cfg.risk_off_bond_weight = 0.35
        cfg.risk_off_cash_weight = 0.20
        ...
        cfg.up_strength_bonus_threshold_1 = float(getattr(cfg, "up_strength_bonus_threshold_1", 0.36))
        cfg.up_strength_bonus_threshold_2 = float(getattr(cfg, "up_strength_bonus_threshold_2", 0.45))
        cfg.up_strength_bonus_threshold_3 = float(getattr(cfg, "up_strength_bonus_threshold_3", 0.55))
        cfg.up_strength_low_vol_threshold_1 = float(getattr(cfg, "up_strength_low_vol_threshold_1", 0.55))
        cfg.up_strength_low_vol_threshold_2 = float(getattr(cfg, "up_strength_low_vol_threshold_2", 0.50))
        cfg.up_strength_low_vol_threshold_3 = float(getattr(cfg, "up_strength_low_vol_threshold_3", 0.45))
        ...
        cfg.stale_offensive_stock_gap_threshold = float(getattr(cfg, "stale_offensive_stock_gap_threshold", 0.04))
        cfg.stale_offensive_up_strength_reset_threshold = float(getattr(cfg, "stale_offensive_up_strength_reset_threshold", 0.30))
    ...
    # v8.6.13 direct low-base overrides after policy defaults.
    if getattr(args, "base_normal_stock", None) is not None:
        cfg.normal_stock_weight = float(args.base_normal_stock)
        rem = max(0.0, 1.0 - cfg.normal_stock_weight); cfg.normal_bond_weight = rem * 2/3; cfg.normal_cash_weight = rem * 1/3
    if getattr(args, "base_watch_stock", None) is not None:
    ...
    pred_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_13_predictions.csv"
    summary_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_13_summary.json"
    latest_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_13_latest.json"
    importance_stage1_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_13_stage1_feature_importance.csv"
    importance_up_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_13_up_feature_importance.csv"
    importance_down_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_13_downrisk_feature_importance.csv"
    importance_down_price_trend_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_13_downrisk_price_trend_feature_importance.csv"
    importance_down_price_volume_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_13_downrisk_price_volume_feature_importance.csv"
    importance_down_volatility_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_13_downrisk_volatility_feature_importance.csv"
    condition_search_path = result_dir / "qqq_xgb_low_base_upside_trigger_v8_6_13_condition_search.csv"
    ...
    for name, diag_df in diagnostics.items():
        if diag_df is not None and not diag_df.empty:
            pth = result_dir / f"qqq_xgb_low_base_upside_trigger_v8_6_13_{name}.csv"
